...
- **`semantic-merge`** owns *semantic sync discipline* — intent investigation, conflict resolution, workflow sync author/reviewer mode references, standalone full-merge mode, Sync Map / impact formats, sync commit boundaries, and intent-changing conflict escalation.
- **`refactor-and-integrate`** owns *generic post-sync integration discipline* — drift-test construction, code-quality standards, project-doc audit, task-local Sync impact propagation, and minimum surviving branch delta. Domain-specific integration content lives alongside the domain skill (for data analysis: `econ-data-analysis/references/integration.md`).
