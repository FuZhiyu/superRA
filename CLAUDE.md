...
- **`semantic-merge`** owns *techniques for semantic coherence* — intent investigation, role classification, conflict resolution, intent-changing escalation, detect-and-resolve stale references, and propagation commits landed until semantic coherence (`SKILL.md §Semantic Coherence Checklist §Scope boundary`) is reached. Also owns the workflow sync author/reviewer mode references, standalone full-merge mode, and Sync Map / impact formats.
