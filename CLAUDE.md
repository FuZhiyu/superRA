...
- **Direct mode reads skill-owned role mirrors.** Main-agent direct mode loads `skills/using-superRA/references/direct-mode-implementer.md` and `direct-mode-reviewer.md`, not raw `agents/*.md`, because cross-repo plugin use cannot assume the main session can open repo-relative agent files outside the shared skill surface.
- **Manual mirrors must be updated with the canonical role files.** The direct-mode role references are temporary manual mirrors of `agents/implementer.md` and `agents/reviewer.md`. Any change to direct-mode-relevant role protocol in the canonical agent files must update the matching mirror in the same change until an automated sync exists.
