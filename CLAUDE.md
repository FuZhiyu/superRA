...
- **Write for the loading agent, not the developer.** A skill body is runtime instruction for the agent that loads it, not a design document for a contributor reading the repo. Teach the agent what to do next; keep rationale, alternatives, and design history in contributor docs, commit messages, and PRs. A line that explains *why the skill is shaped this way* rather than *what to do* is aimed at the wrong reader — cut it.
- **Partition a multi-role skill by loader.** When more than one role loads a skill — e.g. a main-agent orchestrator and a dispatched implementer both load a domain skill — put each role's instructions where only that role loads them: shared framing and the reference load map in `SKILL.md`, orchestration (interactive setup, fan-out/executor, convergence) in a main-agent reference, per-item domain discipline in the references the subagent loads. Test: an implementer dispatched for one unit of work should not load the orchestration it never runs, and the main agent should not carry per-item protocol inline. Annotate the load map with which role loads each entry.
