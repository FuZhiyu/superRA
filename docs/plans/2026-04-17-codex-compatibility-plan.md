...
>    → implemented: `tests/structural-invariants.sh` deletion absorbed via merge (main commit `a635dac`); the bash call at the old line 60 of `tests/check-harness-compatibility.sh` was removed and replaced with a new "Codex skill packaging invariants" Python section (~50 lines) that inlines (i) frontmatter-parses + `description ≤ 500` chars across `skills/*/SKILL.md` and (ii) `.agents/skills/` symlink coverage of every `skills/<name>/`. SURFACED CONCERN: the inlined `≤500` check fails on 7 main-side `SKILL.md` files (`econ-data-analysis` 513, `handoff-doc` 627, `implementation-workflow` 708, `integration-workflow` 1008, `planning-workflow` 846, `refactor-and-integrate` 613, `semantic-merge` 839) — main's PR #12 lengthened descriptions after `a635dac` removed the enforcing test. Returned to orchestrator for adjudication; check kept at the dispatch-mandated 500.
>
