...
| `guard-foreign-checkout` | `PreToolUse` on `Edit\|Write\|Bash\|apply_patch` | Hard-denies a `task.md` write whose task root belongs to neither the session's cwd nor its repository, and a git history-writing command redirected (`cd`, `git -C`) into another task-tree checkout. Worktrees of the session's own repository are not foreign. Codex shell interception is incomplete, so the `Bash` half is best-effort there. |
