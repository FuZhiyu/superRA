...
**For development or forking:** if you want to modify superRA itself — edit skills, add a domain vertical, tune hooks — install from a local clone instead:

```bash
git clone https://github.com/FuZhiyu/superRA.git
```

The repo's committed marketplace file targets the published GitHub source so remote installs work cleanly.
For a live local-development install, create your own personal marketplace entry that points directly at your clone instead of relying on the repo's committed marketplace file — `claude plugin marketplace add /path/to/your/clone` — then run the same `claude plugin install` command above against it.
