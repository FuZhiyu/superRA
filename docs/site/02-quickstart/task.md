...
3. **Mature & Consolidate** — create the user-facing documentation and result files, mature the task tree against them, and prepare one temporary task containing proposed pruning and other refactoring opportunities. You review the finished record and that task together.
4. **Integrate** — execute the approved refactoring task mechanically, then verify that the protected record and its required support paths still hold.
