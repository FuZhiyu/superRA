...
| **ensure-agent-orchestration** | `PreToolUse` on `Skill(superRA:superplan|superimplement|superintegrate)` | Same pattern as above, gating on `superRA:agent-orchestration`. | Yes | — | — |
...
Claude Code has the richest hook surface: all seven hooks fire.
Codex does not support `PreToolUse` on `Skill` calls, so `ensure-using-superra` and `ensure-agent-orchestration` are absent; Codex shell interception (`task-hook`) is also incomplete, making task-tree reconciliation best-effort.
Cursor supports `userPromptSubmit`, `preToolUse`, and `postToolUse` but does not have an `ExitPlanMode` analog or a `Stop` event.
