...
- [**reproducibility**](skills/reproducibility/SKILL.md) — declares producers and checks, diagnoses change impact, and verifies results through selective builds or evidence-backed reuse. Ask "which results does this helper edit affect?" before choosing what to rerun.
- [**result-protection**](#/04-utility-skills/03-result-protection) — helps choose the permanent record and whether documentation alone or an additional red-green-verified drift test should protect each key result.
...
- [**communicate**](#/04-utility-skills/05-communicate) — makes conversation, task files, reports, reviews, and handoffs skimmable: outcome first, evidence and caveats next, linked mechanics last. Its references also own source links, KaTeX-safe math, tables, figures, and standalone-report IO.
