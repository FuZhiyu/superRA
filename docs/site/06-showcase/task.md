...
**A demo analysis — a project mid-flight.** A small, hypothetical research project: estimating whether firm size and price momentum predict next-month stock returns, on a fully simulated equity panel. The data are generated by a known process, so there is a built-in right answer — no real market data, tickers, or vendor records are involved. The tree is deliberately caught in the middle of its work: some tasks are `approved`, one is `implemented` and waiting on review, one is in `revise` after a reviewer caught that the portfolios were equal-weighted instead of value-weighted, and the write-up has not started. This is the shape a real project takes most of the time — partly done, with the frontier showing what to pick up next. It is the same analysis the [Quickstart](#/02-quickstart) walks you through building.

[Open the demo task tree →](demo-tree.html)

**superRA's own development tree — the existence proof.** superRA is built with superRA. The second export is the actual `superRA/` task tree of this repository: the real plan behind the workflow, the dashboard, the skill library, and this documentation site you are reading. It is larger and messier than the demo, because real work is. If you want to know whether the task tree holds up on a non-trivial project, this is that project.

[Open superRA's own task tree →](superra-dev-tree.html)

**A real asset-pricing study — the executed counterpart.** The demo above is caught mid-flight on simulated data. This third tree is the opposite: a finished, real empirical study, run end-to-end through the workflow on fully public data. It is the canonical time-series test of linear factor models — estimate CAPM and the Fama-French three-factor model on Ken French's 25 portfolios sorted by size and book-to-market, then use the Gibbons-Ross-Shanken (GRS) joint test to ask whether either model prices the cross-section. Every task is `approved`, so you are reading the completed handoff: real regression tables, real figures embedded in the `## Results`, and the review history that got each task there. Open a task's `02-analysis` results to see the GRS verdict, or its `03-writeup` for the reader-facing narrative with the math. This is the same workflow the demo tree is mid-way through, carried all the way to a finished result.

[Open the asset-pricing study →](showcase-analysis-tree.html)

### How these are built

All three exports are produced by the task-tree dashboard's standalone export — the same `generate` command documented in the [task-tree skill](skills/task-tree/SKILL.md) — which writes a single self-contained HTML file with everything inlined, no server required. The exports are rebuilt fresh in CI on every deploy and are never committed to the repository; only the task sources are. The exact build invocation lives in the deploy pipeline, which regenerates each tree with:
...
- **Asset-pricing study:** `uv run --script skills/task-tree/scripts/plan_dashboard.py generate --plan-root superRA --root showcase-analysis` (the study is a subtree of superRA, scoped with `--root`)
