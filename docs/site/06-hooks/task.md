...
| **ensure-agent-orchestration** | `PreToolUse` on `Skill(superRA:superimplement|superintegrate)` | Same pattern as above, gating on `superRA:agent-orchestration`. `superplan` is ungated — it dispatches nothing at quick/standard depth and instructs the load at its own dispatch point. | Yes | — |
