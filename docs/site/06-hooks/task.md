...
| **guard-foreign-checkout** | `PreToolUse` on `Edit`, `Write`, and `Bash` (plus `apply_patch` on Codex) | Hard-denies mutating a task tree outside the checkout the session was pointed at: a `task.md` write whose task root belongs to neither the session's cwd nor its repository, and a git history-writing command redirected (`cd`, `git -C`) into another task-tree checkout. Worktrees of the session's own repository stay open, since dispatched implementers work there. | Yes | Yes |
