...
The default is **subagent mode**: a fresh implementer subagent per task, then a reviewer subagent that catches what self-review cannot.

**Direct mode** is the alternative for small, clearly-scoped tasks where dispatching two subagents costs more than it saves — the main agent implements the task itself. Direct mode still dispatches a separate reviewer, so review is never skipped either way.
