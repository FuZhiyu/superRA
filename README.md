...
| **task-system** | Directory-tree task system where the filesystem hierarchy is the task hierarchy. Each task is a `task.md` (plan + results unified) with sibling-only dependencies and automatic status rollup. Includes CLI scripts for CRUD, frontier computation, DAG rendering, migration from `PLAN.md` / `RESULTS.md`, and an HTML dashboard generator. Use when creating or querying a `.plan/` task tree, or visualizing progress. |
