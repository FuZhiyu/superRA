...
| **task-system** | Directory-tree task tooling. Filesystem hierarchy is the task hierarchy; each task is a `task.md` (plan + results unified) with sibling-only dependencies and automatic status rollup. Reading and editing your own task is the executing-agent interface in `using-superRA §Task Interface`. SKILL.md is the load-on-demand tree-tooling layer (query/frontier/DAG, dashboard, migration), with `references/commands.md` for the mutation command surface. Planner-facing `references/planning.md` covers objective writing, task splitting, and hierarchy management; contributor-facing `references/internals.md` covers the data layer, hook architecture, and migration. |
...
| **zotero-paper-reader** | Read and analyze academic papers from a Zotero library. Use when reading, finding, summarizing, or analyzing a paper by title, author, DOI, or topic. Handles the full workflow: search Zotero, retrieve the PDF, convert to markdown, and analyze in sections. Supports library queries (metadata search, full-text search, collection listing, tag listing, DOI-to-key lookup, attachment retrieval). |
