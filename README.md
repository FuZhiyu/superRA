...
| **zotero-paper-reader** | Read and analyze academic papers from a Zotero library. Use when reading, finding, summarizing, or analyzing a paper by title, author, DOI, or topic. Handles the full workflow: search Zotero, retrieve the PDF, convert to markdown, and analyze in sections. Supports library queries (metadata search, full-text search, collection listing, tag listing, DOI-to-key lookup). |
