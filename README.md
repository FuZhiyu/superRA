...
`repro build <task>` builds that task and its nested tasks using saved upstream inputs. Add `--upstream` to include producers and `--force` to rerun every step in that scope; `--upstream --force` replaces `--force-all`. Use `task#step` for one step. Build/status require a target (`.` for every registered step) and are scoped to it; reproduction tiers and `--tier` are retired; completion checks explicitly verify upstream too. See the [command reference](skills/task-tree/references/commands.md#reproduction).
