...
| **semantic-merge** | Intent-based semantic sync with mode references for workflow sync authoring, sync review, and standalone full merge. It classifies conflicts by role (behavior/API, data/schema, docs, generated outputs, tests, config/build), escalates intent-changing decisions to the user, lands sync / propagation commits as appropriate, and records branch-level Sync Map plus task-local or file-local impact notes. Use whenever you would otherwise run `git merge` / `git rebase` / `git cherry-pick` — the `merge-guard` hook flags bare invocations automatically. |
