...
- **Parallel-reviewer pattern added to `agent-orchestration`.** `skills/agent-orchestration/SKILL.md` §Parallelization and Worktree Isolation (was §Concurrent Writers Require Worktree Isolation before Round 3) extended with a paragraph covering parallel reviewers walking disjoint slices of a large diff — same worktree isolation, same disjoint-scope invariant, same orchestrator-aggregates-verdicts pattern. `Worktree:` dispatch field now applies to reviewers in this configuration.
- **`agent-orchestration §Parallelization and Worktree Isolation` generalized + renamed.** The section now covers any parallel subagent batch (implementers + reviewers + cross-role patterns), not just implementers. New subsections: §Difficulty and Agent Type (tier-matching guidance — lower-tier for simple / follow-up checks, higher-tier for first-pass adversarial review); §Worktree lifecycle clarifying that the orchestrator must create the worktree with raw git branching off its current branch, never via the `Agent`-tool `isolation: "worktree"` parameter (which branches off main's HEAD). Background-dispatch preference added to the opener. `--from "$(pwd)"` seeding guardrail added.
- **Branch-naming convention flipped to `<branch>/parallel/<slug>`.** Was `parallel/<branch>/<slug>`. The new layout lets the `merge-guard` hook exempt `*/parallel/*` source refs regardless of parent-branch name, which the old layout could not. Hook, skill, and agent references all swept.
- **Flexible integration base in `integration-workflow` Phase B.** Phase B no longer hardcodes `origin/main` as the integration target. `<base-branch>` defaults to `origin/main` and is researcher-overridable at `execution-workflow` Step 4 when the analysis branched off a release branch, a co-authored track, or another analysis's sibling branch.
- **Phase B Step 3b dispatch collapsed to a pointer.** The embedded `Agent(subagent_type: "superRA:implementer"): …` block in `integration-workflow` Phase B Step 3b is removed. Step 3b now points at `agent-orchestration §Dispatch Templates` for the canonical shape + lists the integration-stage additions (`Tasks in scope:`, refusal-to-refactor-APPROVED, mechanical merge first, drift tests after).
- **Integration-workflow §Red Flags triaged.** Was 11 Never + 12 Always. Now 4 Never + 1 Always. Kept only bullets that catch an RA rationalization specific to Phase B/C/D and not already owned by `refactor-and-integrate`, `using-superRA` §Universal Principles, or the phase prose itself. Cross-cutting pointer block added at the end of the section.
- **`refactor-and-integrate §Scope by Integration Status` removed.** Its load-bearing sentence (APPROVED-integration tasks out of scope for refactor) lifted into the Implementer Self-Check. Task scope now lives at dispatch time (Phase B Step 3b `Tasks in scope:` list) + in the self-check; no separate section needed.
- **`merge-quality.md` commit-structure reframed as intent-separation, not commit-count.** How-To renamed "Commit structure — one or more commits, separate when intent differs". The two-commit templates are the canonical example of intent separation, not a mandate — a trivial merge can legitimately land as one commit. Gated checklist §Commit structure simplified to one `[BLOCKING]` intent-separation item plus two `[ADVISORY]` discipline items (was two `[BLOCKING]` before).
- **Changing Plans protocol tightened.** `planning-workflow §Changing Plans` renamed to §User Feedback and Changing Plans. Step 3 now mandates walking the task list and modifying affected tasks in place (prefer modify over append); the new-task branch only triggers when the change is genuinely independent of every existing task. New Step 5 inserted: sweep PLAN.md for stale content per `handoff-doc` §Stale Content Checklist before committing. Prior Step 5 (commit) and Step 6 (resume) renumbered to 6 and 7. Standalone "DAG cascade on re-entry" paragraph removed — Step 4 carries the rule. `handoff-doc` §What Counts as Stale gains a bullet for task-output descriptions superseded by a later task. All active cross-references updated (seven files).
