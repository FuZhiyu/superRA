...
## Unreleased — workflow-domain-split restructure: DAV + dispatch-prompt standardization + shared §Review gating + CONDITIONAL APPROVE + execution-workflow domain-agnosticism + agent Stage tables + companion-workflow audit

One coordinated restructure, landed in six phased commits on `refactor/workflow-domain-split`. The goal: make superRA's workflow skills strictly domain-agnostic, make the implementer self-check and the reviewer verification walk a single shared checklist, collapse the costly two-stage review into one comprehensive pass with a `CONDITIONAL APPROVE` escape hatch, and anchor every agent dispatch to a canonical prefix so the dispatch prompt carries only steering on top of the agent's standard protocol. Data-analysis discipline is preserved — it just moves to its proper home in `econ-data-analysis`, leaving the workflow skeleton vertical-neutral.
