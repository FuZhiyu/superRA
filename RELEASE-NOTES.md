...
**Iterative re-entry mechanism.** `handoff-doc/SKILL.md §Scope Changes and Re-entry` (renamed from `§Mid-Session Scope Changes`) subsumes both in-execution researcher scope drift and post-integration scope additions (PR-review requests, adjacent features, follow-on ideas) under a single 6-step protocol. The rename reflects the expanded scope: the section now covers re-entry after integration or merge, not just mid-execution pings. Key additions to the protocol:

- **Per-task `**Integration status:**` field** added to the task-block template in `handoff-doc/references/plan-anatomy.md` alongside `**Review status:**`. Values: unset / IMPLEMENTED / REVISE / APPROVED. Set by the integration reviewer.
...
**`plan-anatomy.md` updates.** `**Integration status:**` appears in the task-block template, in the `## Workflow Status` header paragraph (with rollup semantics and re-entry unchecks), and in the Field-by-Field section with the same cascade rule as `**Review status:**`. The `## Workflow Status checkboxes` Field-by-Field bullet pointer updated from `§Mid-Session Scope Changes` → `§Scope Changes and Re-entry`.
...
**Wire-ups across workflow skills.** `execution-workflow` Step 1 gains a new sub-step reading per-task `**Review status:**` and `**Integration status:**` fields on re-entry and proposing the DAG downstream closure for invalidation, cross-referencing `handoff-doc §Scope Changes and Re-entry` for exemption protocol. `integration-workflow` Stage 1 adds the always-full-drift-suite rule and updates the `Drift tests created` box-flip to key on all tasks having `**Integration status:** APPROVED`; Stage 2 adds a refactorer scope clause and the same rollup for `Refactored`; Step 3 adds the doc-writer-always-full and doc-reviewer-diff rules with the `Docs finalized` box-flip updated to rollup. `planning-workflow` and `merge-workflow` receive the pointer rename from `§Mid-Session Scope Changes` → `§Scope Changes and Re-entry`. Stale pointers in `agent-orchestration/references/agent-teams.md`, `skills/CATEGORIES.md`, and both agent files' autonomy references updated to `main-agent.md`.

**Files touched.** `skills/handoff-doc/SKILL.md` (section rename + protocol extension + DAG cascade paragraph + banned shortcut); `skills/handoff-doc/references/plan-anatomy.md` (Integration status field + Workflow Status description + Field-by-Field cascade); `skills/execution-workflow/SKILL.md` (Step 1 sub-step + autonomy pointer); `skills/integration-workflow/SKILL.md` (always-full-drift-suite + refactor scope + doc-writer/reviewer scope + box-flip rollup language); `skills/planning-workflow/SKILL.md` (pointer rename); `skills/merge-workflow/SKILL.md` (no pointer existed — no change); `skills/using-superRA/SKILL.md` (main-agent.md pointer + manifest preamble); `skills/using-superRA/references/main-agent.md` (new); `skills/using-superRA/references/session-bootstrap.md` (deleted); `skills/using-superRA/references/main-agent-autonomy.md` (deleted); `agents/implementer.md` + `agents/reviewer.md` (header-ownership bullet in "You may NOT edit" list; autonomy pointer updated); `agent-orchestration/references/agent-teams.md` (autonomy pointer); `skills/CATEGORIES.md` (main-agent.md pointer).
