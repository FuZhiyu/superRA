...
## Unreleased — workflow-domain-split restructure: DAV + dispatch-prompt standardization + shared §Review gating + CONDITIONAL APPROVE + execution-workflow domain-agnosticism + agent Stage tables + companion-workflow audit + DRY/composability refactor (orchestration consolidation, alias-skill retirement, integration-workflow genericization, workflow-stage-to-skill map)

One coordinated restructure on `refactor/workflow-domain-split`, landed in two rounds. **Round 1** (six phased commits through `b8c6239`) made superRA's workflow skills strictly domain-agnostic, made the implementer self-check and the reviewer verification walk a single shared checklist, collapsed the costly two-stage review into one comprehensive pass with a `CONDITIONAL APPROVE` escape hatch, and anchored every agent dispatch to a canonical prefix so the dispatch prompt carries only steering on top of the agent's standard protocol. **Round 2** (Tasks 7–12) then took the design principle one level higher: cross-stage orchestration patterns consolidated into `agent-orchestration`; alias-only protocol skills retired; `integration-workflow` stripped to workflow-choreography only with data-specific integration discipline moved into a new domain reference; DRY / composability / extensibility + shared-flow elevated to documented design principles in `CLAUDE.md` and `README.md`; a workflow-stage-to-skill map added to the README. Data-analysis discipline is preserved throughout — it just moves to its proper home in `econ-data-analysis`, leaving the workflow skeleton vertical-neutral and extensible.
...
**Structural invariants.** `tests/structural-invariants.sh` is consolidated to cover both rounds. Invariant blocks added or updated: (§Review integration, block 11) `## Review & Self-Check Discipline` heading exists in `econ-data-analysis/SKILL.md`, `[GATING]` marker count ≥ 8, `CONDITIONAL APPROVE` verdict string present, no `implementation-review.md` / `integration-review.md` reference file exists under `skills/econ-data-analysis/references/`; (execution-workflow domain-agnosticism, block 12) `skills/execution-workflow/SKILL.md` free of `data integrity|two-stage review|REVISE \(data integrity\)|REVISE \(implementation\)` phrasing, `## Sensitivity Analysis Tasks` heading removed, `CONDITIONAL APPROVE` verdict present; (agent Stage tables + contract, block 13) both `agents/implementer.md` and `agents/reviewer.md` contain a Stage table whose rows include `implementation` / `implementation review` / `refactoring` / `integration review`, both contain the "What the dispatch prompt carries — and doesn't" contract phrase, and `agents/reviewer.md` encodes `CONDITIONAL APPROVE`; (dispatch-prompt prefix, blocks 10a+10b) every dispatch template in the five workflow-adjacent skill files carries the "Follow the standard stage-relevant workflow" prefix, and no template retains `Work from:` / `Counterpart:` fields; (DAV, blocks 3+3b+3c) `## Describe` / `## Analyze` / `## Validate` sections present in `econ-data-analysis/SKILL.md`, `### Sensitivity analysis` sub-section present, no DAD / describe-analyze-doc strings remain; (Round 2 — orchestration ownership, block 14) `agent-orchestration/SKILL.md` owns the five cross-stage sections (`## Dispatch Templates`, `## Dispatch-Return Deltas`, `## Handling Reviewer Feedback`, `## Review Status Reference`, `## Direct Mode`) and `execution-workflow/SKILL.md` no longer carries `## Dispatch Templates`; (Round 2 — alias-skill retirement, block 15) `skills/implementer-protocol/` and `skills/reviewer-protocol/` are absent; (Round 2 — data-integration reference, block 16) `integration.md` exists with ≥3 `[GATING]` markers and the shared-flow preamble phrase, `codebase-integration.md` no longer carries `## Economic Integration` or `## Data Discipline Through Refactoring`, `integration-workflow/SKILL.md` free of `winsorization|Economic Integration` tokens; (Round 2 — Workflow Map, block 17) `README.md` contains `## Workflow Map` with a Mermaid fence (or a ≥5-row pipe-table fallback); (block 18, renumbered) README 'Why superRA?' lead is workflow-first; (Round 2 — design-principle elevation, block 19) `CLAUDE.md` contains `DRY, composability, extensibility` and `README.md` is free of `two-stage review`. Full run on the final commit: all PASS, 2 known WARN (pre-existing upstream refs in `writing-skills`), 0 FAIL.
