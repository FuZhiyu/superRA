...
**`## PLAN.md Is the Task Tracker` doctrine.** A new section in `skills/handoff-doc/SKILL.md` (lines ~65–81) establishes `PLAN.md` as the single authoritative task tracker for analysis work — not `TodoWrite`, not chat, not status reports, not a session-internal scratchpad. `TodoWrite` (or any harness-provided todo UI) is narrowed to a transient working view: acceptable for ephemeral session-internal todos unrelated to the analysis, but not as a substitute for a PLAN.md task block. The section lists four banned patterns (tracking analysis tasks only in `TodoWrite`; discovering a subtask, completing it, and never updating `PLAN.md`; using `TodoWrite` to coordinate across sessions; treating `TodoWrite` items as "logged") and a precedence rule: if `TodoWrite` and `PLAN.md` disagree about the state of analysis work, `PLAN.md` is right by definition. Cross-referenced from `execution-workflow` Step 1 sub-step 8 and from the step-1 process flowchart node ("Read plan + Workflow Status; optionally mirror open tasks into TodoWrite"). This directly strengthens Workflow Principle P2 (handoff docs are the auditable record and continuation point): prior to this section, the agent file mentioned `TodoWrite` as an optional working view but did not state the precedence rule or the banned patterns explicitly.
