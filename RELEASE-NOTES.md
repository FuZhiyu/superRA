...
- **Three autoload hooks** that keep the superRA skill-load state coherent without requiring the user or the agent to remember it manually:
  ...
  - **`ensure-agent-orchestration`** (`PreToolUse:Skill`) — same pattern as above, gating on `superRA:agent-orchestration`. If both companions are missing on the same call, both hooks deny in parallel; Claude loads them across two retries and the third invocation passes through silently.
