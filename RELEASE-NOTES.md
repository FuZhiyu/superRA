...
## Unreleased — econ-data-analysis restructured around describe-analyze-validate (DAV); dispatch-prompt protocol standardized; §Review & Self-Check Discipline shared between implementer and reviewer; execution-workflow rewritten domain-agnostic with one-pass review

Four structural refactors land together.

**`execution-workflow` rewritten domain-agnostic with one-pass review.** `skills/execution-workflow/SKILL.md` no longer encodes data-analysis specifics — it routes domain-specific verification to the active domain skill's §Review & Self-Check Discipline. The two-stage review loop (data integrity → implementation correctness, two reviewer dispatches per task) collapses into **one comprehensive review pass** per task whose verdict is `APPROVE` / `REVISE` / `CONDITIONAL APPROVE`, matching the verdict protocol in `econ-data-analysis/SKILL.md §Review & Self-Check Discipline`. Stage names renamed per D-Stage-1: implementer `Stage: analysis task` → `Stage: implementation`; reviewer `Stage: data integrity` + `Stage: implementation` collapse to `Stage: implementation review`. The process flowchart replaces the two-node data-integrity → implementation dispatch chain with one "Dispatch reviewer (implementation review)" node with three verdict branches (APPROVE / REVISE / CONDITIONAL APPROVE — the last loops through a narrow re-review that verifies the gating fix plus cited downstream items). Step 3 becomes a five-check orchestrator skeleton whose fourth check points at the active domain skill's §Completion verification rather than hardcoding pipeline / outputs assumptions. The `## Sensitivity Analysis Tasks` section is deleted entirely (content lives in `econ-data-analysis/SKILL.md §Validate §Sensitivity analysis` and `references/data-robustness-checklist.md`); `## Model Selection` collapses from a data-flavored three-tier table to one paragraph that points at the domain skill for complexity examples. The dispatch-template preamble that hardcoded `superRA:econ-data-analysis` + `superRA:script-to-notebook` auto-loads is removed — agent Stage tables handle stage-based auto-load (Task 4 follow-up). Red Flags and the Integration table are scrubbed of two-stage-review references; the Review Status table collapses the two REVISE rows into one and adds a CONDITIONAL APPROVE row. The user's hand-edits (the "If you need a non-default skill load…" sentence and the Step 4 Option 1/2 collapsed narrative) are preserved.

Three pre-existing refactors from prior commits in this release:
...
`tests/structural-invariants.sh` now adds seven new assertions across the three refactors on top of the two dispatch-prefix assertions: (§Review integration) `## Review & Self-Check Discipline` heading exists in `econ-data-analysis/SKILL.md`, `[GATING]` marker count ≥ 8, `CONDITIONAL APPROVE` verdict string present, no `implementation-review.md` / `integration-review.md` reference file exists under `skills/econ-data-analysis/references/`; (execution-workflow domain-agnosticism) `skills/execution-workflow/SKILL.md` free of `data integrity|two-stage review|REVISE \(data integrity\)|REVISE \(implementation\)` phrasing, the `## Sensitivity Analysis Tasks` heading removed, `CONDITIONAL APPROVE` verdict present. Existing prefix-coverage and no-`Work from:`/`Counterpart:` assertions continue to pass. All assertions green on this commit.
