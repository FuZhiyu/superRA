...
## Unreleased — plan-stage-marker + mid-session scope-change protocol + plan-as-task-tracker doctrine

Three coordinated additions to `handoff-doc` and its downstream workflow wiring, landed on `feat/plan-stage-marker`. The gap they close: a fresh agent picking up an in-flight project could not tell from `PLAN.md` alone what stage of the workflow it was at — it had to grep commits, scan task statuses, and reason backward. Separately, researcher-initiated scope changes mid-session (new tasks, objective changes, methodology pivots) had no defined absorption protocol — the class-(b) stop point in `execution-workflow` named them as RA-authority-exceeding decisions, but no skill told the agent what to do with the answer once the researcher gave it. Both gaps could cause state corruption or silent duplication of work across sessions.

**`## PLAN.md Is the Task Tracker` doctrine.** A new section in `skills/handoff-doc/SKILL.md` (lines ~65–81) establishes `PLAN.md` as the single authoritative task tracker for analysis work — not `TodoWrite`, not chat, not status reports, not a session-internal scratchpad. `TodoWrite` (or any harness-provided todo UI) is narrowed to a transient working view: acceptable for ephemeral session-internal todos unrelated to the analysis, but not as a substitute for a PLAN.md task block. The section lists four banned patterns (tracking analysis tasks only in `TodoWrite`; discovering a subtask, completing it, and never updating `PLAN.md`; using `TodoWrite` to coordinate across sessions; treating `TodoWrite` items as "logged") and a precedence rule: if `TodoWrite` and `PLAN.md` disagree about the state of analysis work, `PLAN.md` is right by definition. Cross-referenced from `execution-workflow` Step 1 sub-step 1 (citation) and from the step-1 process flowchart node ("Read plan + Workflow Status; optionally mirror open tasks into TodoWrite"). This directly strengthens Workflow Principle P2 (handoff docs are the auditable record and continuation point): prior to this section, the agent file mentioned `TodoWrite` as an optional working view but did not state the precedence rule or the banned patterns explicitly.

**`## Mid-Session Scope Changes` protocol.** A new section in `skills/handoff-doc/SKILL.md` (lines ~112–147) defines a 6-step protocol for researcher-initiated scope changes: (1) confirm intent — a passing remark in chat is not authorization; (2) log the decision per §User Decisions Log; (3) update `PLAN.md` inline per the four sub-cases (new task / modified task / removed task / reordered tasks); (4) update `## Workflow Status` if the change reverts a completed milestone; (5) commit atomically with title `plan: <one-line scope change>`; (6) resume the appropriate workflow for the new state. The section draws a material/not-material line: adding or removing a task, changing a task's objective or I/O, and analysis-level methodology changes are material and require the protocol; rewording a step to match data-forced deviation, adjusting expected results from early findings, and refining methodology details already approved at planning time are not material and are handled as inline discovery edits per `planning-workflow`'s Living Plan section. Banned shortcuts mirror the tracker section: carrying the change in `TodoWrite` only, creating a `PLAN_v2.md`, resuming in-flight work before reflecting the change in the doc. Cross-referenced from `execution-workflow` Step 1 sub-step 8 (pointing at this section for mid-session scope change mechanics) and from `planning-workflow`'s Living Plan section (distinguishing agent-discovered drift from researcher-initiated scope changes, with a pointer here for the latter).

**`## Workflow Status` checklist.** A new 6-milestone checkbox list in `skills/handoff-doc/references/plan-anatomy.md` (embedded in the header template, lines ~46–59), placed between the standing context and the first `---` separator. Milestones in order: **Plan approved** (planning-workflow Phase 2), **Execution complete** (execution-workflow Step 3), **Drift tests created** (integration-workflow Stage 1), **Refactored** (integration-workflow Stage 2), **Docs finalized** (integration-workflow Step 3 doc-reviewer APPROVE), **Merged** (merge-workflow Step 4). The intent is stated in the checklist's own prose: "A new agent reads this section first to know where the project stands without having to scan task blocks or grep commits." Box-flip wiring added to each owning workflow skill in the same commit (`bbd57d1`): `planning-workflow` Phase 2 Execution Handoff flips **Plan approved**; `execution-workflow` Step 3 flips **Execution complete** after all five pipeline checks pass; `integration-workflow` Stage 1 step 8 flips **Drift tests created**; `integration-workflow` Stage 2 step 6 flips **Refactored** (with a conditional-uncheck clause: if a post-merge integration review or scope change touches refactored code, the box is unchecked until the next integration-reviewer APPROVE); `integration-workflow` Step 3 doc-reviewer APPROVE flips **Docs finalized** (before disposition, not after — rationale: by Sub-part C, `PLAN.md` may have been moved or removed); `merge-workflow` Step 4 flips **Merged**. `merge-workflow` Step 3 (post-merge entry) additionally unchecks **Refactored** if `PLAN.md` is still present (post-merge refactor invalidates the prior milestone until the next integration-reviewer APPROVE). Every flip site carries an explicit cross-reference to `superRA:handoff-doc` references/plan-anatomy.md. The `### Header ownership` paragraph in `plan-anatomy.md` was promoted from bold-prefix prose to a real `###` heading (commit `e6804be`) so these cross-references resolve unambiguously.

The **orchestrator-owned header rule** follows from the checklist: only the orchestrator (or standalone author) flips boxes; subagents may not touch the header including `## Workflow Status` and `## Decisions`. Both `agents/implementer.md` (line 99) and `agents/reviewer.md` (line 122) each gain a new bullet in their "You may NOT edit" list stating this constraint and pointing at `superRA:handoff-doc` references/plan-anatomy.md §Header ownership. If a subagent's work completes a milestone, it reports that in its status return and the orchestrator handles the box flip.

**Drift-test maintenance.** `tests/structural-invariants.sh` block 24 previously counted numbered principles across the entire `handoff-doc` SKILL.md file with a file-wide grep. The new `## Mid-Session Scope Changes` section includes a 6-step numbered protocol (`1. Confirm intent…`, `2. Log the decision…`, …) that spuriously inflated the principle count from 4 to 10 and caused the invariant to fail. Two commits (`2dba05d` then `b6ecb26`) tightened the assertion: the count is now scoped to the `## The Four Principles` section using an awk range pattern (`/^## The Four Principles/{flag=1; next} /^## /{flag=0} flag`). The `b6ecb26` follow-up replaced the section-name literal in the awk pattern with the generic `^## ` end-marker so the invariant survives a future section rename without a test update.

**`execution-workflow` Step 1 expansion.** Step 1 grows from 7 to 8 sub-steps (commit `bbd57d1`) to fold in three concepts introduced by this PR without undoing the recent Round 3 refactor that added domain-skill loading (sub-step 3) and project-guidance walk-up (sub-step 4). Sub-step 1 gains a parenthetical citing `superRA:handoff-doc` §PLAN.md Is the Task Tracker and naming PLAN.md as the authoritative source of truth. New sub-step 2 instructs the orchestrator to read `## Workflow Status` (if present) as the first signal of where to pick up on resumption. Sub-step 8 (previously the final "if no concerns" sub-step 7) gains the `TodoWrite` mirror instruction — labeling it a working view rather than a record — and a pointer at `handoff-doc` §Mid-Session Scope Changes for handling newly discovered analysis tasks.

## Unreleased — workflow-domain-split restructure: DAV + dispatch-prompt standardization + shared §Review gating + CONDITIONAL APPROVE + execution-workflow domain-agnosticism + agent Stage tables + companion-workflow audit + DRY/composability refactor (orchestration consolidation, alias-skill retirement, integration-workflow genericization, workflow-stage-to-skill map) + master-skill refactor (using-superRA promotion, skill-load manifest, agent-orchestration split, Direct Mode relocation)

One coordinated restructure on `refactor/workflow-domain-split`, landed in three rounds. **Round 1** (six phased commits through `b8c6239`) made superRA's workflow skills strictly domain-agnostic, made the implementer self-check and the reviewer verification walk a single shared checklist, collapsed the costly two-stage review into one comprehensive pass with a `CONDITIONAL APPROVE` escape hatch, and anchored every agent dispatch to a canonical prefix so the dispatch prompt carries only steering on top of the agent's standard protocol. **Round 2** (Tasks 7–12) then took the design principle one level higher: cross-stage orchestration patterns consolidated into `agent-orchestration`; alias-only protocol skills retired; `integration-workflow` stripped to workflow-choreography only with data-specific integration discipline moved into a new domain reference; DRY / composability / extensibility + shared-flow elevated to documented design principles in `CLAUDE.md` and `README.md`; a workflow-stage-to-skill map added to the README. **Round 3** (Tasks 13–18) promoted `using-superRA` to the master skill every agent reads — principles + inventory + composable-design map + a single authoritative skill-load manifest — retired the per-agent Stage tables in favor of one-paragraph pointers at the manifest, split `agent-orchestration` into high-level SKILL.md + `references/agent-teams.md` with the stale Team Recipes section deleted, and relocated Direct Mode to `using-superRA` §Execution Modes so every agent gets the direct-mode rubric via the one skill it already reads. Data-analysis discipline is preserved throughout — it just moves to its proper home in `econ-data-analysis`, leaving the workflow skeleton vertical-neutral and extensible.
