...
## Unreleased — econ-data-analysis restructured around describe-analyze-validate (DAV); dispatch-prompt protocol standardized

Two structural refactors land together.
...
`tests/structural-invariants.sh` adds two new assertions: every dispatch template in the live surface carries the "Follow the standard stage-relevant workflow" prefix (prefix count ≥ template count), and no dispatch template retains `Work from:` or `Counterpart:` lines. Both pass on this commit.
