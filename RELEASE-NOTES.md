...
Agent Teams mode was unreliable in practice. `skills/agent-orchestration/references/agent-teams.md` now carries an ARCHIVED banner; no active skill, agent, or hook loads or cites it. Workflows that previously offered a Teams branch (`execution-workflow`, `integration-workflow`, `merge-workflow`, `semantic-merge`) have had their `## Agent Teams Mode` sections removed and fall back to parallel-subagent dispatch. `skills/using-superRA/SKILL.md` §Agent Teams is deleted; `hooks/session-start` no longer probes `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`; `agents/implementer.md` and `agents/reviewer.md` drop their `## If Running as Agent Team Teammate` sections. `tests/structural-invariants.sh` block 22 now asserts the ARCHIVED banner and that no active file under `skills/`, `agents/`, or `hooks/` cites Teams content. See `agent-orchestration` §Workload Balancing (introduced alongside this archival) for the replacement dispatch framework.
