...
|---|---|---|---|---|---|---|
| **LC001** | always-loaded skills (`using-superra` + `report-in-markdown`) | [test_always_loaded_live.py](test_always_loaded_live.py) (frontmatter contract) | [test_always_loaded_live.py](test_always_loaded_live.py) (schema/path mutation + command checks) | [always_loaded_live.py](always_loaded_live.py) (role mutation with zero on-demand loads) | [always-loaded-codex-smoke.sh](always-loaded-codex-smoke.sh) (task-read + markdown-check command events) | **Claude frontmatter + mutation coverage; Codex built, live-pending.** |
| **LC002** | per-stage manifest loads (rollup) | [test_contract.py::…manifest_tables_match_contract](test_contract.py#L164) | [test_stage_loads_live.py](test_stage_loads_live.py) (schema/load IDs/paths) | [stage_loads_live.py](stage_loads_live.py) | unavailable by name | **Claude live-verified** (see LC007–LC010); Codex exposes no skill-load event. |
| **LC003** | per-domain manifest loads (rollup) | [test_contract.py::…manifest_tables_match_contract](test_contract.py#L164) | [test_domain_loads_live.py](test_domain_loads_live.py) (schema/domain IDs) | [domain_loads_live.py](domain_loads_live.py) | unavailable by name | **All 4 domains + multi-domain loading Claude live-verified**; Codex exposes no skill-load event. |
...
| **LC007** | stage `planning-review` → `planning-review.md` | [test_contract.py::…manifest_tables_match_contract](test_contract.py#L164) | [test_stage_loads_live.py](test_stage_loads_live.py) (Read path + artifact path identity) | [stage_loads_live.py](stage_loads_live.py) `STAGE_ROWS[planning-review]` | unavailable by name | **Claude live-verified** via `Read`; Codex exposes no reference-load event. |
...
| **LC023** | stage `maturation` → `task-tree` + `superplan` (always; `writing` conditional) | [test_contract.py::…manifest_tables_match_contract](test_contract.py#L164) | [test_stage_loads_live.py](test_stage_loads_live.py) (ordered skill IDs) | [stage_loads_live.py](stage_loads_live.py) `STAGE_ROWS[maturation]` | unavailable by name | **Structured fixture-covered; Claude live-pending.** |
...
| **LC012** | domain `theory-modeling` (+ multi-domain) | [test_contract.py::…manifest_tables_match_contract](test_contract.py#L164) | [test_domain_loads_live.py](test_domain_loads_live.py) (all matched IDs) | [domain_loads_live.py](domain_loads_live.py) `DOMAIN_ROWS[theory-modeling]` + multi-domain events | unavailable by name | **Claude live-verified**, including both matching skill loads. |
| **LC013** | domain `writing` (+ multi-domain) | [test_contract.py::…manifest_tables_match_contract](test_contract.py#L164) | [test_domain_loads_live.py](test_domain_loads_live.py) | [domain_loads_live.py](domain_loads_live.py) `DOMAIN_ROWS[writing]` + multi-domain events | unavailable by name | **Claude live-verified**, including multi-domain membership. Sub-reference routing is static-only. |
| **LC014** | domain `slide-design` | [test_contract.py::…manifest_tables_match_contract](test_contract.py#L164) | [test_domain_loads_live.py](test_domain_loads_live.py) | [domain_loads_live.py](domain_loads_live.py) `DOMAIN_ROWS[slide-design]` | unavailable by name | **Claude live-verified.** Beamer sub-reference routing is static-only. |
...
| **LC020** | execution-mode contract + `superimplement` default | [test_contract.py](test_contract.py) (structured availability/seat tables) | [test_transcript_assertions.py](test_transcript_assertions.py) (interactive ordering + both main-seat routes), [test_contract.py::…codex_orchestrator_sample](test_contract.py) | [orchestrator-live-smoke.sh](orchestrator-live-smoke.sh) (built) | [orchestrator-live-smoke.sh](orchestrator-live-smoke.sh) `HARNESS=codex` (built) | Availability and seat structure are **static-covered**; interactive ordering, main-seat behavior, and default dispatch are **fixture-covered**. Orchestrator dispatch smoke is **built; its Claude live run was not part of the 08–12 expansion**, and Codex live is pending. |
...
- **No Codex live run yet.** Built Codex rows are CI-tested but live-pending. Stage/domain load-by-name rows are marked unavailable because Codex emits no corresponding event.
...
The downstream live smokes call `run_skill_load_session` and assert on the returned evidence — they consume the harness, not raw SDK calls.
...
- **Claude autoloads** both skills via the agents' frontmatter `skills: [...]`, so an autoloaded skill is invisible to the `Skill` hook. The deterministic check parses both role specs. The live check dispatches the real role, requires an `Edit`/`Write` event that creates an exact schema/task/path artifact, and requires zero on-demand loads for both frontmatter skills.
- **Codex does not autoload**, so its live smoke parses actual `command_execution` records and matches the executable plus ordered arguments for `./superRA/superra task read always-loaded-task` and the markdown self-diagnose script. A match requires a completed zero-exit record. Non-path arguments are exact; only the checker and target path arguments opt into suffix normalization. The artifact is checked separately for exact schema/task/path identity. Fixture: [tests/fixtures/task-trees/always-loaded-canary](../fixtures/task-trees/always-loaded-canary); evaluator: [check_always_loaded_smoke.py](check_always_loaded_smoke.py).

The live SDK dispatch defaults to `CLAUDE_MODEL=sonnet` and uses a bounded pass@k retry window (`attempts`, default 3) for dispatch reliability.

[test_always_loaded_live.py](test_always_loaded_live.py) covers frontmatter declarations, mutation presence, exact artifact structure, zero on-demand loads, and the two command events. Run the live checks:

```bash
# Claude role mutation + autoload event shape:
RUN_LIVE_HARNESS=1 uv run --with claude-agent-sdk \
  python tests/harness-instruction-following/always_loaded_live.py

# Codex task-read + markdown-check command executions:
...
Codex exposes no stage skill/reference load event. The committed artifact contract therefore preserves only observable structure: schema, stage, ordered skill IDs, load kinds, and the planning-review reference path. It does not claim those identities prove a skill body loaded.

[test_stage_loads_live.py](test_stage_loads_live.py) drives the skill/read event evaluator and exact artifact contract, including the multi-skill and negative stages. Run the live Claude per-stage check:
...
Codex exposes no domain skill-load event. The committed artifact contract therefore preserves the schema and ordered matched skill IDs, including every ID in the multi-domain case, without treating authored domain guidance as output evidence.

[test_domain_loads_live.py](test_domain_loads_live.py) drives the domain skill-event evaluator and exact artifact contract, including multi-domain completeness. Run the live Claude per-domain check:
...
- **Whether a specific skill or reference was loaded into Codex context.** Codex emits no structural load event. Stage/domain Codex artifacts preserve schemas, IDs, and paths but make no body-load claim. Claude recovers on-demand loads by name through in-process hooks; always-loaded declarations are parsed from role frontmatter. The Codex always-loaded smoke additionally checks two actual command executions.
