...
- **Codex does not autoload**, so its live smoke checks the actual `command_execution` events for `./superRA/superra task read always-loaded-task` and the markdown self-diagnose script. The artifact is checked separately for exact schema/task/path identity. Fixture: [tests/fixtures/task-trees/always-loaded-canary](../fixtures/task-trees/always-loaded-canary); evaluator: [check_always_loaded_smoke.py](check_always_loaded_smoke.py).
