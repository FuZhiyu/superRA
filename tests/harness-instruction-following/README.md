...
- **Codex does not autoload**, so its live smoke parses actual `command_execution` records and matches the executable plus ordered arguments for `./superRA/superra task read always-loaded-task` and the markdown self-diagnose script. Explicit nonzero outcomes fail. The artifact is checked separately for exact schema/task/path identity. Fixture: [tests/fixtures/task-trees/always-loaded-canary](../fixtures/task-trees/always-loaded-canary); evaluator: [check_always_loaded_smoke.py](check_always_loaded_smoke.py).
