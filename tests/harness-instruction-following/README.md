...
| `superra task read` surfaces ancestor context, comments, dependency status; excludes dependency `## Results` | [test_contract.py::test_task_read_fixture_contract_surfaces_context_without_dependency_results](test_contract.py#L240) | [test_bundle_fixture.py](test_bundle_fixture.py) | — | both loading smokes assert `dependency_results_excluded` in the artifact |
