...
| Each domain-worded task loads its domain skill; multi-domain wording loads **every** matching domain (LC003, LC011–LC014) | [test_domain_loads_live.py](test_domain_loads_live.py) domain-evaluator green/red per domain + multi-domain all-loaded / loaded-only-one cases | — | — | Claude per-domain canary ([domain_loads_live.py](domain_loads_live.py)); Codex per-domain body-load canary (orchestrator-run per [09](../../superRA/task-tree/agent-interface/agent-loading-tests/09-codex-canary-and-dispatch-hook/task.md) convention) |
