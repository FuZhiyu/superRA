...
- **Always-loaded skills** (``using-superra``, ``report-in-markdown``) are
  ...
  preloaded ones, so the ``Skill`` hook cannot see them. Their declaration is
  covered by the static frontmatter contract; live coverage is limited to
  observable role mutation and zero on-demand loads. There is no
  ``InstructionsLoaded`` channel: that is not a
...
def check_skills_loaded_before_first_edit(
    ...
    not loaded through the ``Skill`` tool; their declarations are covered by
    :func:`check_always_loaded_frontmatter`.
    """
