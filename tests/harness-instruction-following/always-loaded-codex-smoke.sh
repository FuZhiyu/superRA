#!/usr/bin/env bash
# Opt-in Codex always-loaded skill behavior smoke.
#
...
# the fixture and asserts actual task-read and markdown-validation command
# executions plus the schema/task/path identity of the output mutation.
#
...
# default. The Claude always-loaded run is the structural/mutation check in
# always_loaded_live.py (run via that module's __main__).
