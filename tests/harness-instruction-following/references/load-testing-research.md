...
- **`--include-hook-events` is undocumented/unconfirmed** in the current CLI reference; the existing `claude-*-smoke.sh` scripts pass it. Audit whether it does anything and drop it if it is a no-op.
