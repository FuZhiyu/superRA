...
- **The always-loaded skill (`using-superra`) reaches a dispatched agent through the role skill's §Before You Start load instruction** — `skills/implement-task/SKILL.md` and `skills/review-task/SKILL.md` both name it there. (Before v0.4 they were preloaded via agent frontmatter `skills: [...]`, which emitted no `Skill` tool_use at all; the SDK init/system message still lists only *available* skills, not per-agent preloaded ones.) The durable observable is the instruction, covered by the static load-instruction contract; Codex additionally retains command evidence and an artifact-schema check.
- **`InstructionsLoaded` is NOT a registrable `claude-agent-sdk` hook event.** Verified against the live SDK (Claude Code 2.1.183): the `HookEvent` union is exactly `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `UserPromptSubmit`, `Stop`, `SubagentStop`, `PreCompact`, `Notification`, `SubagentStart`, `PermissionRequest`. Registering an `InstructionsLoaded` hook is a silent no-op (the live run captured zero such events), so it cannot be the CLAUDE.md/rules or always-loaded-skill signal. Use the static frontmatter contract instead.
...
Chosen: **Claude via the Python `claude-agent-sdk` with in-process hooks; Codex via command/schema evidence + a `SubagentStart` hook.**

- **Claude side** — drive the SDK with a `PreToolUse(matcher="Skill")` hook that records every on-demand skill loaded by name (and its order relative to edits), and **dispatch a general-purpose agent whose prompt names the role skill** (`superRA:implement-task` / `superRA:review-task`) rather than a bare `query()` so the manifest-driven stage/domain loads actually fire. This yields real skill-load-by-name assertions across stages and domains, reliably in headless mode (unlike filesystem hooks under `claude -p`). The SDK session must be configured with the superRA plugin dir (`plugins=[{"type": "local", "path": ...}]` — a bare path string raises `TypeError`) so `Skill(...)` resolves and the role skills are loadable. The `Skill` hook must capture loads happening *inside* the dispatched subagent (the SDK delivers subagent tool-lifecycle hooks); confirm that delivery on the live path before the stage/domain smokes (11/12) rely on it. Always-loaded skills remain a static load-instruction contract.
- **Codex side** — skill-load and subagent-dispatch are unobservable in JSONL. The always-loaded path retains prescribed-command evidence, stage/domain artifacts retain schema and identity only, and a `SubagentStart` hook writes agent-type sentinels for orchestrator dispatch.
- **Cross-harness backbone** — structured command, schema, mapping, and dispatch evidence stays deterministic where a direct load event is unavailable.
- **A failing load assertion is a real finding, not a test bug.** If `using-superra` (or any skill the contract claims loads) never loads, that is a gap in the loading contract to escalate, not something to weaken the test around.
