...
- **Always-loaded skills (`using-superra`, `report-in-markdown`) reach a dispatched agent through the role skill's §Before You Start load instruction** — `skills/implement-task/SKILL.md` and `skills/review-task/SKILL.md` both name the pair there. (Before v0.4 they were preloaded via agent frontmatter `skills: [...]`, which emitted no `Skill` tool_use at all; the SDK init/system message still lists only *available* skills, not per-agent preloaded ones.) The durable observable is the instruction, covered by the static load-instruction contract; Codex additionally retains command evidence and an artifact-schema check.
