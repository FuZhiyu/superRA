...
- **Claude side** — drive the SDK with a `PreToolUse(matcher="Skill")` hook that records every on-demand skill loaded by name (and its order relative to edits), and **dispatch the real plugin role agent** (`superRA:implementer` / `superRA:reviewer`, present in the SDK init `agents` list) rather than a bare `query()` so the manifest-driven stage/domain loads actually fire. This yields real skill-load-by-name assertions across stages and domains, reliably in headless mode (unlike filesystem hooks under `claude -p`). The SDK session must be configured with the superRA plugin dir (`plugins=[{"type": "local", "path": ...}]` — a bare path string raises `TypeError`) so `Skill(...)` resolves and the role agents are dispatchable. The `Skill` hook must capture loads happening *inside* the dispatched subagent (the SDK delivers subagent tool-lifecycle hooks); confirm that delivery on the live path before the stage/domain smokes (11/12) rely on it. Always-loaded skills are not on this channel and remain a static frontmatter contract.
