...
1. **CRITICAL — standalone children-DAG panel is broken for every nested (multi-segment) task.** [plan_dashboard.py:926](../../../../skills/task-system/scripts/plan_dashboard.py#L926) keys the `/dag` fragment map with the bare task path (`fragments[f"/dag?root={task.path}"]`, e.g. `/dag?root=task-system/dashboard`). But the standalone client requests `'/dag?root=' + encodeURIComponent(path)` at [base.html:1876](../../../../skills/task-system/scripts/templates/base.html#L1876), and JS `encodeURIComponent` **does** escape `/` → `%2F`. So the client asks for `/dag?root=task-system%2Fdashboard` while the map holds `/dag?root=task-system/dashboard`. The shim's exact-string `STANDALONE_FRAGMENTS.hasOwnProperty(url)` ([base.html:1216](../../../../skills/task-system/scripts/templates/base.html#L1216)) misses, returns 404, and `loadChildrenDag` falls into `if (!resp.ok) { region.innerHTML = ''; return; }` — the card-flow children panel renders **empty** offline for every non-leaf task whose path has more than one segment. Reproduced on a minimal nested plan: map key `/dag?root=01-parent/01-child` vs. client request `/dag?root=01-parent%2F01-child` → no match. Single-segment paths (e.g. `01-parent`) happen to work because they contain no `/` to escape, which is why the in-repo smoke test on `02-second` and the route-parity test both passed. The code comment at [plan_dashboard.py:920-922](../../../../skills/task-system/scripts/plan_dashboard.py#L920) asserts the opposite ("the encoded URL == the path … task slugs … contain nothing encodeURIComponent escapes") — this is factually wrong about `/` and must be corrected, not just worked around. Fix options: key the map by the encoded form to match what the client sends, or decode the URL inside `standaloneFetch` before the map lookup. Whichever you choose, make `/node/<path>` and `/nav/<path>` (which use raw concatenation, no `encodeURIComponent`) stay consistent.

2. **MAJOR — route-parity test does not exercise the encoded multi-segment path the client actually requests.** [test_task_system.py:797](../../../../skills/task-system/scripts/test_task_system.py#L797) (`test_build_standalone_fragments_match_server_routes`) compares `fragments["/dag?root=02-second"]` against `c.get("/dag", params={"root": "02-second"})` — a single-segment path with the test client decoding params for you, so it never reproduces the `%2F` mismatch from item 1. After fixing item 1, add a test that (a) uses a nested fixture task (multi-segment path) and (b) looks the fragment up under the **exact URL string the standalone client builds** (`'/dag?root=' + encodeURIComponent(path)`), so the regression is locked. Re-check this item only after item 1's fix lands, since the assertion shape depends on the chosen keying strategy.
