...
- §What You Own: remove `review_status:` from owned fields. Replace with: "Implementer owns `status:` transitions up to `implemented` (and `revise → implemented` on fix rounds)."
...
`agents/reviewer.md`:
- §What You Own: replace `review_status:` ownership with: "Reviewer owns `status:` transitions `implemented → revise` and `implemented → approved`."
...
- Line 211: update status clear instructions (now just reset `status` instead of clearing `review_status`)

`skills/integration-workflow/SKILL.md`:
- Update any references to `review_status` in integration dispatch context
...
`skills/using-superRA/references/main-agent.md`: update frontmatter reading instructions
