...
Bring superRA's task-related hook behavior into a coherent Codex-compatible state while honoring the researcher decision to drop the user-decision log reminder. The work has two independent implementation branches plus a final reconciliation pass:
...
3. Update active docs and tests so they describe the supported hook set accurately: Codex should list autoload, merge-guard, codex-plan-stop, and the task-system `PostToolUse` hooks; it should not list or imply the user-decision reminder.

Scope boundary: do not redesign the task-system data model, status rollup semantics, dashboard UI, or superRA decision logging protocol. This is hook packaging, hook input compatibility, active documentation, and focused regression coverage.
