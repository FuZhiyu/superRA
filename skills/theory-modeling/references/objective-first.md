...
> Loaded on demand from `references/integration.md` Section A. Teaching material — the principle and its `[BLOCKING]` checklist live there.
...
Why this fails: $\eta$ arrives before the reader knows why it matters, lives for one algebraic detour, and is discarded. The objective stays hidden until the final substitution, so the Euler equation reads as a coincidence rather than a consequence of intertemporal optimization.
...
The bad derivation traverses this dependency chain from the wrong end.
...
Each equation appears because the previous prose named the object it computes.

## Identification training

Two held-out snippets. Per snippet: name the target, walk the dependency chain backward to primitives, and flag whether the prose makes the walk visible or which anti-pattern is present.
...
- **Visible in prose?** No. "We compute." names neither target nor strategy; the reader reconstructs the walk after the fact. **Anti-pattern: deferred goal — target not named before the first displayed equation.** Fix: one sentence before the differentiation — "We sign $\mathrm dp/\mathrm d\theta$ via the IFT on market clearing; the denominator's sign comes from regularity, the numerator from the shift in supply minus demand."
...
- **Visible in prose?** Partially. The proposition implies a top-level target, but the two sub-arguments carry no opening signposts and no transition between them. **Anti-pattern: signpost-less sub-arguments.** A reader entering at the differentiability block cannot recover its local goal. Fix: a one-sentence head per sub-argument ("We first establish concavity by ...; we then establish differentiability by ...") plus a transition at the join.
