...
Both modes return `[BLOCKING]` findings for poor tree design, not only unclear prose. Review against [task-tree-design.md](task-tree-design.md): durable ownership, depth vs. breadth, branching and dependency quality, parent/sibling context, update-task lifecycle, action-verb durability, `## Objective` / `## Planner Guidance` split.
