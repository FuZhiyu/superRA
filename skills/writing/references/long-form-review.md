...
- **Per-aspect blocks ARE task blocks** per `plan-anatomy.md §Task Block Anatomy` — one block per dimension (or, in deep mode, per perspective), all `**Depends on:** *(none)*`. Reviewer findings land in the existing review-notes blockquote of the task block, each finding in its loaded `consistency/<dim>.md` output format including the new `Auto-fixable: Yes / No` line.
- **Final summary block at the top** of REVIEW.md: severity × auto-fixable counts table, top-3 priorities, pointer to each per-aspect block, and an auto-fixable batch table sized for polish-mode shape C handoff. Built by the orchestrator, optionally with one final-summary reviewer pass over the assembled doc.
