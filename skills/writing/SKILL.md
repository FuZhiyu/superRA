...
**Where they live (lifecycle ladder, ordered by permanence).** Root `superRA/task.md` (workflow-scoped) → `CLAUDE.md` (project-permanent). Long-form review retrofits a task tree around the user's existing draft; reviewers then write findings as task-local review notes. Writing-side conventions live in the `## Project Conventions` header of root `superRA/task.md`; when no task tree is in play, return the inventory as a conversation reply. Promote up the ladder when the user signals durability — same pattern as theory-modeling's per-task ledger → root task.md Notation Conventions promotion (`theory-modeling/SKILL.md §Documentation and handoff`).
