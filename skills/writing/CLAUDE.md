...
- **Auto-fixable flag in every `consistency/<dim>.md` output format.** Every finding carries `Auto-fixable: Yes / No` so polish-mode shape C (apply review findings) can triage mechanically without re-judging each finding. This was the load-bearing parity addition vs the retired `draft-reviewer` plugin and is the contract that lets a review feed a polish without an intermediate human pass.
