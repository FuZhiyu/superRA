...
The implementer/reviewer find their task by running `superra task read <task-path>`, which injects the task's context (focused tree + ancestor objectives), sibling dependency status, and any unresolved researcher comments (each with its full anchored block) automatically; you can additionally run `superra task comment list <task-path>` to fold those comments into `Additionally:` steering when useful.
