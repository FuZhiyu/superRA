...
- **Defer** when the main result stands and the open items do not affect downstream work: leave the findings in `## Review Notes`, leave `status: revise`, and proceed to the dependent tasks — a task may stay in revision while downstream work advances, and `superra task frontier` already lists its dependents. Track deferred items as you go; when the active frontier's implementations are done, clear them all in one bundled fix pass (§Workload Balancing Tier 2) and re-review to `approved`.
