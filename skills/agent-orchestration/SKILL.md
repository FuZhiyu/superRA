...
- **Defer** when the open items do not affect downstream work: leave the findings in `## Review Notes`, leave the task at `revise`, and move on — the frontier lists its dependents. Tasks sitting at `revise` are the deferral record; clear them in one bundled fix pass (§Workload Balancing Tier 2) and re-review to `approved` before the workflow's completion gate.
