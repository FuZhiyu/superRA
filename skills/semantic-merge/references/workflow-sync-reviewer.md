...
Verify the sync commit chain before maturation. Walk `semantic-merge/SKILL.md` §Semantic Coherence Checklist; `workflow-sync-author.md` carries the workflow boundary and the `## Sync Impact` format.

## Review Scope

Review the sync commits (merge commit plus any propagation commits) before maturation begins — catching wrong branch intent, bad conflict resolution, missing user escalation, misleading `## Sync Impact` context, or scope creep before permanent documentation and refactoring build on it.
...
4. Inspect the sync commits and their combined diff. Confirm every kept, dropped, or synthesized hunk has a semantic rationale, classified by role per `SKILL.md §Shared Steps` step 2.
...
8. Confirm the scope boundary. Generated outputs within the merge's semantic reach are regenerated, or escalated per `SKILL.md §Shared Steps` step 4 and recorded — flag a skipped regeneration or silently re-expected drift-test results. Flag codebase-coherence work in the sync commit chain, and any `## Sync Impact` note presenting unresolved semantic work as an Integrate target.
