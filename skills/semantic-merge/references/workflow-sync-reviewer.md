...
Workflow sync reviewer mode verifies the sync commit chain before maturation. Walk `semantic-merge/SKILL.md` §Semantic Coherence Checklist; use `workflow-sync-author.md` for the workflow boundary and the `## Sync Impact` format.

## Review Scope

Review the sync commits (merge commit plus any propagation commits) before maturation begins. The goal is to catch wrong branch intent, bad conflict resolution, missing user escalation, misleading `## Sync Impact` context, or scope creep before permanent documentation and refactoring build on it.
