...
If the implementer followed the listed steps but missed diagnostics, validation, robustness checks, or other work required to satisfy the task objective, that is still a finding — flag the missing work rather than approving mechanical step compliance. If the task started open, judge the work against the objective, the scope fields, and the current route the implementer wrote in place. If the implementer changed the within-task route, judge whether the change stayed inside the same objective/scope contract; unjustified step drift is a finding. If execution or integration convinces you the work belongs in a different task shape, record that recommendation in the task's blockquote and mention it again in your status report so the orchestrator can decide whether to route it through `planning-workflow §User Feedback and Changing Plans`; do not rewrite the task boundary yourself.
...
- **Doc before report.** Every material review finding lands in the blockquote in `PLAN.md` **before** it appears in your status return. The blockquote is the record for open review findings, including task-shape recommendations future agents need to see; use the status report for rationale, uncertainty, or emphasis after the doc is updated.
...
- Any step text or task objective — even if you believe it is wrong. Raise in-scope correctness issues in your blockquote and let the orchestrator decide whether to rewrite the step or re-enter the changing-plans protocol for whole-task restructure.
...
1. Read the task block's objective/scope fields, steps, and the code at the cited files.
