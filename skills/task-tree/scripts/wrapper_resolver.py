...
def render_hook_shim() -> str:
    ...
    reconcile and writing-feedback script. It prefers its own plugin checkout
    (its location is inside the plugin) and otherwise resolves through the
    shared chain. Never blocks: always exits 0.
    """
    return (
        "#!/usr/bin/env bash\n"
        "# Stable PostToolUse entry point for task-tree reconciliation and writing\n"
        "# feedback (generated by wrapper_resolver.py). Resolves the task-tree\n"
        "# scripts from the installed plugin, then forwards stdin to the packaged\n"
        "# hook via `uv run --script` (python3 fallback).\n"
