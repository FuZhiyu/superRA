...
- **Which trees** — the task root beside or above the session cwd, each tool-supplied path, and each absolute path the Bash command names (an agent in another worktree addresses it by absolute path); the upward walk stops at a repository top. A root holding more than `MAX_TREE_FILES` watched files is treated as mis-resolved and ignored.
