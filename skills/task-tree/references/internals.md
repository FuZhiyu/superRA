...
Load when modifying the task-tree skill itself — scripts, data layer, hooks, migration logic.

## Setup: the `superra` CLI

Bootstrap and end-user invocation: `SKILL.md §CLI Setup`. Contributor run forms (local checkout): `CLAUDE.md §Local Task-Tree CLI Development`.

**Never run bare `uv run superra` from a research project.** There is no `superra` console entry, and `uv run` without `--script` provisions that project's environment. Use the wrapper or the bootstrap form in `SKILL.md §CLI Setup`.

## Data Layer: `_task_io.py`

All scripts share `_task_io.py` as the data layer:
...
Validation rules live in their own module: one owner and one message source per rule. `invalid_status_message(status)` is the single source for the status-validity message — `parse_task`'s lenient warning and `task check`'s strict `[ERROR]` finding both render it.
...
`_walk_children` sorts children **topologically** by `depends_on` via Kahn's algorithm (`_topological_sort`), so traversal order respects the dependency DAG, alphabetical on ties. On detected cycles the remaining tasks fall back to alphabetical order.
...
**Checkout-isolation gate.** `guard-foreign-checkout` (`hooks/checkout_isolation_gate.py`) returns `ask` for a `task.md` write whose task root belongs to neither the session's cwd nor its repository, and for a git history-writing `Bash` command redirected by `cd`, `git -C`, or `--git-dir`/`--work-tree` into another task-tree checkout. `ask` rather than `deny` leaves a route through for cross-checkout work the researcher set up deliberately, while an unattended session still stops. Membership is the git common dir, so every worktree of the session's own repository passes; the rule lives in `skills/task-tree/scripts/_checkout_scope.py` and the PostToolUse edit detector applies it too, so the hook writes nowhere this gate would prompt about. It fails open wherever identity is unresolvable (no git, unreadable payload). `deny_reason()` carries the whole decision; the live Agent-SDK trace harness registers it as an in-process PreToolUse hook and hard-denies on it, since a traced session has no approver.
...
- **Watched set** — every `.md`, common script (`_edit_detect.SCRIPT_SUFFIXES`), and `config.yaml` under the task root, plus the literal-path file deps, scripts, and Julia include closures of registered steps, minus step outs. Directory deps are not walked.
- **Baseline** — `<project_root>/.superra-repro/hook-baseline/<session-or-hour-key>.json` maps each watched path to `(size, mtime_ns, sha256)`; a file is changed when its hash differs, and a file above `HASH_MAX_BYTES`, or past `HASH_BUDGET_BYTES` hashed in one call, is compared by stat alone. The first call of a session seeds silently. Writes are atomic (`os.replace`), so concurrent hook processes read a whole baseline; a lost update can only repeat a reminder. `.superra-repro/.gitignore` holds `*`. Baselines older than a week are pruned at seed time.
...
The migration script uses strict regex patterns. Normalize a non-conforming PLAN.md before migration, or migrate it by hand.
...
Only `###` headings with the exact `Task N: Title` pattern are recognized; `##`/`####`, unnumbered, and colon-less tasks are invisible to the parser.
...
Variants like `- [~]`, `- [-]`, or `- [X ]` (extra space) match neither pattern and are invisible, so status inference goes wrong.
...
**Normalization vs manual migration:** badly diverging structure (no numbered task headings, deeply nested prose, ≤3 tasks) migrates faster by hand:
...
The wrapper's `dashboard` subcommand routes straight to `plan_dashboard.py` via `uv run --script`; against a local checkout use the loose `plan_dashboard.py dashboard …` form for live-source edits. `cli.py` translates the user-facing `dashboard` surface (`export` / `stop` / default-serve / `artifact`) to `plan_dashboard.py`'s internal `serve` / `stop` / `generate` argv.
...
`superra dashboard --no-open` guarantees a server is up without a browser: it reuses a healthy background server (reuse check reads the repo-scoped PID file) or starts one detached, then prints this worktree's scoped live URL.
...
Writes `.github/workflows/superra-dashboard-artifact.yml`; refuses to overwrite a non-superRA-managed workflow without `--force`. Options:
...
Repo-access-gated by GitHub Actions artifact permissions, but not a hosted webpage: collaborators download the artifact from the workflow run and open the HTML locally.
