...
**Checkout-isolation gate.** `guard-foreign-checkout` (`hooks/checkout_isolation_gate.py`) denies a `task.md` write whose task root belongs to neither the session's cwd nor its repository, and a git history-writing `Bash` command redirected by `cd` or `git -C` into another task-tree checkout. Membership is the git common dir, so every worktree of the session's own repository passes. It fails open wherever identity is unresolvable (no git, unreadable payload). `deny_reason()` carries the whole decision; the live Agent-SDK trace harness registers it as an in-process PreToolUse hook so a traced session is confined by the same rule.
