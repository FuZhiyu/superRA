...
Both happen once at planning time and then live in root `superRA/task.md` as sections the analysis references.

---

## Data Inventory (Hard Gate)

The plan cannot be written without a data inventory. Your job is data logistics — what exists, what's missing, where to find it — not research design. The inventory becomes the `## Data Inventory` section of root `superRA/task.md`.
...
6. **Present the inventory and get researcher approval.** Use the Data Inventory format from `task-system/references/planning.md` (the root task.md anatomy). Ask the researcher to confirm before proceeding to task drafting.
...
- **Skipping the inventory:** verbal agreement on data, no written record. Always document the inventory — it becomes part of root `superRA/task.md`.
...
**Never:**
- Skip the gate and propose the data inventory verbally. It must be written into root `superRA/task.md`.
