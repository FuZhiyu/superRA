...
The plan cannot be written without a data inventory. Your job is data logistics — what exists, what's missing, where to find it — not research design. The inventory becomes a `### Data Inventory` subsection of the `## Objective` on the task whose subtree the data governs (the top `superRA/task.md` for a single-workstream analysis).
...
6. **Present the inventory and get researcher approval.** Use the Data Inventory format from `task-system/references/planning.md` §Task Anatomy. Ask the researcher to confirm before proceeding to task drafting.
...
- **Skipping the inventory:** verbal agreement on data, no written record. Always document the inventory — it becomes part of the governing task's objective in `superRA/`.
...
**Never:**
- Skip the gate and propose the data inventory verbally. It must be written into the governing task's objective in `superRA/`.
