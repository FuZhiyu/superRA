...
```bash
bash .plan/serve          # shortcut — generated alongside the task tree
uv run <skill-dir>/scripts/plan_dashboard.py serve --root .plan/   # direct invocation
```

`.plan/serve` is a short shell script generated by `task_create.py` (when creating the first root-level task) or by the planning-workflow agent. It resolves the path to `plan_dashboard.py`, runs it with `uv run`, and forwards extra arguments (`--port`, `--no-open`, etc.). The script is committed alongside the task tree so collaborators can launch the dashboard without knowing where the skill directory lives.
