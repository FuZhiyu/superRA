...
def test_resolver_env_var_outranks_everything(
    ...
    (plugin / "skills" / "task-system" / "scripts").mkdir(parents=True)
    (plugin / "skills" / "task-system" / "scripts" / "cli.py").touch()
...
def test_resolver_github_branch_clones_then_resolves_loose_script(
    ...
    # Build a local bare remote carrying skills/task-system/scripts/cli.py.
    work = tmp_path / "work"
    (work / "skills" / "task-system" / "scripts").mkdir(parents=True)
    (work / "skills" / "task-system" / "scripts" / "cli.py").write_text(
    ...
    assert resolved.endswith("/skills/task-system")
...
def test_resolver_picks_claude_manifest_over_codex(
    ...
    (claude_install / "skills" / "task-system" / "scripts").mkdir(parents=True)
    (claude_install / "skills" / "task-system" / "scripts" / "cli.py").touch()
...
def test_resolver_codex_picks_highest_semver_within_depth_bound(
    ...
    for ver in ("0.1.0", "0.10.0", "0.2.0"):
        d = cache / ver / "skills" / "task-system" / "scripts"
        d.mkdir(parents=True)
        (d / "cli.py").touch()
    # Decoy nested too deep for the depth-3 glob; must be ignored.
    deep = cache / "extra" / "deep" / "0.99.0" / "skills" / "task-system" / "scripts"
...
def test_resolver_skips_cache_lacking_cli_script(
    ...
    (stale / "skills" / "task-system" / "scripts").mkdir(parents=True)  # no cli.py
    (home / ".claude" / "plugins").mkdir(parents=True)
