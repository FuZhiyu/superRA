...
1. Load plan from `.plan/` task tree; if the tree is missing, structurally incomplete, or contradicted by unlogged material decisions, the main agent's Workflow Frontier Resolver routes to `superRA:planning-workflow` first.
...
The autonomy contract is in `superRA:using-superra/references/main-agent.md` (main-agent only). This section lists the **implementation-workflow-specific stop points** that plug into its pause classes.
