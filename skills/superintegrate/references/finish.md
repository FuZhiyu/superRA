...
Run `superra repro build --tier required --upstream` and confirm a clean `superra repro status --tier required --upstream` on the final tree (`../../reproducibility/references/protect-and-completion.md` §The completion gate); on a tree that declares no `## Reproduction` section, run targeted verification instead. Investigate any failure before cleanup.
