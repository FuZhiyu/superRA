...
Every existing check plus the mechanisms selected at Protect, `superra repro build --tier required --upstream` followed by a clean `superra repro status --tier required --upstream` among them (`../../reproducibility/references/protect-and-completion.md` §The completion gate). A failing drift test, document check, or reproduction step blocks Integrate until classified under its owning protection discipline.
