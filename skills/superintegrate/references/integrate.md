...
Every existing check plus the mechanisms selected at Protect, `superra repro build --tier canon` followed by a clean `superra repro status --tier canon` among them. A failing drift test, document check, or reproduction step blocks Integrate until classified under its owning protection discipline.
