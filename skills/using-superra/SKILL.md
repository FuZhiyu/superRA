...
**Place files by lifespan and owner:** Keep disposable session work in scratch folder. Put task companions under that task's `attachments/`. Put maintained code, shared or runtime-consumed files, and promised durable deliverables in the project's conventional permanent paths. Load `references/task-companion-files.md` for the task-companion definition and before retaining, reviewing, promoting, or maturing one.
