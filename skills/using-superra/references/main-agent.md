...
- On a tree carrying reproduction config, run `./superRA/superra repro status .` and report what it finds stale through [the stale rule](../../reproducibility/references/rerun-or-accept.md#the-stale-rule) — the researcher's own edits, git operations, and earlier sessions leave staleness nobody has seen.
