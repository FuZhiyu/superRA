...
2. `superimplement` — runs tasks through the implementer-reviewer loop, verifies reproducibility, records the researcher's completion disposition before integration begins.
...
- **Tree not all-approved** → implementation work remains. Resume the implement loop (`superimplement`): `superra task frontier` lists every actionable leaf with its status — `not-started` / `in-progress` to implement, `implemented` to review, `revise` to fix.
...
2. **A pre-set workflow gate** — a stop the workflow deliberately places for the researcher (the `superimplement` Step 4 menu, drift-test selection at `superintegrate` Protect, intent-changing conflict escalation in `semantic-merge`).

Resolve what you can from code and data first.

## Execution Modes

Two dials, selected by autonomy and human cadence.
...
- **subagent** (default) — autonomous execution, the mode all workflows assume. Axis B picks the seat structure; a main-agent-filled seat loads and runs that seat's role skill (`superRA:implement-task` or `superRA:review-task`) and dispatches the other. Everything routes through `superRA:agent-orchestration`.
- **interactive** (or `direct`) — the main agent executes the task itself at high human cadence, via the fused light-plan → execute-yourself → record canvas loop in `superplan/references/interactive-mode.md`. On finishing a unit of work it **asks the researcher** whether to dispatch a reviewer.
