...
| `task-tree` | Directory-tree task tooling — filesystem hierarchy as task hierarchy, `task.md` per task, sibling-only dependencies, status rollup, frontier computation, DAG rendering, live dashboard server, and legacy migration from `PLAN.md` / `RESULTS.md`. Load-on-demand by orchestrators, planners, and contributors. |
