...
| `literature-review` | Economics/finance literature-review discipline — web-first multi-modal discovery, task-shaped candidate-paper records outside git before screening, promoted paper cards in the superRA dashboard after inclusion/escalation, verbatim provenance-tracked metadata, and per-paper quote-grounded extraction into a curated collection. Composes `zotero-paper-reader`, `mistral-pdf-to-markdown`, and `writing`; bundles a citation-graph + metadata client plus candidate materializer. User-invocable; orchestrator-driven, not loaded by workflow agents. |
