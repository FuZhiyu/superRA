...
| `reproducibility` | Discipline for the task-declared reproduction graph. SKILL.md carries the model, the routine loop, and the gates; one reference per moment — `designing-the-graph.md` (what earns a step, graph and script design, declaring, step lifecycle, graph review), `rerun-or-accept.md` (the stale rule and reviewed acceptance), `diagnosing.md` (the rerun model, cascade stoppers, environment changes), `protect-and-completion.md`, `adoption.md`. Section schema, `superra repro` CLI, and hook mechanics live in `task-tree`. Loaded by Protect / `Stage: protection` agents and before a maintained output is produced. |
