...
| `literature-review` | Economics/finance literature-review discipline — web-first multi-modal discovery, screen-first snowball search (backward citation BFS + forward web sweep), verbatim provenance-tracked metadata, and per-paper quote-grounded extraction into a curated collection. Composes `zotero-paper-reader`, `mistral-pdf-to-markdown`, and `writing`; bundles a citation-graph + metadata client. Two-part workflow — interactive setup then loop-until-dry fan-out. User-invocable standalone; orchestrator-driven, not loaded by workflow agents. |
