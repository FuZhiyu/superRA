...
| `reproducibility` | Discipline for the task-declared reproduction graph — what earns a `## Reproduction` step, `required` vs `on-demand` tiering, the content-hash rerun model, boundary inputs, check steps, presenting a graph for review, and the Protect and IMPLEMENT-completion duties. Section schema, `superra repro` CLI, and hook mechanics live in `task-tree`. Loaded by Protect / `Stage: protection` agents and before a maintained output is produced. |
