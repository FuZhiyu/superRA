...
| `semantic-merge` | Intent-based semantic sync for any vertical or caller. Provides mode references for workflow sync authoring, workflow sync review, and standalone full merge; resolves conflicts by intent, escalates intent-changing decisions to the user, lands sync / propagation commits as appropriate, and records branch-level Sync Map plus task-local or file-local impact notes. |
