...
| Skill | Purpose |
|---|---|
| `using-superra` | Master skill every agent reads. Carries the commit rules, the Task Interface, the three-axis Skill-Load Manifest (Role + Stage + Domain), and the Execution Modes (subagent dispatch vs direct). Main-agent loads (workflow map, cross-session detection, autonomy contract) live in `references/main-agent.md`. |
