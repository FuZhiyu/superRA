...
2. **Results recorded?** Read the completed task files. The gate fails in either direction, against `using-superra` §Reporting: missing, thin, or status-report-only results for a task with substantive work; and results that restate an artifact, a diff, a commit body, or a child task instead of pointing at it. Figure attachments in each task's `attachments/` directory are committed.
