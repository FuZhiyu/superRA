...
**Compute the frontier with `superra task frontier`.** This returns leaf tasks whose dependencies are satisfied — `approved`, or `implemented`/`revise` (the work product exists while review or a deferred fix round stays open); sequence those dependents against the open items per `agent-orchestration` §Handling Reviewer Feedback. Execute frontier tasks singly or as same-parent bundles; use a parallel Agent-tool batch when multiple selected seats are dispatched and independent (subject to `agent-orchestration` §Workload Balancing). Serialize only when no parallel batch is available. Re-compute the frontier after each completed task or bundle. When only `revise` tasks remain, clear them in one bundled fix pass and re-review to `approved` before Step 3.
