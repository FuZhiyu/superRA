...
**Last updated:** 2026-05-02 (main re-synced; sync-review correction applied)
**Status:** In Progress
...
- **2026-05-02 re-sync (semantic-merge, standalone mode):** Merged `main` into `domain/writing-skills`. The first commit `fde4751` resolved all three-way merge conflicts but left untouched ~50 paths where the branch had pre-merge drift from main without a conflict (hook surfaces, harness-adapter references, agents-harness symlinks, archived plans). The dispatched sync reviewer flagged this as a CRITICAL take-main-verbatim violation; a correction commit then absorbed every non-allowlisted divergent path from main (`git checkout main -- <path>` where main has it, `git rm <path>` where it does not). Final post-merge `git diff main..HEAD` is now a clean minimum-net-diff: writing-vertical artifacts + writing-references docs + handoff docs + five surgically-rethreaded routing surfaces only. Three behavioral consequences fixed by the correction: (a) the three autoload hooks (`autoload-superra`, `ensure-using-superra`, `ensure-agent-orchestration`) and their `hooks.json` wiring are restored (runtime autoload would have been broken otherwise); (b) `.agents/skills/*` symlinks restored (agents-harness skill discovery now works); (c) `skills/using-superRA/references/gemini-tools.md` no longer says "execution-workflow" (the silent rename re-introduction was the most-flagged finding). Re-threaded writing-vertical additions into the surfaces main reshaped: writing row in `using-superRA/SKILL.md` Skill Inventory + Skill-Load Manifest add-on table; writing row in `planning-workflow/SKILL.md` Phase 1 vertical table; writing notes in `integration-workflow/SKILL.md` §Protect and §When to Lighten; writing rows in `skills/CATEGORIES.md` and `README.md` domain tables. Refreshed stale references inside `skills/writing/` to match main's new structure (workflow-pipeline names, `§Universal Principles` rephrased generically since main moved that content into its owning files). Full record in `SEMANTIC_MERGE.md` (two entries: original sync + correction commit). Note: `CLAUDE.md` did not need a writing-specific amendment because main's restructured §Adaptive, Composable Workflows already states "Domain and utility skills stand alone" — the original Task 7 amendment is subsumed.
