...
**Status:** IMPLEMENTED — awaiting re-review. Layout reworked for vertical alignment + arrowheaded, side-routed scope-change edge.
...
**Layout change (2026-04-20 rework).** User inspected the prior revision's render and rejected it: the subgraph layout still looked messy, the scope-change edges lacked arrowheads, and the `§User Feedback and Changing Plans` cross-reference on the edge label was unwanted. Followed the simpler-alternative path from the feedback: dropped both subgraphs entirely and collapsed the internal detail (implementer ⇄ reviewer loop, A → B → C → D chain) into multi-line labels inside three flat phase nodes. Replaced the two undirected dotted edges with a single directed dotted edge `INTEGRATE -. "scope change" .-> PLAN` (arrowhead via `.->`, label trimmed to "scope change"). Rendered to PNG at `-s 2` and inspected through Read: PLAN, IMPLEMENT, INTEGRATE sit in a single vertical column; the scope-change dotted arrow curves out the right side of INTEGRATE, up the right margin, and lands on PLAN with a visible arrowhead; no edge crosses through the column; no `§User Feedback and Changing Plans` text appears in the diagram.

**Design choices:**

- `flowchart TB`. Three flat phase nodes (PLAN, IMPLEMENT, INTEGRATE) plus a terminal `merged` node. The implementer ⇄ reviewer loop and the A → B → C → D chain live inside each phase node's multi-line label — readable at a glance, no extra subgraph boxes cluttering the column.
- Scope-change re-entry is a single arrowheaded dotted edge `INTEGRATE -. "scope change" .-> PLAN`. A researcher skimming the diagram sees one clear return path; the README prose above carries any cross-reference to `planning-workflow §User Feedback and Changing Plans`.
- Shows only what the Task 1a constraints require: the three phases, the implementer/reviewer loop (as label text), Phases A–D (as label text), the scope-change re-entry, and a `merged` terminal node off INTEGRATE. Still excludes: skill-load manifest detail, cross-cutting skills rail, stage → reference mapping (those live in `using-superRA`).

**Validation.** Rendered via `npx -p @mermaid-js/mermaid-cli mmdc -i v2.mmd -o v2.png -b white -s 2` and visually inspected through the Read tool on the PNG. Confirmed against the acceptance gate: (1) three phase boxes vertically aligned in a single column; (2) scope-change arrow has a visible arrowhead pointing at PLAN; (3) scope-change edge routes along the right side, does not cross through the column; (4) no `§User Feedback and Changing Plans` text appears. GitHub Mermaid uses a compatible feature subset — only standard `classDef` / `-.->` / `-->` syntax used.

## Task 1b: Rewrite README.md as a careful user-facing doc

**Status:** IMPLEMENTED — awaiting re-review (diagram re-embed only; new layout reworked in Task 1a).
