...
- **Doc before report.** Every material review finding lands in the blockquote in `PLAN.md` **before** it appears in your status return. The blockquote is the record for accepted review findings; use the status report for rationale, uncertainty, or task-shape recommendations that the orchestrator has not yet promoted into the docs.
...
- Any step, step code, or task objective — even if you believe it is wrong. Raise in-scope correctness issues in your blockquote and let the orchestrator decide whether to rewrite the step or re-enter the changing-plans protocol for whole-task restructure.
