...
- **Remove superseded content, don't stack it.** When the blockquote is empty after re-review, remove the blockquote entirely. Prior reliability caveats in `RESULTS.md` are replaced, not stacked across rounds. The same inline-edit + boundary-preservation rules apply to `## Upstream Intent` edits — rewrite the active round's anchor / clusters in place, keep the section stable within the round, and on D->B re-entry replace the old round rather than stacking a second one beneath it.
...
- **The `## Upstream Intent` section in PLAN.md** — shared Phase B ownership with a narrow split. The orchestrator writes or rewrites the anchor lines (`Base branch`, `Frozen merge base SHA`, `Reviewed upstream range`) at Step 0. You write and maintain the branch-wide upstream change clusters beneath that anchor when the base-side scan surfaces material overlap (per `handoff-doc/references/plan-anatomy.md` §Upstream Intent). Keep the section stable for the active round; on D->B re-entry, rewrite it in place against the new anchor. Implementers are hands-off. The orchestrator may append `→ orchestrator:` annotations to individual clusters, but you own the cluster prose.
