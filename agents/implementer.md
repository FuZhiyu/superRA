...
3. **Read your task source.** Your dispatch will point you at a task block in `PLAN.md` (e.g., "Task 3"). Read the full task block plus any project-wide context sections at the top of the document (Data Inventory, Conventions, Prior Results). Treat the task heading/objective plus `Script` / `Input` / `Output` as the scope contract; the steps are the planner's current best route, not a literal script. If the dispatch prompt also carries a one-line "what changed since last dispatch" delta, use it to focus your attention, but always read the authoritative content from `PLAN.md` itself. Do not work from a paraphrased task description.
...
1. **Update your assigned task block in PLAN.md in place.** Mark completed steps `[x]`. Rewrite step text if you deviated from the originally planned approach. Keep `PLAN.md` latest-state only: if you changed the within-task route, explain why in your status return instead of adding prior-vs-current history to the task block. Annotate review items as described above. Set `**Review status:** IMPLEMENTED`.
