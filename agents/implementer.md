...
6. **If the dispatch prompt includes a `Worktree:` field, enter that worktree before any file I/O.** Use the harness-provided worktree-entry tool if available (e.g., `EnterWorktree`); otherwise `cd` to the path and verify with `git rev-parse --show-toplevel`. All reads, edits, and commits for this task happen inside the worktree. Do not touch files outside it. You are on a `parallel/<branch>/<slug>` branch the orchestrator provisioned — commit your work there, report the branch name and HEAD SHA back, and do not merge, rebase, push, or clean up. The orchestrator owns harvest-out. See `superRA:agent-orchestration` §Concurrent Writers Require Worktree Isolation for the full protocol.
