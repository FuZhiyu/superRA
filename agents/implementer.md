...
3. **Read your task source.** Your dispatch will point you at a task block in `PLAN.md` (e.g., "Task 3"). Read the full task block plus any project-wide context sections at the top of the document (Data Inventory, Conventions, Prior Results). At Stage `integration`, also read `## Upstream Intent` (if present) plus every task-local overlap note for your in-scope tasks before touching code — your job is to rebase branch intent onto that upstream contract, not preserve prior branch structure by default. The dispatch prompt also carries a one-line "what changed since last dispatch" delta — use it to focus your attention, but always read the authoritative content from `PLAN.md` itself. Do not work from a paraphrased task description.
...
- **The `## Upstream Intent` section in PLAN.md.** The orchestrator owns the anchor lines; the integration reviewer owns the branch-wide change clusters. You read this section before integration edits, but you do not rewrite, append to, or delete it.
