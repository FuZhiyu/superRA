...
## Revision Notes
Maturation decision (2026-07-01 integration pass): keep as a top-level leaf; distil `## Results` to a short note pointing at the shipped rule (`md_integrity.py` `inline-math-line-break`), its tests, and the live doc break it fixed; drop the integration self-check trail.
