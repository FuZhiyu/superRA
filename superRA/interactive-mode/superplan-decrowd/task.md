...
**Regression protection.** [test_contract.py](../../../tests/harness-instruction-following/test_contract.py) now checks that both routed references remain reachable and that the Phase 2 stop, planning-review gate, user review, execution handoff, wrapper-first/no-checkbox/dependency gates, Phase 4 self-review, and six-step task-tree-change protocol remain present in their owning files. Red-green verification perturbed the final no-resume-before-commit expectation: the test failed, then passed after restoration.
