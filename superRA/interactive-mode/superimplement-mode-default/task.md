...
**Regression protection.** The deterministic contract test in [test_contract.py](../../../tests/harness-instruction-following/test_contract.py) locks subagent-driven defaulting and explicit interactive opt-in. The orchestrator evaluator and fixtures in [tests/harness-instruction-following](../../../tests/harness-instruction-following/) now require structural implementer and reviewer dispatch evidence for the default-path smoke; the retired trivial/direct-mode skip-pass and its stale load-contract/README descriptions are removed. Prose-only interactive narration is a negative fixture and cannot mask a missing default dispatch.
