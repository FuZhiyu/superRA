...
Drove samples through the dashboard's exact config — `markdownit({ html:false, linkify:true })` then `md.use(texmath, { engine: katex, delimiters: 'dollars' })`, mirroring [base.html:1649](../../../skills/task-tree/scripts/templates/base.html#L1649) — using the repo's own vendored assets ([skills/task-tree/scripts/vendor/](../../../skills/task-tree/scripts/vendor/): `markdown-it.min.js`, `texmath.min.js`, `katex.min.js`) loaded into a Node `vm` sandbox (Node v25). This is the same markdown-it + texmath + KaTeX stack the standalone dashboard inlines.
