...
- The three migrated tasks (`task-system/planning-redesign/review-planning-protocol/03-planning-review-mode`, `lean-interface/01-core-in-using-superra`, `lean-interface/02-task-system-slim`) each read under `/usr/bin/python3 task_read.py --path <t>` with exit 0 and clean stderr — the regression in finding 1 is gone.
