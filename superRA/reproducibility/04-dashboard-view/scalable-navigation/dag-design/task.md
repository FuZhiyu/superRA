...
- Highlight cyclic dependencies in the dashboard's error red, with an accessible explanatory label so color is not the only signal. Distinguish a cycle in collapsed task groups from an executable-step cycle; do not imply that aggregation alone makes execution impossible. Keep acyclic edges neutral and preserve cycle visibility during edge inspection.
