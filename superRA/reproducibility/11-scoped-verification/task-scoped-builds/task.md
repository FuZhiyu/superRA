...
The design is hand-authored from the researcher discussion and code inspection. Independent thorough correctness/scope-fidelity review approved the fixes with no remaining findings. Its final six targeted checks verified metadata transitions, changed-byte detection, forced execution, and the original regressions; the separate dashboard redesign is outside this change.
