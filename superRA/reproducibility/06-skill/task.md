...
- **Validation criteria:** each file passes the CLAUDE.md three-test gate line by line and the terse style; frontmatter description names the triggers; one realistic dry run: an agent given only this skill and a fixture project registers a two-step pipeline that `repro build` executes (transcript or command log attached).
