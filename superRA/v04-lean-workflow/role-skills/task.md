...
- Create the role skills (working names `skills/implement-task`, `skills/review-task`) carrying the protocol now in `agents/implementer.md` / `agents/reviewer.md`. This task moves and re-homes content; protocol rewrites (calibration, scope, writing economy) belong to sibling tasks.
