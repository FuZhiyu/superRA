...
- Fix the planning instructions that produce over-granular trees, applying the ranked edits in the [over-granularity diagnosis](../attachments/diagnosis-overgranular-planning.md) §c: a shared-edit-surface "do not split" rule and a bidirectional granularity self-review item at minimum, plus re-anchoring the sizing cost floor off dispatch (each task costs a contract, results record, verdict, and reading time regardless of how it executes).
