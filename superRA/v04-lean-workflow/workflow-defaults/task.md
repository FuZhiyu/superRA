...
- Update task-sizing guidance (`task-tree-design.md` §Splitting Tasks) to the new execution economics: split for parallel dispatch or genuinely independent verdicts, not one-task-per-concern; children that would edit the same files or reload the same context belong in one task.
