...
**Two of the objective's three traces were never executed** — the broad-frontier recommendation trace and the high-stakes review recommendation. The second attempt escaped its scratch fixture and committed into this repo's own worktree (reset as `7014a366`), so live tracing was stopped rather than risk a second escape. The `guard-foreign-checkout` gate from [task-tree/agent-cwd-isolation](../../task-tree/agent-cwd-isolation/task.md) closes the escape, and the trace harness registers it in-process. The two traces remain owed here as validation debt: they need API credentials, a broad-frontier fixture, a completed high-stakes fixture, and graders. Nothing verifies those two behaviors beyond the instruction text.
