...
- Rebase verification passes the 17-case shared guard suite, Claude manifest wiring, the 16-case Codex hook suite, harness compatibility, and task-tree validation. The instruction-following suite passes 125 of 127 checks; the two failures are unchanged v0.4 baseline assertions for the retired `#### Seat execution` table and `references/decomposition.md` route.
