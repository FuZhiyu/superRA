...
1. **[BLOCKING] `08-deploy`'s recorded in-branch verification path does not work: `workflow_dispatch` requires the workflow file on the default branch.** [08-deploy/task.md:16](08-deploy/task.md#L16) describes the `workflow_dispatch` trigger as "runnable from this branch for verification now," and the validation ([08-deploy/task.md:23](08-deploy/task.md#L23)) gates on a `workflow_dispatch` run from this branch. GitHub only registers `workflow_dispatch` workflows whose file exists on the **default branch** (`main` here): with the workflow file only on this branch, the Actions UI, `gh workflow run`, and the REST dispatches endpoint all refuse the dispatch. The in-branch path only works after the workflow file is first pushed to `main` (then dispatched with `--ref` this branch, which checks out the branch's scripts) — and pushing anything to `main` outside the squash-merge landing flow is a researcher-level action the task does not record. Fix (one line): extend the existing stop point at [08-deploy/task.md:21](08-deploy/task.md#L21) so the researcher action covers both Pages enablement *and* seeding the workflow file onto the default branch before the `workflow_dispatch` verification — or, if the researcher declines to touch `main` early, fall back to local-build + exported-artifact verification in-branch and defer the live-URL validation to landing.
   → implemented: 08 now states the default-branch registration constraint explicitly, the stop point covers both researcher actions (Pages enablement + seeding the workflow file onto `main`, dispatching with `--ref` this branch), and the validation carries the fallback path (local-build + exported-artifact verification, live-URL check deferred to landing) for when the researcher declines to touch `main` early.
