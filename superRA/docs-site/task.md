...
- 2026-07-01 — Researcher-approved consolidation (Mature & Consolidate): collapse this subtree to this `task.md` plus the postponed `10-version-switcher` child only; delete all other child directories and their attachments. Distil `## Results` here to a matured narrative. Surviving files must be self-contained: plain inline-code repo paths only — no markdown links to files outside `superRA/`, no links to deleted descendants. Normalize frontmatter to `title`/`status`/`depends_on`. Two external citations move with the fold: the docs-authoring contract now in `01-information-architecture` §3 relocates into `docs/site/task.md` (the doc-source root it governs), and the `docs/build_site.sh` header comment that cites `14-canonical-showcase/02-progression-exports/task.md` carries the fixture-refresh instructions directly instead.
