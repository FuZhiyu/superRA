...
- **Skill-page quality bar (per-skill pages under `02`/`03`).** Each per-skill page is an operating manual, not a pitch. It states three things concretely: (1) the specific failure a *bare* agent produces that this skill prevents — named, not gestured at (e.g. theory-modeling: an unaided agent invents fresh notation and lets one symbol silently mean two things; the skill forces reuse of existing, consistent notation with every symbol defined before use); (2) what the skill actually does — its real modes, gates, checks, and commands; (3) how a researcher invokes it, with concrete example prompts/commands and usage recipes (e.g. writing: edit a section, leave it unstaged, ask the agent to polish the unstaged diff; drop an inline `% TODO:`/`[fill in]`; fence a block `DO NOT EDIT`; trigger Review vs Polish vs Draft). Write from the skill's own `SKILL.md` + references, not the prior thin page; drop advertisement voice ("You get…" openers, empty intensifiers). Keep the exhaustive contract in the linked `SKILL.md` (authority-not-paraphrase) — the page shows how to operate the tool, the skill file is the full spec.
