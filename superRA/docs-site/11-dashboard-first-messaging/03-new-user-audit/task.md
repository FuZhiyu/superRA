...
The dashboard is genuinely top-billed, not buried. On both front doors it is item #1, carries the monitor + handoff duality in one sentence, and lands the self-referential "you are reading one" hook ([README.md:9](../../../../README.md#L9); [docs/site/01-welcome/task.md:15](../../../../docs/site/01-welcome/task.md#L15)). The "why not an unguided agent / why not Superpowers" contrast is concrete and fair — it characterizes superRA's own human-in-the-loop emphasis for exploratory social-science research rather than knocking the alternative ([docs/site/01-welcome/task.md:19](../../../../docs/site/01-welcome/task.md#L19)). The README is self-sufficient for a 60-second skim: what it is, the dashboard, the workflow diagram, install. The value proposition is compelling and concrete (the "half the sample silently dropped" failure mode in the README lands).

Two advertising defects found and fixed:

1. **[fixed] slide-design advertised but not shipped on this branch (carryover from `01`).** The welcome page framed all four of *data analysis / theory modeling / academic writing / slide design* as "domain skills." Only the first three are research verticals per [`skills/CATEGORIES.md`](../../../../skills/CATEGORIES.md) — and a review check confirmed `slide-design` ships on **no** state reachable from `better-handoff`: no `skills/slide-design/` directory, absent from CATEGORIES.md, and named in neither the README nor any other doc page (it lives only on the separate `slide-design-vertical` branch). Advertising it on the public welcome page would claim a capability this branch does not ship, and the carryover note's "frame it by capability" instruction does not extend to advertising an absent skill. Dropped the slide-design claim and aligned the welcome page with the README's actual three-vertical framing: "domain skills … for the research work at hand — currently data analysis, theory modeling, and academic writing" ([docs/site/01-welcome/task.md:17](../../../../docs/site/01-welcome/task.md#L17)). The user's stated wish to feature slide design is preserved as an explicit pending decision under §Left for the user, since whether to forward-advertise a not-yet-landed vertical is an intent call the Objective reserves for the user.
...
- **Feature slide design once the vertical lands? (intent decision).** The carryover note says the user "explicitly wants slide design featured," but `slide-design` is not on `better-handoff` — it lives on the separate `slide-design-vertical` branch. To keep the welcome page factually accurate and consistent with the README *now*, the unshipped claim was dropped (see Q1 fix #1). Two paths when slide-design merges to trunk: (a) leave the front doors at the three research verticals and mention slide decks only on a presentation/how-to page, or (b) feature it on both the welcome page and the README, framed as a presentation capability (not a fourth research domain, per [`skills/CATEGORIES.md`](../../../../skills/CATEGORIES.md)). Either way, the welcome page and README must be edited together so the two front doors agree. This needs the user's call on timing (advertise ahead of the merge vs. after) and prominence.
