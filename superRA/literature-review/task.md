...
**Metadata backends.** Semantic Scholar (relevance search, backward `/references`, forward `/citations`, plain-text abstracts) + **Crossref** (authoritative published-version-of-record metadata by DOI; covers NBER working papers) + arXiv (q-fin/econ preprint abstracts). **No OpenAlex** (data quality). **No Docker / translation-server** (setup friction) — the Zotero item is built from the verbatim Crossref/S2 record via a small mapper.

**Metadata policy — never agent-authored.** Bibliographic fields are queried by identifier and stored **verbatim**; an agent never types them. Metadata is always taken from the **published version of record** (Crossref DOI). SSRN working papers usually lack a Crossref DOI → discover/resolve them via S2/web, and adopt the published Crossref metadata once a published version exists.
