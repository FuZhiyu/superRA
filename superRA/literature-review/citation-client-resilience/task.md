...
**Post-review fix (MINOR).** Review surfaced that under `--no-cache` the `RateGate` still creates `gate-<backend>.json` files in the coord dir but the self-ignoring `.gitignore` was written only inside the cache-enabled branch. Resolved by calling `_ensure_cache_gitignored(coord_dir)` unconditionally in `build_default_transport`, so the coord dir is self-ignored regardless of the cache flag. Suite still 64 passing.
