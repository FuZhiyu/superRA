...
**Framing reworked by [04-objective-and-method-redesign](../04-objective-and-method-redesign/task.md).** The three-branch prune *behavior* installed here is kept, but the dense asymmetry-explanation prose this task added (the "deletion drops a hunk so no later reviewer sees what vanished…" sentence) is rewritten there as plain "walk each hunk, every hunk earns its place" triage. The never-silently-delete branch survives as behavior, stated without the essay.
