...
- **No new top-level protocol introduced.** All inherited binding context stays inside planner-owned `## Objective` via scoped `###` subsections, as the objective preferred. The writing vertical's pre-existing `## Project Conventions` top-level surface and promotion ladder were left intact as that vertical's own design — see Notes.
...
- **Generated artifacts** were regenerated, never hand-edited: `.codex/agents/superra_implementer.toml`, `.codex/agents/superra_reviewer.toml`, `skills/using-superRA/references/direct-mode-implementer.md`, `skills/using-superRA/references/direct-mode-reviewer.md` via `python3 skills/codex-superra-setup/scripts/sync_codex_agents.py --scope project`. The generator's inline direct-mode `## Before You Start` strings for both roles were updated at source so the regenerated files carry the ancestor-context wording.
- **Writing vertical scope decision:** the writing skill maintains its own `## Project Conventions` top-level-section protocol with a promotion ladder (`writing/SKILL.md §Project Conventions`, `references/polish.md`, `references/draft.md`, `references/long-form-review.md`, `CLAUDE.md`). This is an established alternative design owned by that vertical; the objective explicitly cautions against introducing/disrupting a top-level `## Conventions` protocol unless required. I gave the manuscript-inventory placement scoped semantics (`writing/references/planning.md`) but left the `## Project Conventions` protocol itself untouched to avoid redesigning a sibling vertical out of scope.
