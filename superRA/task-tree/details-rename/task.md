---
title: "Rename Planner Guidance to Details and state the binding test"
status: not-started
depends_on: []
---
...
Rename the `## Planner Guidance` body section to `## Details`, and make the binding-versus-information test the explicit rule for what goes where.
...
- **Keep reading the old heading, silently.** Section parsing recognizes `## Planner Guidance` as the same section indefinitely, for trees in other projects and for files this sweep never sees. No warning, no auto-rewrite; agents write `## Details` from here on.
