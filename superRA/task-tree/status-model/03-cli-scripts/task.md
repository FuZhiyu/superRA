...
Update all task-tree CLI scripts to remove `review_status` and `integration_status` references, add `archived` status support, and add `--cascade` for branch tasks.
...
**`task_update.py`** ([task_update.py](skills/task-tree/scripts/task_update.py)):
...
**`task_create.py`** ([task_create.py](skills/task-tree/scripts/task_create.py)):
- Removed `review_status: ~` and `integration_status: ~` from `TASK_TEMPLATE`

**`task_read.py`** ([task_read.py](skills/task-tree/scripts/task_read.py)):
...
**`task_query.py`** ([task_query.py](skills/task-tree/scripts/task_query.py)):
