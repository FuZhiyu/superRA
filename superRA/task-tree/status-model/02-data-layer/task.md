...
All changes in [`_task_io.py`](skills/task-tree/scripts/_task_io.py).

**Removed stale fields:**
- Deleted `VALID_REVIEW_STATUSES` and `VALID_INTEGRATION_STATUSES` constants ([`_task_io.py:20`](skills/task-tree/scripts/_task_io.py#L20))
- Removed `review_status` and `integration_status` from `Task` dataclass ([`_task_io.py:24`](skills/task-tree/scripts/_task_io.py#L24))
- `parse_task()` no longer reads these fields; silently ignores them if present in old files ([`_task_io.py:225`](skills/task-tree/scripts/_task_io.py#L225))
- `write_task()` no longer writes these fields ([`_task_io.py:252`](skills/task-tree/scripts/_task_io.py#L252))
- `serialize_frontmatter()` field_order no longer lists them ([`_task_io.py:165`](skills/task-tree/scripts/_task_io.py#L165))
- `validate_frontmatter()` no longer validates them ([`_task_io.py:502`](skills/task-tree/scripts/_task_io.py#L502))

**Added `archived` status:**
- Added to `VALID_STATUSES` tuple ([`_task_io.py:20`](skills/task-tree/scripts/_task_io.py#L20))
- `compute_status()` filters out archived children before rollup; returns `archived` when all children are archived ([`_task_io.py:390`](skills/task-tree/scripts/_task_io.py#L390))
- `_collect_frontier()` skips archived tasks (never on frontier) and treats archived dependencies as satisfied ([`_task_io.py:436`](skills/task-tree/scripts/_task_io.py#L436))
