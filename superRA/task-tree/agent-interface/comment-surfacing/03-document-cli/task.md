...
- Add a short "Comments" subsection covering the read/resolve loop: `task_comment.py list <task>`, `list-tree`, `resolve <task> <id>`, the unresolved/resolved lifecycle, and that comments also surface via `task_read` (task 02). This lands in the **new structure** `lean-interface/02` produces — into `references/commands.md` if that reference is created, else `references/internals.md` — with at most a one-line pointer from the slimmed `task-tree/SKILL.md` body. Do not document it in the old SKILL.md layout.
...
- No duplication with `task-tree/SKILL.md` (single owner per command, per the lean-router design).

**Output:** `skills/task-tree/scripts/task_comment.py`; `skills/task-tree/references/internals.md` and/or `references/commands.md`; `skills/task-tree/SKILL.md` (one-line pointer); `skills/agent-orchestration/SKILL.md` (one pointer line).
...
- `~/.venv/bin/python -m pytest skills/task-tree/scripts/test_task_tree.py -q` → **285 passed**.
