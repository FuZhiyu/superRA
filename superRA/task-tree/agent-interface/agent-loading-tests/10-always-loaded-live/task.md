...
The introspection evaluator's green/red logic is exercised in CI on synthetic answers: green (rule recited, zero loads); red NO_RULE; red backtick-default; red anchor-without-contrast; red rule-recited-but-loaded-on-demand (autoload violation); red both-failures-together.
...
`uv run --with pytest --with pyyaml python -m pytest tests/harness-instruction-following -q` → 75 passed. Both gated entry points print `SKIP` and exit 0 without the gate. README markdown self-diagnose clean.
