...
> 1. [MINOR] Codebase-fit decision — the deleted `tests/test-sync-integration-contract.sh` enforced "do not move the writing review-only task-tree exception into the workflow skills"; its removal silently drops that prose-regression guarantee. The integration edit to [skills/writing/CLAUDE.md](../../../../../skills/writing/CLAUDE.md) correctly removed the now-false "Contract tests in `tests/test-sync-integration-contract.sh` enforce this" sentence, but no test in the new structural suite replaces that ownership assertion. This is consistent with the suite's design (the objective requires avoiding prose-regression assertions), so I kept the edit rather than restoring the test; flagging for orchestrator visibility that a previously-enforced ownership invariant is now doc-only.
> 2. [MINOR] Doc-coherence gap (fixed this turn) — the README coverage matrix was stale vs `load_contract.json` (missing the LC023 `maturation` row, range claim said "LC001–LC022"). Fixed by adding the row and correcting the range. Raising the underlying gap: no CI test cross-checks that the README matrix lists every `load_contract.json` entry, so this class of drift recurs silently on any future LC addition. A follow-up could extend `test_contract.py` to assert the README matrix and the contract entry set agree; out of scope for this integration pass.
