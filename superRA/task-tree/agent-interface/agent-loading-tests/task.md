...
**Mature & Consolidate decision (researcher-approved).** Fold all 13 numbered children (01–13) into this parent task and remove their directories — their per-step implementation detail now lives durably in the test suite under `tests/harness-instruction-following/` and its `README.md` coverage matrix (LC001–LC023), and in git history. Rewrite this task's `## Objective`/`## Results` to current state: a short coverage narrative (what the suite delivers across the four layers — static CI, fixture, live-claude, live-codex — and the CI/manual boundary) that points at `tests/harness-instruction-following/README.md` as the canonical reference, with stale pre-trunk-sync numbers ("136 passed", "LC001–LC022", "22 entries") corrected to current state (140 passed, LC001–LC023). After folding, this task becomes a leaf — set its `status: approved` explicitly. No external `depends_on` edges reference the children, so no edges to repoint.
