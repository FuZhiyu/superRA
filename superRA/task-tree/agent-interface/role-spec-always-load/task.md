...
**Live evidence (orchestrator ran the SDK harness against the subscription, sonnet):** a dispatched real `superRA:implementer`, asked to state its markdown file-citation rule **without loading any skill or reading any file**, answered "markdown links with line anchors, not backtick-wrapped paths" with **zero `Skill`-tool loads** — proving `report-in-markdown` is autoloaded into context via the frontmatter `skills:` field in Claude. The conditional wording therefore no-ops correctly in Claude (no double-load) and is the load mechanism for non-autoload harnesses (Codex). The `Skill` PreToolUse hook is blind to autoloaded skills by construction, so always-loaded coverage uses a discriminating behavioral canary (LC001 in the [agent-loading-tests](../agent-loading-tests/task.md) suite), not the hook.
