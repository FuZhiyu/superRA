...
Create `skills/task-tree/scripts/task_read.py` — the primary way agents understand a task before working on it. Reads a task and injects ancestor context as a chain.
...
Created `skills/task-tree/scripts/task_read.py` (stdlib-only, argparse CLI). Supports `--plan-root`, `--path`, `--no-ancestors`, and `--json` flags. Auto-detects plan root from cwd when `--plan-root` is omitted. Ancestor chain uses hierarchical `#`/`##`/`###` headers with first-section excerpts (10-line cap). Full task frontmatter rendered as human-readable key-value pairs. Sibling dependencies listed with effective status. JSON mode emits `ancestors`, `task`, and `dependencies` keys. All 53 existing tests pass.
