...
### 1. Hook script (`skills/task-tree/scripts/task_hook.py`)
...
```json
{
  "hooks": {
    "PostToolUse": [
      {
        "matcher": "Edit|Write",
        "command": "python3 skills/task-tree/scripts/task_hook.py"
      }
    ]
  }
}
...
### 1. Hook script ([`skills/task-tree/scripts/task_hook.py`](skills/task-tree/scripts/task_hook.py))
...
Added a `PostToolUse` entry matching `Edit|Write` to the plugin's tracked `hooks/hooks.json`. Hook command: `python3 "${CLAUDE_PLUGIN_ROOT}/skills/task-tree/scripts/task_hook.py"`, using the same `${CLAUDE_PLUGIN_ROOT}` convention as all other plugin hooks.
