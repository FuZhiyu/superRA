...
The current placement/update instructions sit partly in `skills/task-tree/references/planning.md` and partly in `skills/superplan/SKILL.md`. That split makes task-tree carry workflow judgment while superplan points back to a utility reference for tree-design decisions. Recent failures show the cost:
...
The redesign should make `superplan` own task-tree design and keep `task-tree` focused on task-file mechanics and command surfaces.

### Design Direction

- Create a superplan-owned reference such as `skills/superplan/references/task-tree-design.md`.
- Rename `skills/task-tree/references/planning.md` to a task-file contract reference, e.g. `task-file-contract.md`, and keep only task-file schema, section semantics, status/dependency mechanics, stale-content rules, and results-shape rules there.
...
- Cross-reference sweep leaves no active source surface pointing to `task-tree/references/planning.md` for tree-design policy.
...
- `task-tree` now owns task-file mechanics through [task-file-contract.md](../../../../skills/task-tree/references/task-file-contract.md). The old `skills/task-tree/references/planning.md` path has been removed from active source references.
...
- The final sweep recorded clean active-reference checks, generated-agent sync, targeted task-tree tests, and retained historical task-record citations only as provenance.
