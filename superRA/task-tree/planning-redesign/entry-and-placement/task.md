...
Rewrite the planning-workflow SKILL.md entry logic and add task placement guidance to the task-tree planning reference. Three integrated changes that reshape how agents enter the planning workflow and decide where work goes.
...
- `skills/task-tree/references/planning.md` — Add §Placing Work in the Tree section with the concern-then-granularity logic (complements existing §Splitting Tasks)
...
- No DRY violations with agent-orchestration, task-tree, or §User Feedback and Changing the Task Tree
