...
Update all remaining operational references to PLAN.md/RESULTS.md across skill files and references. After this task, `grep -rn 'PLAN\.md' skills/` should return only migration/historical references (in `task-tree` and `CATEGORIES.md`), not operational references.
...
- Moved Stage 2 result maturation into task-tree ownership: selected task `## Results` sections mature in place, child tasks carry evidence and caveats, parent tasks summarize direct children selectively, reviewers verify result substance, and orchestrators own upward rollups ([../../../../skills/task-tree/references/planning.md](../../../../skills/task-tree/references/planning.md)).
- Updated Documentation-stage choreography to ask the researcher where summaries should be updated before dispatching doc work, and to review matured task-file results against task-tree results guidance rather than a separate final-results artifact ([../../../../skills/superintegrate/SKILL.md](../../../../skills/superintegrate/SKILL.md)).
...
- [`skills/task-tree/scripts/_task_io.py`](skills/task-tree/scripts/_task_io.py) — added `revision_notes` field to `Task` dataclass, kept `decisions` as legacy
- [`skills/task-tree/scripts/task_query.py`](skills/task-tree/scripts/task_query.py) — added `revision_notes` to JSON output
- [`skills/task-tree/references/internals.md`](skills/task-tree/references/internals.md) — updated data model docs with `revision_notes` field, marked `decisions` as legacy
- [`skills/task-tree/scripts/test_task_tree.py`](skills/task-tree/scripts/test_task_tree.py) — added `revision_notes` to JSON key test and body-sections parse test
...
All 92 task-tree tests pass. `sync_codex_agents.py --check` passes. Contract tests show 48 passed / 6 failed (all 6 pre-existing).
