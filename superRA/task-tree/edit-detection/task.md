...
- **Own baseline, not `HashCache`.** `HashCache` reads every file on a miss, so seeding would hash a multi-gigabyte data dep. The baseline hashes files up to 4 MB ([`HASH_MAX_BYTES`](../../../skills/task-tree/scripts/_edit_detect.py#L37)) and compares larger ones by size and mtime. Directory deps are not walked.
...
- **The hook absorbs its own writes.** Reconcile rewrites ancestor statuses; after a `task.md` change the hook runs detection once more and discards the result, otherwise the next call reported the rollup as an edit.
...
- A Bash edit in another worktree is seen only when the command names an absolute path into it.
