...
Read against the [12-agent-protocol group decisions](../../reproducibility/12-agent-protocol/task.md#decisions-researcher-2026-09-20) and its three children, the redesigned [reproducibility skill](../../../skills/reproducibility/SKILL.md), [_repro_signals.py](../../../skills/task-tree/scripts/_repro_signals.py), and the `guard-foreign-checkout` gate from [agent-cwd-isolation](../agent-cwd-isolation/task.md). Verified by replaying two checkouts against `45dba240`, `a6e7ff6e`, and `f10568e4`, and by running the commands the hook prints. The narrow re-review confirms the mention route closed; one blocking finding remains.

1. `[BLOCKING]` **The escape returns whenever the session's payload `cwd` sits in the foreign checkout.** The filter asks `is_foreign(root, cwd)` with `cwd` from [_cwd](../../../skills/task-tree/scripts/task_hook.py#L839-L841), the payload field, while the gate that owns the rule anchors on `CLAUDE_PROJECT_DIR` first and falls back to the payload only for harnesses that do not set it ([checkout_isolation_gate.py:243-252](../../../hooks/checkout_isolation_gate.py#L243-L252)), *because* — as [agent-cwd-isolation](../agent-cwd-isolation/task.md) §Results states — a `cd` earlier in the session can move the payload `cwd` into the foreign checkout. Replayed at `f10568e4` with `CLAUDE_PROJECT_DIR` on checkout A and a payload `cwd` of checkout B: two `ls` calls with an edit between them rewrote B's `parent/task.md` to `approved` with requoted title and `depends_on: []`, and created `.superra-repro/` there — the original escape, unchanged. Nothing prompts on the way in either, since `cd /B && ls` carries no git verb. [_checkout_scope.py](../../../skills/task-tree/scripts/_checkout_scope.py) promises "a path the gate would prompt about is also a path the hook declines to touch"; sharing the rule but not the anchor breaks that. Fix: resolve the detector's anchor the way the gate does, `CLAUDE_PROJECT_DIR` before the payload `cwd`.

    → implemented: the membership test moved to [_checkout_scope.py](../../../skills/task-tree/scripts/_checkout_scope.py), shared by the gate and by [plan_roots](../../../skills/task-tree/scripts/_edit_detect.py#L75-L107) rather than copied; your two-checkout reproduction is now [test_a_foreign_checkout_is_left_alone](../../../skills/task-tree/scripts/test_edit_detect.py#L197-L210), red before the filter and green after.
    - Confirmed for the mention route: from a session whose `cwd` is its own checkout, naming a path in another one now leaves it untouched, with no `.superra-repro/`, while the session's own tree still reconciles in the same call. The sibling-worktree reach and the gate's own 24 checks still pass.
    - The tool-supplied-path judgment holds, once the anchor is fixed. A foreign `task.md` is the only tool path that reaches a reconcile ([_task_path_from_file_path](../../../skills/task-tree/scripts/task_hook.py#L805-L812) matches nothing else), and that is exactly the shape the gate asks about. It depends on the two surfaces agreeing on the session anchor, and degrades where the gate's `ask` degrades — the runtimes [agent-cwd-isolation](../agent-cwd-isolation/task.md) §Boundaries leaves unverified.

    → implemented: the anchor moved into the shared module as [`session_anchor`](../../../skills/task-tree/scripts/_checkout_scope.py#L26-L42) — `CLAUDE_PROJECT_DIR` before the payload `cwd` — and both surfaces call it: the gate's [`main`](../../../hooks/checkout_isolation_gate.py#L238-L251), which no longer resolves its own, and [_detected_paths](../../../skills/task-tree/scripts/task_hook.py#L881-L900), which passes it to `plan_roots` in place of the payload `cwd`. Your replay is [test_a_payload_cwd_inside_the_foreign_checkout_does_not_unlock_it](../../../skills/task-tree/scripts/test_edit_detect.py#L212-L224): `CLAUDE_PROJECT_DIR` on checkout A, payload `cwd` on checkout B, red on the payload-cwd anchor and green on the shared one.
