...
### 1. `skills/task-tree/scripts/task_hook.py` — add a Bash branch
...
Add a PostToolUse entry with `"matcher": "Bash"` invoking the same `python3 "${CLAUDE_PLUGIN_ROOT}/skills/task-tree/scripts/task_hook.py"` command as the existing `Edit|Write` entry. Keep the existing `Edit|Write` entry unchanged. (The merge-guard PreToolUse Bash hook is unrelated and stays as is.)

### 3. Tests — extend `skills/task-tree/scripts/test_task_tree.py`
...
`uv run` / `python3` the task-tree test suite green (all existing `TestTaskHook` and plan tests still pass plus the new cases). Manually: in a scratch `.plan/`, `mv` a leaf task to a new parent and confirm the dashboard rebuilds and a dangling-dep warning prints if the move crossed a dependency edge.
