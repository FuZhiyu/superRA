...
- Dashboard: live SSE server (FastAPI + htmx + markdown-it + KaTeX); Google Fonts CDN for typography (system-font fallback offline); render libraries vendored and served locally in live mode, inlined for offline export (landed with the [dashboard](dashboard/task.md) hardening subtree).
