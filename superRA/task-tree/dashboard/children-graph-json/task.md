...
**Final diff self-check (integration pass, 2026-07-21):** `git diff ad967e9b..HEAD` (merge-base is exactly `ad967e9b`, so two-dot equals three-dot). This task's surviving hunks — the new `GET /api/children-graph` JSON route + `_children_graph_payload`, the client `childrenGraphFromPayload` replacing the mermaid-regex parse and the `DAG_FILL_STATUS` map, the `/dag` root-param removal, the JSON standalone-fragment embedding, and the rewritten `TestChildrenDagContract` — all trace to the Objective above. No suspicious hunks (no `skills/*`/`agents/*` instruction edits, no base-side restorations, background-supervisor region untouched, `vendor/` hand-managed per its README); host-fit verified against `plan_dashboard.py` conventions (`_`-prefixed helpers, `Task`-data route with no template render). Full script suite: 721 passed, 4 skipped. Status left `approved` — the merged diff needed no codebase-fit change.
