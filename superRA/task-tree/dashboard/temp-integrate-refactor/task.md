...
The pass pruned nothing. The governing diff, recomputed at `9cd918b2` rather than trusted from the maturation triage, matches the survivor set hunk for hunk; the surviving work was already host-consistent; and the two consolidations the objective warned against are correctly *not* consolidations. Three edits landed: the one the objective directed at the parent record, one truth repair the lifted verification waiver forced, and the restoration of a protection assertion the branch had loosened.
...
| Selected protection | [test_dashboard.py:3147-3726](../../../../skills/task-tree/scripts/test_dashboard.py#L3147-L3726) — `TestLocalOpen`, `TestTabTitle`, `TestTabTitleWiring`, plus the `_client_for(base_url=…)` parameter and the `TestWorktreeOpenButton` / `TestFileLinkConsistency` updates | the Protect decision |
...
3. Restored the protection the branch had loosened, at [test_dashboard.py:3086-3092](../../../../skills/task-tree/scripts/test_dashboard.py#L3086-L3092). `test_href_uses_project_root_via_shared_uri_builder` lost its `assert "RESOLVED_ROOT" not in body` when the local-open branch landed, and the whole-body form is no longer the right shape — the local-open branch legitimately targets a task file under `RESOLVED_ROOT`. The guarantee is now scoped to the pre-route `else` branch and strengthened to cover the indirect reach: neither `RESOLVED_ROOT` nor `taskFileVscodeHref` may appear there, so the pre-route deep link stays pinned to the worktree folder. Red-green verified — the assertion fails when the pre-route branch is perturbed to build a task-file href, passes on the real shape.
