...
**Final diff self-check (integration pass, 2026-07-21):** `git diff ad967e9b..HEAD` (merge-base is exactly `ad967e9b`, so two-dot equals three-dot). This task's surviving hunks — the `WorktreeState`-threaded standalone render, deletion of the `_root_task`/`_task_index`/`_project_root` module globals plus `_sync_default_globals`/`_set_module_state` and the `/export` snapshot-restore dance, the shared `conftest.py` `_launch_state` helper, and the repointed tests — all trace to the Objective above. Deleted globals are objective-required, not stray reverts; no suspicious hunks otherwise (no `skills/*`/`agents/*` edits, background-supervisor region untouched, `vendor/` hand-managed per its README); the change reuses the pre-existing `WorktreeState` seam rather than a parallel struct, matching host convention. Full script suite: 721 passed, 4 skipped. Status left `approved` — the merged diff needed no codebase-fit change.
