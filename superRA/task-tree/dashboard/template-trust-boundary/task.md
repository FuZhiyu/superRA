...
**Final diff self-check (integration pass, 2026-07-21):** `git diff ad967e9b..HEAD` (merge-base is exactly `ad967e9b`, so two-dot equals three-dot). This task's surviving hunks — `autoescape=True`, the audited intentional `| safe` sites, the kanban `data-path` + delegated `onKanbanCardClick` (mirroring the existing `onChildCardClick` pattern), the `CSS.escape` comment-selector fixes, and `TestServerSideEscaping` — all trace to the Objective above. No suspicious hunks (no `skills/*`/`agents/*` edits, no base-side restorations, background-supervisor region untouched, `vendor/` hand-managed per its README); reuse of the in-file escaping helpers and delegation pattern matches host convention. Full script suite: 721 passed, 4 skipped. Status left `approved` — the merged diff needed no codebase-fit change.
