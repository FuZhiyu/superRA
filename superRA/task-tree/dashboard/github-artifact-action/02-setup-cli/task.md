...
- `uv run --project skills/task-tree --with pytest python -m pytest skills/task-tree/scripts/test_task_tree.py -k DashboardArtifactWorkflow` — 10 passed.
- `uv run --project skills/task-tree --with pytest python -m pytest skills/task-tree/scripts/test_task_tree.py` — 246 passed, 11 skipped, 1 expected invalid-status warning from the existing diagnostic test.
- `tmpdir=$(mktemp -d); uv run --project skills/task-tree superra dashboard artifact setup --repo-root "$tmpdir" --preview-ref Feature/Foo; test -f "$tmpdir/.github/workflows/superra-dashboard-artifact.yml"` — passed and printed the installed workflow path plus branch artifact preview.
