...
- The dashboard/task-tree test suite passes (`uv run pytest skills/task-tree/scripts/test_task_tree.py` and the live-server tests under `skills/task-tree/scripts/tests/`); add or adjust tests so the unified `generate` path is covered (offline render, no `DASHBOARD_HTML`, data embedded inline).

## Results

The duplicate `DASHBOARD_HTML` constant is gone; the static `generate` path now renders the same [`base.html`](../../../../skills/task-tree/scripts/templates/base.html) the live server serves, in a new **standalone mode**. There is exactly one dashboard source.
...
- **Template flag.** [`base.html`](../../../../skills/task-tree/scripts/templates/base.html) takes a `standalone` Jinja boolean. When set, it emits `window.STANDALONE = true`, the embedded `STANDALONE_FRAGMENTS` map (`| tojson`), a `standaloneFetch(url)` resolver, and replaces `window.fetch` wholesale so every task-data load resolves offline. When unset (serve mode), none of this emits and the page is byte-for-byte the old live shell.
- **Fragments pre-rendered** in [`_build_standalone_fragments()`](../../../../skills/task-tree/scripts/plan_dashboard.py#L893): `/nav` (full sidebar tree), `/nav/<path>` (lazy nav children for every non-leaf), `/node/<path>` (body-only partial per task), `/dag?root=<path>` (sibling child-graph per task), and `/kanban`. Each reuses the same `_render_*` helpers / Jinja env the routes use — verified byte-identical against the live `TestClient` routes in `test_build_standalone_fragments_match_server_routes`.
- **URL keying / encoded-path resolution.** The fragment map is keyed by the **bare (decoded) path**. The `/node` and `/nav` loaders fetch with raw string concatenation, so their URLs carry the path un-encoded and hit the map directly. The `/dag` loader fetches `'/dag?root=' + encodeURIComponent(path)`, which escapes the `/` of a multi-segment path to `%2F` — so the standalone fetch shim ([base.html:1219](../../../../skills/task-tree/scripts/templates/base.html#L1219)) tries an exact-string hit first, then falls back to a `decodeURIComponent(url)` lookup, matching the encoded `/dag` request and the raw `/node`/`/nav` requests against the same bare keys. Without this decode fallback the children-DAG panel rendered empty offline for every nested (multi-segment) task.
...
- Multi-segment resolution verified end-to-end against the real `.plan` tree: the client builds `/dag?root=task-tree%2Fdashboard`, which misses the raw lookup but the shim's `decodeURIComponent` resolves to the bare key `/dag?root=task-tree/dashboard` present in the embedded map.
- Test suite: **215 pass** (`uv run pytest test_task_tree.py tests/`). `TestDashboard` covers the unified path — standalone flag, embedded fragments, no live-server calls, `DASHBOARD_HTML`-removed, default output path, route-parity of the pre-rendered fragments, and (new) `test_standalone_dag_resolves_for_nested_path`, which exercises a nested fixture task under the exact encoded URL the standalone client builds and locks the decode-then-lookup against the byte-identical server route.
