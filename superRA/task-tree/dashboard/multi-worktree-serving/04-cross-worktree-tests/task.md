...
**Deliverables — integration tests in [test_dashboard.py](../../../../../skills/task-tree/scripts/test_dashboard.py) (or a new `test_multi_worktree.py` alongside it), building ≥2 task trees in `tmp_path` and registering them as discoverable worktrees:**
...
- Running the full task-tree suite is green: `uv run --project skills/task-tree --with pytest --with httpx python -m pytest skills/task-tree/scripts -q`.
...
- Reuse the `TestClient` + `tmp_path` + `_write_task_md()` fixtures already in [test_dashboard.py](../../../../../skills/task-tree/scripts/test_dashboard.py); the worktree-discovery tests in `test_worktree_selector.py` show how to register/mock discovered worktrees.
