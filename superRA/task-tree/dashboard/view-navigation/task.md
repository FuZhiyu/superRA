...
- **Dead-code prune.** Removed `Task.has_child_dependency_graph()` from [`_task_io.py`](../../../../skills/task-tree/scripts/_task_io.py); its only caller was the removed mermaid panel. Net governing diff for the method is now zero (it was added on this branch for that panel). No test exercised it.
...
- **Tests (headline deliverable).** Added 15 tests to [`test_dashboard.py`](../../../../skills/task-tree/scripts/test_dashboard.py): suite went **229 → 244**.
...
1. **MINOR** — the Objective's scope sentence "the static `generate` / `DASHBOARD_HTML` path is legacy and self-contained and stays out of scope" is stale: [unify-static-export](../unify-static-export/task.md) deleted `DASHBOARD_HTML` and the static path now renders the same `base.html` this workspace lives in, so the drill-down UI *does* reach the static export. Rewrite the sentence (the scope boundary it drew no longer describes the system).
