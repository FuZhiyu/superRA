...
**Files to modify:**
- `skills/task-tree/scripts/_task_io.py` — add `propagate_parent_status()` and `compute_review_status()`
- `skills/task-tree/scripts/task_update.py` — call propagation after status writes; add `--propagate-all` flag
- `skills/task-tree/scripts/task_hook.py` — call propagation in the post-write hook
...
**Functions added to [`_task_io.py`](skills/task-tree/scripts/_task_io.py):**
...
**Changes to [`task_update.py`](skills/task-tree/scripts/task_update.py):**
...
**Changes to [`task_hook.py`](skills/task-tree/scripts/task_hook.py):**
...
**Validation:** Ran `--propagate-all` on the live `.plan/` tree. Result: 7 parent tasks updated (live-server, comments, comment-ui, tests, dashboard, task-tree, root). Second run confirms idempotency ("All parent statuses already consistent"). `--fix` and `--propagate-all` produce consistent state with no oscillation.
