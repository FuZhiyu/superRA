...
1. **CRITICAL** — [base.html:1834](../../../../../../skills/task-tree/scripts/templates/base.html#L1834): the served-mode image rewrite `img.setAttribute('src', '/files/' + pathPrefix + src)` references `pathPrefix`, but this task's delink (commit `44fcc469`) deleted the `var pathPrefix = taskPath ? 'superRA/' + taskPath + '/' : ''` definition and replaced it with `taskDirRel`/`rootRel`/`repoPathPrefix` ([base.html:1777-1779](../../../../../../skills/task-tree/scripts/templates/base.html#L1777)) without updating this branch (the commit's diff touches no `/files/` hunk). In the live server, expanding any section whose body contains a relative image (e.g. `task-tree/dashboard/hyperlink-styling`'s screenshots) throws `ReferenceError: pathPrefix is not defined` inside `renderMarkdown`; the un-try/catch-wrapped expand handlers ([base.html:2007](../../../../../../skills/task-tree/scripts/templates/base.html#L2007), [base.html:2436](../../../../../../skills/task-tree/scripts/templates/base.html#L2436)) abort, so the section renders empty and comment loading is skipped. Standalone export is unaffected (takes the `window.STANDALONE` branch first). Fix: rewrite to the resolved-root basis this task established — `'/files/' + rootRel + taskDirRel + src` (i.e. `ROOT_PREFIX + '/' + taskPath + '/' + src`) — and re-verify the rendered `<img src>` against a served task with a real figure, the same "real rendered hrefs, not the builder in isolation" bar this task's own Validation set.
   → implemented: rewrote the server-mode image branch to `'/files/' + repoPathPrefix + src` (= `ROOT_PREFIX + '/' + taskPath + '/' + src`, matching the `/files/` route's project-root-relative resolution) ([base.html:1834](../../../../../../skills/task-tree/scripts/templates/base.html#L1834)); verified by executing the real `renderMarkdown` under a node DOM/markdown shim (`TestRenderMarkdownImageRewrite`), which red-greens on the `pathPrefix` ReferenceError.
2. **MAJOR** — [test_dashboard.py:2074](../../../../../../skills/task-tree/scripts/test_dashboard.py#L2074): the only assertion touching `pathPrefix` is `assert "pathPrefix = taskPath ? 'superRA/'" not in BASE_HTML` — it enforces that the *definition stays deleted* while nothing checks the surviving reference, i.e. it guards the regression backwards; this is how item 1 shipped through a green suite. The image-rewrite branch has no coverage in any JS runtime (string-presence tests and hand-extracted node harnesses cannot catch an undefined identifier in an untested branch). Add a behavioral check that executes `renderMarkdown` on a body containing a relative image in server mode (node/jsdom harness, matching the existing extracted-function harness pattern) and asserts the rewritten `src` is `/files/<ROOT_PREFIX>/<taskPath>/<src>`.
   → implemented: added `TestRenderMarkdownImageRewrite` ([test_dashboard.py:1895](../../../../../../skills/task-tree/scripts/test_dashboard.py#L1895)) — extracts the real `renderMarkdown` from base.html and runs it under node against a minimal DOM/markdown-it shim (`sectionName=null` skips block-wrap), asserting the rewritten `src` is `/files/<ROOT_PREFIX>/<taskPath>/<src>` for default root, non-default root, and the empty-path root body; a ReferenceError surfaces as a nonzero node exit. Also turned the backwards guard at [test_dashboard.py:2074](../../../../../../skills/task-tree/scripts/test_dashboard.py#L2074) into a forward one: asserts `'/files/' + repoPathPrefix + src` is present and `'/files/' + pathPrefix + src` is gone. Both red-green confirmed against the reintroduced bug.
