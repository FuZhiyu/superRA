...
**Regression protection** (contract item 5): `TestBackgroundLaunch::test_nonloopback_host_launch_binds_reports_and_reuses` ([test_dashboard.py:4912-4969](../../../../skills/task-tree/scripts/test_dashboard.py#L4912-L4969)) discovers a real non-loopback interface IP (UDP-connect trick; skips if the host has none), launches a real background server bound to it, asserts launch success and the exact bound-interface URL, verifies `/healthz` is reachable at that interface but **not** via 127.0.0.1 (the exact scenario the old loopback-only probe missed), then re-launches with the same arguments and asserts the exact reuse URL, no spawn, and the same PID before stopping it. The default lifecycle regression asserts the exact unchanged localhost launch and reuse messages ([test_dashboard.py:4479-4507](../../../../skills/task-tree/scripts/test_dashboard.py#L4479-L4507)). Supporting unit tests cover the 3-tuple PID-file format and legacy `pid+port` parsing ([test_dashboard.py:4390-4406](../../../../skills/task-tree/scripts/test_dashboard.py#L4390-L4406)).
