...
**Final diff self-check (integration pass, 2026-07-21):** `git diff ad967e9b..HEAD` (merge-base is exactly `ad967e9b`, so two-dot equals three-dot). This task's surviving hunks — the `asyncio.to_thread` offload on `/export`, `/api/worktrees`, and `/api/comments/summary`, the async `resolve_worktree` (cache write kept on the loop thread), the startup Jinja-env warm-up, and `TestEventLoopOffload` plus the `asyncio.run` test-caller updates — all trace to the Objective above. No suspicious hunks (no `skills/*`/`agents/*` edits, no base-side restorations, background-supervisor region untouched, `vendor/` hand-managed per its README). Full script suite: 721 passed, 4 skipped. Status left `approved` — the merged diff needed no codebase-fit change.
