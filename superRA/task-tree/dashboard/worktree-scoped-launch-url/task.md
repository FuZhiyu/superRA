...
This task does not change canonical role specs or generated Codex agents. If scope expands into `agents/*`, regenerate and check `.codex/agents/superra_implementer.toml` and `.codex/agents/superra_reviewer.toml` with `python3 skills/codex-superra-setup/scripts/sync_codex_agents.py --scope project`.
