...
Layer the mobile/touch polish onto the finalized layout from [`01-touch-sidebar`](../01-touch-sidebar/task.md). All edits in [`base.html`](../../../../../skills/task-tree/scripts/templates/base.html); reuse the existing theme tokens and motion discipline.
...
- `cd skills/task-tree/scripts && pytest test_dashboard.py test_task_tree.py` green; `node --check` passes.
...
`cd skills/task-tree/scripts && pytest test_dashboard.py test_task_tree.py` → **383 passed** (was 372; +9 cheap template-level regression tests in `TestTouchPolish` and +2 Playwright-gated rendered tests in `TestTouchPolishRendered`). ([test_dashboard.py TestTouchPolish](../../../../../skills/task-tree/scripts/test_dashboard.py))
