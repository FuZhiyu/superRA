...
Test suite for the worktree discovery and server switching features. Create `skills/task-tree/scripts/test_worktree_selector.py`. Use pytest, consistent with the existing `test_task_tree.py` patterns.
...
Test file created at [`skills/task-tree/scripts/test_worktree_selector.py`](skills/task-tree/scripts/test_worktree_selector.py). 53 tests, all passing (`uv run pytest skills/task-tree/scripts/test_worktree_selector.py -v`).
