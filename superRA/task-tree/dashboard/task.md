...
Those counts predate the local-open and tab-title work. The merged tree — this work plus the incoming attachment subsystem — passes 795 tests across `skills/task-tree/scripts` with the same four expected warnings, on the same command, run at Integrate.
...
**What the green suite still does not reach.** The route's refusals and both open paths run end to end through ASGI, and the tab-title composition runs under node against the extracted functions. The two surfaces the merge created — the attachment open path and the artifact pane's tab title — are pinned by source assertion against `dashboard.js` rather than by execution, and the in-browser click path (button hit area, toast rendering) has never been driven from a terminal in any pass.
