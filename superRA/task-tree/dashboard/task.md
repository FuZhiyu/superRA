...
Researcher-approved distillation (2026-07-21): fold the approved `nonloopback-host-serve` and `worktree-scoped-launch-url` update tasks into this durable dashboard owner and remove both child directories. Retain the protected non-loopback bind/readiness/reuse behavior, exact host-aware worktree-scoped launch URLs, unchanged loopback defaults, and scoped-routing invariant as a short serve-lifecycle protection narrative with current `723 passed, 4 skipped` evidence. Make no other structural change to the dashboard subtree.
