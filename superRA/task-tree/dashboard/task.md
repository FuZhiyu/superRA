...
The dashboard hands files to the machine it runs on instead of only firing a `vscode://` URI, which names one editor and cannot express a workspace. A loopback-gated [`POST /api/open`](../../../skills/task-tree/scripts/plan_dashboard.py#L1413) backs four surfaces: the card-head **Open** button (the OS default application for the type), the header **VS Code** button (the active task's file, inside the window already holding *its* worktree), body file links, and attachment links in a rendered task body. One delegated handler serves all four — a plain left-click calls the route, while any modifier or middle click falls through to the element's own `vscode://` or `/api/artifact` href, so the browser's open-elsewhere gestures survive and the `#L123` line anchor still reaches VS Code.
...
Those counts predate the local-open and tab-title work, whose own suites passed when each was implemented (750 tests across `skills/task-tree/scripts` at the later of the two). **Runtime verification was skipped for this integration at the researcher's instruction: no test suite and no live server were run against the merged tree.** Everything §Opening files, and naming the tab describes about the post-merge shape — the fourth (attachment) open surface and the artifact pane's tab title — is established by reading the merged code, not by executing it.

## Sync Impact

The merge with `origin/main` landed the companion-file/attachment subsystem (PR #52) into the same dashboard surfaces this subtree owns, and both sides were synthesized rather than one picked. Two consequences a maturation or Integrate reader needs: an attachment link is now a local-open surface — `renderMarkdown`'s attachment branch carries `data-open-path` alongside its `/api/artifact` href; and `loadActiveArtifact` sets and clears the tab title on the same terms as the task card, because an attachment pane is a page in its own right. Maturation has since absorbed both into §Opening files, and naming the tab.
...
Maturation review of the fold at `a64c0418` against the Protect decision at `91252c25`. Settled and not to be redone: the anchor position holds — this pass's 16-line RELEASE-NOTES insertion shifted exactly the three `#L` citations into that file, all three are re-cut correctly (+16, verified against the cited prose), no other file in the fold shifted a line, and every anchor the fold added resolves to what it claims. The runtime waiver is accepted; the §Opening files, and naming the tab assertions were re-verified here by reading `plan_dashboard.py`, `base.html`, `dashboard.js`, and `test_dashboard.py`, and every one of them holds, including the four open surfaces, the delegated handler's modifier fall-through, the standalone/doc-mode `local_open` falsity, the attachment pane's tab title, and both card-error clears. The out-of-set repairs to `interactive-mode` and `econ-data-efficiency` were warranted — this pass caused that drift and both edits touch the anchor only. No temporary refactoring task was created this round: the record must pass first, and the two MAJOR items below change which surviving hunks it justifies.

1. **[MAJOR] The fold dropped the child's `### Chrome` and failure-notice paragraphs, leaving roughly sixty lines of surviving diff with no trace in the protected record.** `local-file-open/task.md`, deleted at `a64c0418`, recorded four things §Opening files, and naming the tab does not: the `.vscode-btn` → `.open-btn` rename and its four ripple sites (doc-mode hide rule, tap-highlight list, the `#worktree-open-btn` margin override, `base.html`'s header element); the replacement of the off-palette `#007acc` hover with the `--accent-soft` / `--accent` pair the neighbouring `.share-btn` already uses ([dashboard.css:1097-1125](../../../skills/task-tree/scripts/templates/dashboard.css#L1097-L1125)); the retirement of the solid VS Code brand wordmark for the outline `EDITOR_ICON` / `OPEN_ICON` pair, which is what lets the card-head button name no editor when it opens in the default application ([dashboard.js:1066-1080](../../../skills/task-tree/scripts/templates/dashboard.js#L1066-L1080)); and the in-page notice a refused or failed open raises, without which the button reads as dead since a successful open's only result is on the desktop ([showOpenError](../../../skills/task-tree/scripts/templates/dashboard.js#L1108), [.open-toast](../../../skills/task-tree/scripts/templates/dashboard.css#L1127-L1146)). The Protect decision selected nothing to drop, so all four belong in the parent's `## Results`. As the record stands, Integrate triage finds these hunks unmatched and puts them on the automatic pruning list — reverting a shipped UI change and a shipped user-visible behavior. Add a paragraph to §Opening files covering the control-family restyle (rename, hover, icon pair) and the failure notice, cited.

2. **[MAJOR] The record understates its own verification, and the header button's window-targeting claim lost the only evidence that ever supported it.** §Verification now says runtime verification was skipped and that everything §Opening files describes "is established by reading the merged code, not by executing it." That is true of the merge, but `local-file-open` had verified the section's hardest claim — the header **VS Code** button opening "the active task's file, inside the window already holding *its* worktree" — against the real `code` CLI: a four-window `code --status` before/after table showing each file landing in its own worktree's window rather than the focused one, no window spawned, plus the degraded subfolder case opening a correctly-rooted new window, and a `SUPERRA_EDITOR` recording script capturing the exact `code <worktree> <file>` pairing. The suite can only pin the argv ([test_dashboard.py:3426](../../../skills/task-tree/scripts/test_dashboard.py#L3426)); that `code <folder> <file>` reuses the right window is a claim about VS Code, not about this code, and only that live pass established it. The live DNS-rebinding demonstration survives as a clause with its evidence gone. Carry a compact form of both into `## Results` — they are pre-merge and the merge did not touch that path — and narrow the skipped-verification sentence to the post-merge behaviors it actually covers, so a reader does not conclude the window-targeting claim was never executed.

3. **[MINOR] Two tab-title behaviors from the folded child are absent from the record.** The two halves join only when they differ, so a task whose title equals its branch does not read `main · main` and an export root does not double ([dashboard.js:931](../../../skills/task-tree/scripts/templates/dashboard.js#L931)); and the deep-descent patch re-reads the title once that tick's sidebar update settles, so a cold deep link does not strand the tab on the path slug ([patchTabTitleWhenReady](../../../skills/task-tree/scripts/templates/dashboard.js#L2064)). §Opening files describes neither, and the "pre-fetch state" named in the regressions sentence is the worktree half, not this. One clause each.
