...
Sync review APPROVED at `d80d791c`. The two prior rounds' anchor findings are resolved: every citation in the eight re-anchored task files was re-read against its own prose in the current tree, and the sweep's exhaustiveness holds — all 131 `#L` citations under `superRA/` were classified against the merge's touched-file set, and no citation into a touched file was left unchecked. The folded children keep their drifted anchors by the researcher's decision that both directories are deleted at Mature & Consolidate. The `dashboard.js#L291-L295` call in round two was wrong: that range held the server-mode `wtUrl('/files/…')` rewrite at the merge base and now holds the attachment `data-open-path` branch, so the recut to `#L375-L379` was required.

**[ADVISORY] `test_dashboard.py#L5828-L5918` in [worktree-scoped-launch-url](worktree-scoped-launch-url/task.md) over-runs the four regressions it cites.** The launch/reuse/selector tests run 5828-5897; 5899-5918 is `test_stop_terminates_and_removes_pid`, which the citing sentence does not name. `#L5828-L5897` bounds the claim. The anchor was already mis-cut at the merge base (the same four tests sat at 5207-5277 there), so this is inherited looseness, not merge drift.
→ implemented: recut to `#L5828-L5897`, the last line of `test_scoped_url_encodes_collision_safe_selector` ([worktree-scoped-launch-url/task.md:27](worktree-scoped-launch-url/task.md#L27))
