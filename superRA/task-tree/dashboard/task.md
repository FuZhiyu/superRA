...
Every file surface therefore opens the same way. The artifact pane's button was the last that did not: it read **Open raw** and only ever handed the file to a browser tab over `/api/artifact`, an affordance the task-file button has no equivalent of. It is now plain **Open**, carrying the route address for the plain click and the same `/api/artifact` href for a modifier click ([`buildArtifactPreviewHead`](../../../skills/task-tree/scripts/templates/dashboard.js#L1389)) — the pair an attachment link in a task body already carried. Both addresses come from one composition helper, [`taskRelOpenPath`](../../../skills/task-tree/scripts/templates/dashboard.js#L1103), which `taskFileOpenPath` now delegates to, so the card head and the artifact pane cannot drift apart on a `--root`, a nested tree, or a rootless forest. Outside the gate — doc-mode, standalone, an off-loopback bind — the button keeps the href alone and degrades exactly as the task-file button degrades to its `vscode://` or GitHub link.
