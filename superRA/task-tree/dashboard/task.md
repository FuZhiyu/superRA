...
**[BLOCKING] Sync-caused stale line anchors in `## Results` were deferred instead of resolved, and the deferral rationale does not hold for the sections that carry them.** The merge shifted `plan_dashboard.py` by ~53 lines above the watcher region: `_ensure_watcher` moved 526 → 579 and `_schedule_forced_process_exit` / `_stop_watcher` moved 562/585 → 615/638. §Reconnect freshness still cites [plan_dashboard.py:505-546](../../../skills/task-tree/scripts/plan_dashboard.py#L505-L546), which no longer reaches `_ensure_watcher` at all, and §Shutdown lifecycle protection still cites `#L557-672`, which now opens inside `_ensure_watcher`'s body rather than at the watchdog. §Release 0.3.3 cites [RELEASE-NOTES.md#L5](../../../RELEASE-NOTES.md#L5) as "the dated 0.3.3 release notes"; the merge inserted a `0.3.5` section, so line 5 is now `## [0.3.5]`. `## Sync Impact` hands all of this to maturation as a to-do, but the stated reason — that folding the two open/title children rewrites those sections anyway — does not cover §Reconnect freshness, §Shutdown lifecycle protection, or §Release 0.3.3, which consolidation has no reason to touch. Re-anchor them in a propagation commit and reduce `## Sync Impact` to post-sync context.

**[ADVISORY] The `## Sync Impact` drift inventory is inaccurate in both directions.** It says "every `test_dashboard.py` range" drifted; the merge changed that file only from line 2632 down, so the `1162-1518`, `1177-1193`, and `1240-1327` anchors are untouched (only `test_dashboard.py:4850` moved, by ~31 lines, and it already pointed at unrelated content before the merge). It also omits the `RELEASE-NOTES.md#L5` anchor, which the merge did move.

**[ADVISORY] `test_body_links_carry_open_path_beside_the_vscode_href`'s new docstring claims a code path that cannot run.** It says the `contentDirRel` base means "a link inside a rendered attachment resolves from its own directory," but `renderMarkdown`'s `if (artifactPath)` branch returns unconditionally at [dashboard.js:300](../../../skills/task-tree/scripts/templates/dashboard.js#L300), so an attachment-rendered link never reaches the `vscode://` / `data-open-path` composition below it. With `artifactPath` unset, `contentDirRel === taskDirRel`, so the rewrite is behavior-preserving — only the docstring's justification is wrong.
