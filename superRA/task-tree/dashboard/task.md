...
**[BLOCKING] The anchor drift the fix round resolved inside this file is unresolved in three sibling task records.** The branch side of the merge moved `plan_dashboard.py` by +167 lines below its old 1273 and `dashboard.js` by +12 below 121 and ~+149 below 1112, so already-integrated records that cite those files now point at unrelated code: [worktree-scoped-launch-url](worktree-scoped-launch-url/task.md) cites `plan_dashboard.py#L179-L182` for the URL constructor (`_dashboard_url` is now 203; the anchor now lands on the `_launch_wt_id` global) and `#L2315-L2324` / `#L2384-L2392` / `#L2679-L2699` for the launch/reuse/foreground emission sites (all +167), plus `test_dashboard.py#L5340-L5465` and `#L5925-L5942` (~+589); [02-dashboard-artifact-data](../task-local-artifacts/02-dashboard-artifact-data/task.md) cites `plan_dashboard.py#L1433-L1482` and `#L1901-L1946` (both +167); [03-dashboard-artifact-ui](../task-local-artifacts/03-dashboard-artifact-ui/task.md) cites `dashboard.js#L136-L158` (+12) and `#L1451-L1459` (~+149). Nothing downstream rewrites these three files — the same test that made §Reconnect freshness, §Shutdown lifecycle protection, and §Release 0.3.3 blocking rather than deferrable. Their `_artifacts.py`, `internals.md`, and `dashboard.js#L291-L295` anchors are unaffected and need no edit.

**[ADVISORY] The two folded children carry the same drift, and their disposition decides whether it matters.** Every `plan_dashboard.py`, `base.html`, and `dashboard.js` anchor in [local-file-open](local-file-open/task.md) and every `dashboard.js` anchor in [task-scoped-tab-title](task-scoped-tab-title/task.md) is stale (`BOUND_HOST` 88→98, `_local_open_enabled` 1328→1368, `POST /api/open` 1373→1413, `serve()` 2218→2472, `window.LOCAL_OPEN` 233→240; `SITE_TITLE` 848→911, `setTabTitle` 852→915, `refreshTabTitle` 859→922, `patchTabTitleWhenReady` 1257→2064, `worktreeLabel` 2559→3450, `fetchWorktrees` 2661→3533). If consolidation keeps them as the detail record §Opening files points to, they are permanent records and need re-anchoring; if it deletes them, the work is wasted. Their `dashboard.css` and `internals.md` anchors are unaffected, and local-file-open's `test_dashboard.py#L3155-L3518` for `TestLocalOpen` was already stale before the merge (that class sat at 3333 at the pre-merge tip, 3347 now).

**[ADVISORY] The two re-anchored `plan_dashboard.py` ranges tile the region instead of bounding the functions the prose names.** `#L579-L637` opens exactly on `_ensure_watcher` but runs 38 lines past its end at 599, through `_is_benign_awatch_teardown_race` and `_schedule_forced_process_exit`; `#L638-L747` is `_stop_watcher` exactly, but it excludes `_schedule_forced_process_exit` (615-635), which is where §Shutdown lifecycle protection's "restricted to a standalone `plan_dashboard.py` process running as `__main__` on its main thread" is implemented — the pre-merge `#L557-672` did cover it. `#L579-L599` and `#L615-L745` would match both sections' claims. Separately, `test_dashboard.py#L5414` is the blank line above `test_detached_repeated_abrupt_disconnects_exit_cleanly` (5415), and §Reconnect freshness's `#L1177-L1193` opens just past the cross-worktree isolation assertions it cites (1170-1176) — pre-existing, not merge drift.
