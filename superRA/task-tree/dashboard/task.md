...
- **Serve lifecycle** — background-by-default launch with idempotent reuse, idle self-exit, `stop`, and a loopback-default `--host` bind whose readiness probes, reuse checks, and worktree-scoped launch URLs follow an explicitly requested interface.
...
The background supervisor derives its health-check address from `--host` and records `<pid> <port> <host>`, so readiness, failed-bind diagnosis, idempotent reuse, and cross-mode conflict checks use the interface the server actually bound; wildcard binds probe through loopback, while explicit LAN or Tailscale interfaces are probed directly ([plan_dashboard.py:2107-2279](../../../skills/task-tree/scripts/plan_dashboard.py#L2107-L2279), [plan_dashboard.py:2303-2501](../../../skills/task-tree/scripts/plan_dashboard.py#L2303-L2501)). Launch, reuse, and foreground messages compose that host with the canonical percent-encoded `?wt=` selector: explicit non-loopback binds report a reachable interface URL, cross-worktree reuse routes to the invoking worktree even under basename collisions, and loopback/wildcard launches retain the exact `localhost` defaults ([plan_dashboard.py:179-188](../../../skills/task-tree/scripts/plan_dashboard.py#L179-L188)).

Permanent regressions protect collision-safe scoped routing ([test_dashboard.py:670-714](../../../skills/task-tree/scripts/test_dashboard.py#L670-L714)), legacy and host-aware PID records ([test_dashboard.py:5158-5174](../../../skills/task-tree/scripts/test_dashboard.py#L5158-L5174)), exact loopback launch/reuse output and cross-worktree reuse ([test_dashboard.py:5251-5353](../../../skills/task-tree/scripts/test_dashboard.py#L5251-L5353)), the recorded-host cross-mode invariant ([test_dashboard.py:5571-5611](../../../skills/task-tree/scripts/test_dashboard.py#L5571-L5611)), and a real non-loopback bind/readiness/reuse/stop cycle ([test_dashboard.py:5735-5800](../../../skills/task-tree/scripts/test_dashboard.py#L5735-L5800)). Live Tailnet validation additionally confirmed that a peer could fetch `/healthz` from the reported worktree-scoped interface URL, an identical relaunch reused the same PID and URL, and `stop` made the peer connection fail.
...
**Verification.** The full task-tree script suite passes at 723 passed, 4 skipped (`uv run --with pytest --with pyyaml --with fastapi --with jinja2 --with 'uvicorn[standard]' --with watchfiles --with httpx python -m pytest skills/task-tree/scripts`). Each hardening diff traced cleanly to its objective with no scope-ambiguous hunks, no generated artifacts were hand-edited, and `vendor/` stayed hand-managed per its `README.md`; focused red-green and byte-identical-export checks remain recorded in this task's git history.
