...
Those counts predate the local-open and tab-title work, whose own suites passed when each was implemented (750 tests across `skills/task-tree/scripts` at the later of the two).
...
**Runtime verification was skipped for this integration at the researcher's instruction: no test suite and no live server were run against the merged tree.** What that leaves unexercised is the post-merge shape specifically — the fourth (attachment) open surface and the artifact pane's tab title — which is established by reading the merged code rather than by executing it. Separately, the in-browser click path (button hit area, toast rendering) has never been driven from a terminal in any pass and rests on source assertions.
