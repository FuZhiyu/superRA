...
**Final diff self-check (integration pass, 2026-07-21):** `git diff ad967e9b..HEAD` (merge-base is exactly `ad967e9b`, so two-dot equals three-dot). This task's surviving hunks — the `base.html` → `dashboard.css`/`dashboard.js` verbatim extraction (largest single hunk class in the subtree diff), the cacheable `GET /static/{name}` route with SHA-256 ETag/304, the standalone `_build_standalone_assets` inlining of the two new files, the retained inline Jinja config block, and the `BASE_HTML`-concat test updates — all trace to the Objective above. The bulk deletions/additions are the extraction move, not reformatting; no suspicious hunks (no `skills/*`/`agents/*` edits, background-supervisor region untouched, `vendor/` hand-managed per its README). The `GET /static/{name}` route is later extended by `local-assets`; the two land as one coherent route. Full script suite: 721 passed, 4 skipped. Status left `approved` — the merged diff needed no codebase-fit change.
