...
The structural exception is centralized in [_task_io.py](../../../../skills/task-tree/scripts/_task_io.py#L28-L97): child enumeration, task parsing, task-path resolution, validation, sibling/dependency reads, command mutators, dashboard rebuilds, hooks, root discovery, and both migration passes share the rule that `attachments/` is opaque. Structural scans reject symlink task directories and symlink `task.md` files before reading them. The [task-path resolver](../../../../skills/task-tree/scripts/_task_io.py#L925-L963) rejects every task-relative symlink component, then rechecks containment and `attachments/` against the canonical resolved path before returning a location to any mutator. A retained `attachments/**/task.md` can be listed as an artifact but cannot become a task, dependency target, move/update target, hook reconciliation target, migration input, or alias-based write destination.

Task moves now enforce the same boundary for Markdown maintenance. The [link-rewrite scanner and no-follow file opener](../../../../skills/task-tree/scripts/_task_io.py#L618-L760) exclude symlinked Markdown files and symlinked directory components from both the moved-subtree and tree-wide passes. The [shared rewrite application](../../../../skills/task-tree/scripts/_task_io.py#L763-L773) validates the complete queue as regular paths contained by the task root before opening any target through no-follow directory descriptors. The [move preflight and rollback boundary](../../../../skills/task-tree/scripts/task_rename.py#L131-L180) rejects a symlinked structural `task.md` before rename and restores the source path if a queued target becomes unsafe after the rename; the post-move hook uses the same contained writer.
...
Focused regressions cover [external task symlinks and atomic alias rejection](../../../../skills/task-tree/scripts/tests/test_artifacts.py#L183-L245), [move preflight and external Markdown containment](../../../../skills/task-tree/scripts/test_task_tree.py#L929-L1070), structural opacity and traversal budgets, path security and live APIs, artifact-scoped watcher events, a sixteen-level manifest/content/watcher/export round trip, and standalone image-map fallbacks. The contributor-facing [dashboard internals](../../../../skills/task-tree/references/internals.md#companion-file-data-path) document the shared invariant, API, and ceilings.

Verification:

- Focused artifact suite: 25 passed.
- Combined task-tree and artifact regression suite: 303 passed.
- Full task-tree script suite: 760 passed with four expected warnings from dependency deprecations and intentional malformed-task fixtures.
...
1. **MAJOR** — Move rollback is still not transactional across the queued rewrites and later structural maintenance. [`apply_move_link_rewrites`](../../../../skills/task-tree/scripts/_task_io.py#L763-L773) prevalidates path shapes but then truncates and writes targets sequentially; when a later open fails, [`rename_task`](../../../../skills/task-tree/scripts/task_rename.py#L169-L180) restores only the moved directory. With two inbound rewrites and the second target read-only, the source directory returned to its original path but the first file remained rewritten to the now-nonexistent destination. Separately, destination-parent preflight accepts any existing `task.md` ([`task_rename.py:138-143`](../../../../skills/task-tree/scripts/task_rename.py#L138-L143)): a symlinked parent `task.md` allowed the source to move, after which [status propagation](../../../../skills/task-tree/scripts/task_rename.py#L187-L190) rejected that symlink and raised, leaving the task under a directory structural scans do not recognize. Validate the destination parent and every ancestor needed by post-move maintenance before renaming; make queued application leave no earlier rewrite committed when a later target fails, and add regressions proving all file bytes and both task paths remain unchanged for a later read-only queue target and a symlinked destination parent.
   → implemented: [move preflight and rollback](../../../../skills/task-tree/scripts/task_rename.py#L131-L180) now reject a symlinked source `task.md` before rename and restore the source when queued rewrite validation fails; [both rewrite passes and their shared writer](../../../../skills/task-tree/scripts/_task_io.py#L618-L773) exclude symlinked files and directory components, prevalidate regular contained targets, and use no-follow opens. Regressions preserve the source path and every external target byte-for-byte for [source, companion, directory-component, and unsafe-queue cases](../../../../skills/task-tree/scripts/test_task_tree.py#L929-L1070).
