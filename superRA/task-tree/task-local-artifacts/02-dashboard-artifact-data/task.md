...
1. **CRITICAL** — Task moves still follow and write symlinked Markdown outside the task tree. [`compute_move_link_rewrites`](../../../../skills/task-tree/scripts/_task_io.py#L617-L660) reads every `*.md` returned by `rglob` and queues destination writes without rejecting symlinks; [`rename_task`](../../../../skills/task-tree/scripts/task_rename.py#L163-L167) performs the rename and then writes those queued paths. In a cross-parent reproduction with a valid task containing `linked.md -> /tmp/external.md`, moving `source` to `parent/moved` silently changed the external target from `[outside](../shared.md)` to `[outside](../../shared.md)`. A real source directory whose `task.md` is a symlink also passes the source `.exists()` and same-parent prechecks ([`task_rename.py:130-165`](../../../../skills/task-tree/scripts/task_rename.py#L130-L165)); a local link in that target caused the directory move to occur before a stale-path write raised, leaving a partial mutation. Reject a symlinked source `task.md` before every move, exclude symlinked files and symlink-component paths from both link-rewrite passes, and add regressions proving the task path and external target remain byte-for-byte unchanged in both scenarios.
