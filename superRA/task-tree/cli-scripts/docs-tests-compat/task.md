...
Update active instructions, role references, tests, and compatibility notes so users and agents learn the packaged task-tree CLI as the normal interface.

### Scope

- Update `skills/task-tree/SKILL.md`, `skills/task-tree/references/planning.md`, and `skills/task-tree/references/internals.md` to document `superra task ...` commands instead of `<skill-dir>/scripts/...` for normal use.
- Update workflow and role references that mention direct task-tree scripts, including `skills/superplan/SKILL.md`, `skills/using-superRA/references/main-agent.md`, `agents/implementer.md`, and `agents/reviewer.md`.
...
- `rg "superRA/serve|skills/task-tree/scripts|plan_dashboard.py|task_read.py|task_query.py|task_check.py" skills agents hooks README.md` has only intentional compatibility or historical references.
- The packaged CLI tests and existing task-tree tests pass.
...
- Active task-tree instructions now teach packaged CLI commands for normal use: `superra task read/tree/frontier/dag/create/status/result/dep/rename/migrate` and `superra dashboard`, while direct script names remain only in internals, tests, script usage text, or compatibility references ([../../../../skills/task-tree/SKILL.md](../../../../skills/task-tree/SKILL.md), [../../../../skills/task-tree/references/planning.md](../../../../skills/task-tree/references/planning.md), [../../../../skills/task-tree/references/internals.md](../../../../skills/task-tree/references/internals.md)).
...
- CLI tests now cover packaged dashboard export through `uv run --project ... superra`, assert task creation does not create a `serve` wrapper, and verify direct `task_query.py` script compatibility during the transition ([../../../../skills/task-tree/scripts/test_cli.py](../../../../skills/task-tree/scripts/test_cli.py)).

### Verification

- `uv run pytest skills/task-tree/scripts/test_cli.py -q` — 18 passed.
- `uv run pytest skills/task-tree/scripts/test_cli.py skills/task-tree/scripts/test_task_tree.py skills/task-tree/scripts/test_dashboard.py skills/task-tree/scripts/test_worktree_selector.py skills/task-tree/scripts/tests/test_state_preservation.py -q` — 396 passed.
