...
**1. CLI behavior — [`_task_io.py` `resolve_path`](../../../../skills/task-tree/scripts/_task_io.py#L451).** `resolve_path` now strips a leading path segment when it equals **both** the plan-root basename (`plan_root.name`) **and** a known task-root dirname (`superRA` or legacy `.plan`) before joining. So `superRA/task-tree/planning-redesign` resolves to the same directory as `task-tree/planning-redesign` instead of doubling to `superRA/superRA/...`. Because every verb (`read`, `update`, `result add`, `status`, `dep`, `comment`) routes through `resolve_path`, the fix applies across the whole command surface at once.
...
**3. Tests — [`test_task_tree.py` `TestResolvePath`](../../../../skills/task-tree/scripts/test_task_tree.py#L222).** Six regression tests: plan-root-relative path resolves; redundant root prefix is stripped (red-green confirmed — fails on pre-fix code with the exact `superRA/superRA/01-first` doubled path, passes after); legacy `.plan` prefix is stripped; prefix is stripped only when it matches the root basename; a nonexistent path resolves to a path that does not exist (no invented directory); `../` traversal still raises `escapes plan root`.
