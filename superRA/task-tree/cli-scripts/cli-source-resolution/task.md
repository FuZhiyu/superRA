...
> 1. [MAJOR] Resolution-chain failure modes in [wrapper_resolver.py](../../../../skills/task-tree/scripts/wrapper_resolver.py#L59): (a) the GitHub fallback is pinned to `main`, which the file's own comment ([wrapper_resolver.py:66](../../../../skills/task-tree/scripts/wrapper_resolver.py#L66)) admits does not yet carry `skills/task-tree` — the last resort still shallow-clones the full repo (network + disk) before failing the `cli.py` existence test; (b) once a clone exists, every invocation runs `git fetch --depth 1` with no TTL and no timeout ([superra:150](../../../superra#L150), [task-hook:151](../../../../hooks/task-hook#L151)) — in fallback mode each `superra task read` and every PostToolUse hook fire is a network round-trip; (c) the plugin key `superRA@superRA` is hardcoded in the manifest probe ([wrapper_resolver.py:118](../../../../skills/task-tree/scripts/wrapper_resolver.py#L118)), so a marketplace rename silently degrades resolution to the Codex/GitHub branches; (d) the `records[0]` fallback ([wrapper_resolver.py:125](../../../../skills/task-tree/scripts/wrapper_resolver.py#L125)) picks an arbitrary install record, possibly another project's; (e) the shared clone dir has no concurrency guard — a concurrent `reset --hard` can yank source mid-run. Fix at minimum (a) fail fast on the GIT branch until the pin carries the subdir, and (b) gate the refresh fetch behind a TTL.
>    → implemented: (a) added `GITHUB_REF_HAS_SUBDIR` (default `"0"`) gating the GIT branch — `_superra_resolve_source` returns non-zero with no output instead of cloning while the pin lacks the subdir ([wrapper_resolver.py:79](../../../../skills/task-tree/scripts/wrapper_resolver.py#L79)); (b) TTL-gated the refresh `git fetch` via `GITHUB_FETCH_TTL_SECONDS` (24 h) + a `.superra-fetch-stamp` file ([wrapper_resolver.py:226](../../../../skills/task-tree/scripts/wrapper_resolver.py#L226)); also dropped the (d) arbitrary `records[0]` fallback ([wrapper_resolver.py:140](../../../../skills/task-tree/scripts/wrapper_resolver.py#L140)). Regenerated both artifacts; tests `test_resolver_fails_fast_…` and `test_resolver_emits_github_spec_…`. Did not address (c) hardcoded plugin key or (e) clone concurrency guard — the review scoped the required fix to (a)+(b); flagging (c)/(e) as still open.
> 2. [MAJOR] `uv run --script <dir>/scripts/cli.py dashboard export …` crashes with a raw `ModuleNotFoundError: No module named 'fastapi'` (reproduced; [plan_dashboard.py:726](../../../../skills/task-tree/scripts/plan_dashboard.py#L726) imports FastAPI at module top): [cli.py](../../../../skills/task-tree/scripts/cli.py#L340) registers `dashboard` while its PEP 723 block ([cli.py:4](../../../../skills/task-tree/scripts/cli.py#L4)) deliberately omits the web stack, and the routing fix exists only in the wrapper. `CLAUDE.md §Local Task-Tree CLI Development` teaches direct `uv run --script …/cli.py …`, so this is a documented, reachable path. Fix: catch the ImportError in `_run_dashboard` with a message pointing at the wrapper / `plan_dashboard.py`, or exec `plan_dashboard.py` via `uv run --script` subprocess.
>    → implemented: `_run_dashboard` catches the `ImportError` and prints an actionable message naming the missing module + pointing at the wrapper / `plan_dashboard.py`, then exits 1 ([cli.py:155](../../../../skills/task-tree/scripts/cli.py#L155)); verified live (`dashboard export` now prints the friendly error, no traceback) and pinned by `test_dashboard_missing_web_stack_reports_friendly_error`.
> 3. [MINOR] Only `hooks/task-hook` is pinned by a byte-identity test ([test_cli.py:519](../../../../skills/task-tree/scripts/test_cli.py#L519)); the committed [superRA/superra](../../../superra) currently matches `render_wrapper()` (verified by diff) but nothing pins it, and the wrapper copies committed into research repos have no refresh mechanism beyond manual `wrapper init`. Add the symmetric wrapper byte-identity test.
>    → implemented: added `test_committed_wrapper_matches_generator` pinning `superRA/superra` to `render_wrapper()` ([test_cli.py:547](../../../../skills/task-tree/scripts/test_cli.py#L547)).
> 4. [MINOR] The "one file test" speed-gate claim ([wrapper_resolver.py:41](../../../../skills/task-tree/scripts/wrapper_resolver.py#L41), repeated in this task's Results) is overstated: the legacy `skills/task-system` alias doubles every probe (up to ~6 file tests) and the Claude branch spawns a python3 heredoc per call. Correct the docstring or drop the claim.
>    → implemented: rewrote the speed-gate docstring to describe the real cost (up to two file tests per branch for the task-tree + task-system alias, a python3 heredoc for the Claude branch, one fixed-depth glob for Codex, first-match short-circuit) ([wrapper_resolver.py:41](../../../../skills/task-tree/scripts/wrapper_resolver.py#L41)); corrected the matching Results sentence.
