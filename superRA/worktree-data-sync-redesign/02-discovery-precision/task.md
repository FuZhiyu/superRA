...
**Verification:** `uv run --with pytest python -m pytest skills/worktree-data-sync/scripts/test_worktree_data_sync.py -q` (run outside the sandbox per dispatch note, since uv panics inside it) — `44 passed`. This includes the `TestSeedFastPath` suite from sibling task 01, which task 02 does not touch.
