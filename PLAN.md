...
>    → implemented: changed "Four" to "Five" in the `## Multi-agent review pattern` introductory sentence (`skills/writing/CLAUDE.md:45`)
>
...
>    → implemented: rewrote paragraph (c) to name manuscript-geography batching ("§3 polish" / "§4 polish") as the rejected alternative, state the load-bearing why (PR-friendly diffs, reviewable commit scopes, each sweep is one coherent concern), drop the example list, and cross-reference `long-form-review.md §Stage-2 task granularity` for the operative spec (`skills/writing/CLAUDE.md` paragraph (c) in `## Two-stage REVIEW.md → PLAN.md lifecycle`)
