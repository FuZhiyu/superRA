...
> 1. [MAJOR] `## Implementer / reviewer interaction walk-through` (lines 73–87) violates the repo CLAUDE.md DRY/Necessity gate ([BLOCKING] per dispatch). The section was *verification activity* in the design spec (§Implementer/reviewer interaction walk-through — "verifies the design works against the canonical … protocols without adapter prose"); once verified, retaining it as standing instruction is precisely the "design essay in skill body" anti-pattern. Concrete violations:
>    - Line 77 verbatim-restates line 19 (User feedback gate — "Main agent collects user decisions per finding, writes `**User feedback:**` into each Stage-1 task block, commits.")
>    - Line 79 restates lines 21–27 (Stage 1 → Stage 2 four-move rewrite) at lower fidelity.
>    - Line 83 ("Standard `subagent_type: superRA:reviewer` against the commit range. … REVISE round uses the standard `→ implemented:` annotation pattern. Cycle terminates at APPROVED.") is pure default reminder of canonical reviewer protocol owned by `agents/reviewer.md`.
>    - Line 85 ("Standard implementer runs build per `refactor-and-compile.md §Compile` … standard reviewer APPROVES on green") is default reminder of canonical implementer behavior.
>    - Line 81's parenthetical "(header read covered by `implementer.md §Before You Start`)" is the same default reminder pattern.
>    - Only line 87 (Closeout — delete/archive PLAN.md) carries a non-default disposition that is not already in §Doc convention §Workflow Status Stage 2.
>    Fix: delete the whole §Implementer / reviewer interaction walk-through section. The Stage-2 §Workflow Status "Finished" box body already says "PLAN.md archived or deleted per the lifecycle ladder," so no surviving content needs a new home. (Step 2 self-check claimed only one wrapper sentence was removed; this is the larger violation Step 2 missed.) Note: the design spec told the implementer to include this section — that does not override the dispatch's [BLOCKING] DRY/Necessity gate; this finding is the correct call against the spec.
>    → implemented: deleted entire `## Implementer / reviewer interaction walk-through` section (lines 73–87); Closeout line not needed elsewhere since Stage-2 §Workflow Status "Finished" already covers it (`long-form-review.md` now ends at line 83)
> 2. [MAJOR] Line 29 (Stage 2 description) contains a "here is what you will receive" description plus a default reminder: "agents read this file exactly as they read any other PLAN.md. Each Stage-2 task uses the standard Task Block Anatomy from `plan-anatomy.md`: `Depends on`, `Review status`, Steps with the polish work, review-notes blockquote on REVISE rounds." The pointer to `plan-anatomy.md §Task Block Anatomy` is sufficient; listing the fields it owns is DRY violation, and "agents read this file exactly as they read any other PLAN.md" is a default reminder. Fix: collapse to "Stage 2 task blocks follow the standard `plan-anatomy.md §Task Block Anatomy`. `Integration status:` is omitted; review-driven polish work does not flow through `integration-workflow`." (skills/writing/references/long-form-review.md:29)
>    → implemented: collapsed Stage 2 description to pointer + omission note per fix clause (`long-form-review.md:33`)
> 3. [MINOR] Line 15 ends with "this ID is used for cross-referencing throughout both stages." This is purpose narration, not action — and the purpose becomes obvious when `**Sources:** F2, F5, F9` appears in line 25. Without this clause an agent's behavior is unchanged. Fix: delete the clause; keep the F-ID assignment rule. (skills/writing/references/long-form-review.md:15)
>    → implemented: deleted "this ID is used for cross-referencing throughout both stages" clause (`long-form-review.md:15`)
> 4. [MINOR] Line 17 second sentence ("Granularity is per-finding because authorial decisions and per-item exceptions need explicit authorization; per-task tier directives bolt exceptions on awkwardly") is design rationale that Task 3 explicitly puts in `writing/CLAUDE.md` entry (b). Repo CLAUDE.md: "Skip design essays in skill bodies … keep rationale in contributor docs." Fix: drop the rationale clause; keep only the rule and the `**User feedback:**` format line. (skills/writing/references/long-form-review.md:17)
>    → implemented: dropped rationale sentence from line 17; kept only the rule + format block (`long-form-review.md:17`)
> 5. [MINOR] Line 17 specifies the `**User feedback:**` field and the per-line format `F<n>: accept | defer | reject [— optional reason]`, but a fresh main agent has no example structure (single-line vs multi-line list; date-tagged vs plain header). Design spec anatomy sketch lines 105–109 showed `**User feedback (YYYY-MM-DD):**` followed by one `- F<n>: ...` bullet per finding. Fix: either inline the example header + a single bullet (one line), or accept the gap as agent-baseline competence and note the decision. (skills/writing/references/long-form-review.md:17)
>    → implemented: inlined example block with date-tagged header + bullet format from design spec anatomy sketch (`long-form-review.md:18–21`)
> 6. [MINOR] Line 62 (Review-time indices) lost the lifecycle-ladder coupling the prior version carried. The new wording says only "in the `## Project Conventions` header" without naming which doc when riding a workflow (where no REVIEW.md exists, per line 31). The prior text routed via `SKILL.md §Project Conventions in the handoff doc / CLAUDE.md`. A fresh orchestrator could put indices in the wrong place when riding a workflow. Fix: restore a one-clause pointer, e.g., "in the `## Project Conventions` header of whichever doc the lifecycle ladder in `SKILL.md §Project Conventions in the handoff doc / CLAUDE.md` resolves to (REVIEW.md standalone, the workflow's PLAN.md when riding one)." (skills/writing/references/long-form-review.md:62)
>    → implemented: restored lifecycle-ladder pointer with standalone-vs-riding qualifier (`long-form-review.md:66`)
> 7. [MINOR] Line 95 (Final summary block) says "at the top of REVIEW.md" without the standalone-vs-riding qualifier. When riding a workflow there is no REVIEW.md (line 31), so the summary lives at the top of the workflow PLAN.md instead. Fix: generalize to "the active Stage-1 handoff doc (REVIEW.md standalone, the workflow's PLAN.md when riding one)." (skills/writing/references/long-form-review.md:95)
>    → implemented: generalized to "the active Stage-1 handoff doc (REVIEW.md standalone, the workflow's PLAN.md when riding one)" (`long-form-review.md:83`)
