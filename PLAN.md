...
> 1. [MAJOR] `tests/claude-code/test-adaptive-planning-principles.sh:45-51` and `tests/claude-code/test-doc-before-report-task-shape.sh:45-51` are still too brittle to support the documented fast suite on the current merged tree. Fresh evidence on this tree: one full run of `tests/claude-code/./run-skill-tests.sh` failed with `Passed: 3`, `Failed: 1`, `STATUS: FAILED` because `test-adaptive-planning-principles.sh` rejected a semantically-correct `Steps` line that said the planner should write only enough guidance and use a short note or skeleton when the route is unknown, while a direct rerun of `./run-skill-tests.sh --test test-adaptive-planning-principles.sh` passed on unchanged code. A second full run of the same suite then failed with `Passed: 3`, `Failed: 1`, `STATUS: FAILED` because `test-doc-before-report-task-shape.sh` rejected a semantically-correct `PLAN.md` line that documented the durable handoff action but did not include one of the narrow `task-shape|split|recommendation|concern|boundary|change shape|restructure` keywords, and a direct rerun of `./run-skill-tests.sh --test test-doc-before-report-task-shape.sh` also passed on unchanged code. Minimal allowed branch delta: keep the fast-suite smoke coverage, but relax both assertion surfaces so they accept equivalent current superRA phrasing for open-route planning and durable `PLAN.md` task-shape recording rather than one narrow wording family. Stale branch-side content that must not survive: line-level regexes in the supported fast suite that turn semantically-correct answers into intermittent failures on unchanged code.
...
>    → implemented: broadened the open-route planning and durable `PLAN.md` task-shape regexes to accept equivalent current superRA wording like “write only enough guidance,” “short note,” “skeleton,” and planning-change flags without weakening the underlying semantic checks.
