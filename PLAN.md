...
> 1. [MAJOR] `CLAUDE.md:72`, `skills/handoff-doc/references/plan-anatomy.md:67`, and `skills/handoff-doc/references/plan-anatomy.md:122` still do not land Task 1 on the current `origin/main` contract. `origin/main` commits `b7f0f02`, `cc192a3`, `4036130`, and `24f157e` kept the direct-mode generated-artifact rule in `CLAUDE.md` and replaced the older Phase B `## Integration Intent` model with `## Upstream Intent` in `plan-anatomy.md`; the surviving branch diff still omits the direct-mode ownership rule and still defines `## Integration Intent` as the Phase B section. Preserve Task 1's objective-first task-boundary semantics, but port them onto those newer concern-owner surfaces. Minimal allowed branch delta: `CLAUDE.md` keeps the direct-mode generated-artifact rule while Task 1 adds the objective-first principle, and `plan-anatomy.md` expresses the same boundary semantics under the `## Upstream Intent` section name, placement, ownership, and lifecycle. Stale branch-side content that must not survive: the `plan-anatomy.md` `## Integration Intent` section / field-note language and any `CLAUDE.md` Codex-design prose that omits the direct-mode generated-artifact rule.
>    → implemented: preserved the upstream manual-mirror contributor rule by leaving `CLAUDE.md` on its `origin/main` wording and rewrote `plan-anatomy.md` onto the `## Upstream Intent` contract while keeping the objective-first task-boundary semantics (`skills/handoff-doc/references/plan-anatomy.md`)
...
> **Review notes (present only during active REVISE rounds):**
> 1. [MAJOR] `agents/reviewer.md:111`, `.codex/agents/superra_reviewer.toml:112`, and `skills/using-superRA/references/main-agent.md:86` still sit on the pre-upstream direct-mode / Phase B contract, and `skills/using-superRA/references/direct-mode-implementer.md` plus `skills/using-superRA/references/direct-mode-reviewer.md` are missing from the branch. `origin/main` commits `b7f0f02`, `cc192a3`, and `24f157e` moved direct-mode execution onto those skill-surface role refs, kept `main-agent.md` loading them, and updated reviewer-facing Phase B language to `## Upstream Intent` plus the surviving-diff pruning sweep. Preserve Task 2's objective-first role semantics, but land them on those newer owners. Minimal allowed branch delta: the direct-mode role refs exist and tell the same objective-first story as the canonical `agents/*.md` files, `main-agent.md` points direct mode at them, and reviewer-facing surfaces use `## Upstream Intent` with the pruning-sweep re-review rule. Stale branch-side content that must not survive: `main-agent.md` telling direct mode to read raw `agents/*.md`, any reviewer-facing `## Integration Intent` language, and the deletion of the direct-mode role-ref files.
>    → implemented: merged the canonical agents onto the objective-first scope contract plus the newer `## Upstream Intent` reviewer protocol, then regenerated `.codex/agents/*.toml`; the direct-mode mirror files were intentionally left untouched per this dispatch's out-of-scope rule (`agents/implementer.md`, `agents/reviewer.md`, `.codex/agents/superra_implementer.toml`, `.codex/agents/superra_reviewer.toml`)
...
>    → implemented: rewrote the Phase B intro and the canonical handoff/reviewer surfaces onto the frozen-merge-base `## Upstream Intent` loop and surviving-diff confirmation while preserving the branch's objective-first boundary-change escalation (`skills/integration-workflow/SKILL.md`, `skills/handoff-doc/references/plan-anatomy.md`, `agents/reviewer.md`)
...
- [x] **Step 2: Wire the focused test into the runner and shared harness.** Added the new test to `tests/claude-code/run-skill-tests.sh`, updated the runner help text so the default fast suite advertises only current green superRA coverage, documented the supported repo-local path in `tests/claude-code/README.md`, kept the stale `subagent-driven-development` scripts as legacy manual-only references, and made `tests/claude-code/test-helpers.sh` provide a portable `run_with_timeout` wrapper plus argv-safe `run_claude` execution so the suite runs on this macOS environment.
