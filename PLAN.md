...
>
> 1. **MAJOR — README negative-control recipe documents the very approach PLAN.md and RESULTS.md identify as broken.** [tests/README.md:116-148](skills/research-project-setup/tests/README.md#L116-L148) tells a future reader: "scaffold `$PROJ/NegCtrl`, strip `additionalDirectories=[]` and the absolute `writable_roots` lines, then `run_with_outer_timeout bash skills/research-project-setup/tests/cases/test_a_sandbox.sh claude`." But [tests/cases/test_a_sandbox.sh:28-35](skills/research-project-setup/tests/cases/test_a_sandbox.sh#L28-L35) unconditionally does `PROJ=$(mktemp -d "$HOME/rps-tests/A-proj-XXXX")` and `scaffold_project "$PROJ_DIR" "$SHARE"` — it scaffolds a FRESH `A-proj-XXXX` with valid registration and never touches `$PROJ/NegCtrl`. The case script ignores the broken project entirely. PLAN.md Step 8 ([PLAN.md:943](PLAN.md#L943)) and RESULTS.md ([RESULTS.md:326](RESULTS.md#L326)) both explicitly call this out: "the `cases/test_a_sandbox.sh` script scaffolds its OWN fresh `A-*` project with a valid registration, so it can't be re-targeted at a pre-broken project just by argument" — yet the README still recommends exactly that re-target invocation. Future maintainers following the README will get a vacuously-passing run and conclude the assertion is load-bearing when in fact the case script silently ran against a clean project. Fix: rewrite the README's recipe to match what was actually executed (replay Test A's assertions directly against `$PROJ/NegCtrl` rather than invoking `test_a_sandbox.sh`), OR add a dedicated `cases/test_a_negative_control.sh` that takes the pre-broken project as input and replays Test A's prompt + assertions against it (preferred — makes the negative control re-runnable as part of the suite). Either way, the README must not document a recipe that PLAN.md and RESULTS.md describe as broken.
>    → implemented: added [`cases/test_a_negative_control.sh`](skills/research-project-setup/tests/cases/test_a_negative_control.sh) — self-contained script that scaffolds → strips registration → replays Test A's prompt+assertions against the broken project; emits `"expected FAIL got FAIL ✓"` per CLI and exits 1 on UNEXPECTED PASS. README §Negative-control regression check rewritten to recommend `bash .../cases/test_a_negative_control.sh both` and explain why a dedicated script is required ([tests/README.md:111-141](skills/research-project-setup/tests/README.md#L111-L141)). Live re-run on both CLIs: claude wall=10s denial=yes, codex wall=20s denial=no (file absent both) — recorded in RESULTS.md Task 8 §Negative-control.
>
> 2. **MINOR — `with_timeout` perl-alarm fallback has no SIGKILL grace period.** [tests/lib/common.sh:158-167](skills/research-project-setup/tests/lib/common.sh#L158-L167) uses `perl -e 'use POSIX; alarm shift; exec @ARGV'` when neither `gtimeout` nor `timeout` exists. PLAN.md ([PLAN.md:849](PLAN.md#L849)) explicitly required: "The `with_timeout` helper must SIGKILL after a grace period if SIGTERM is ignored — `perl alarm` fallback alone has been known to leave child processes running on macOS; prefer `gtimeout --kill-after=10 <secs>` (install via `brew install coreutils` if missing)." Verified empirically: a child process that does `$SIG{ALRM}='IGNORE'` survives the alarm and runs to completion (tested locally — alarm-ignoring child still running after 4s under a 1s alarm). On this machine `gtimeout` is absent and was not installed. The 8/8 PASS run and the negative-control's 8s/10s wall times suggest neither `claude` nor `codex` ignored SIGALRM in practice, so this is not a blocker for the recorded results. But the invariant the PLAN called out as load-bearing for the negative-control safety net ("never hang beyond ~3 minutes") is not actually implemented. Fix at minimum one of: (a) `brew install coreutils` and re-run so the harness exercises the `gtimeout --kill-after=10` branch end-to-end; (b) make the perl-alarm branch a `kill -TERM` from a backgrounded watchdog with a `kill -KILL` follow-up; or (c) downgrade the PLAN's MUST to a SHOULD and note that the perl-alarm fallback is best-effort.
>    → implemented: chose path (b) — replaced the perl-alarm branch in [tests/lib/common.sh:155-184](skills/research-project-setup/tests/lib/common.sh#L155-L184) with a setpgid + process-group-kill pure-bash watchdog: `( "$@" ) &` is run under `set -m` (its own process group); a backgrounded watcher sends `kill -TERM -$pid` at `<secs>` and `kill -KILL -$pid` 10s later. The watcher's stdout is redirected to `/dev/null` and the watcher's own group is killed after the child exits so its inner `sleep` doesn't hold the parent's command-substitution pipe open. Verified locally: child that ignores SIGTERM gets SIGKILL'd at ~12s elapsed (5s alarm + 10s grace), subprocess trees are reaped, no orphan `sleep` processes left behind, command-substitution captures still return promptly. Re-ran Test A live afterwards (claude=9s, codex=12s — back to pre-fix timings, confirming the watcher cleanup also fixed an incidental hang).
>
> 3. **MINOR — README negative-control recipe inherits the same fallback weakness.** [tests/README.md:135-139](skills/research-project-setup/tests/README.md#L135-L139) defines `run_with_outer_timeout` with the same `gtimeout / timeout / perl alarm` chain. If finding #2 is fixed by installing `gtimeout`, this becomes moot; if fixed in `with_timeout` only, the README recipe should be updated to point at a shared helper rather than re-inlining the weak fallback. Take whichever path resolves #2.
>    → implemented: the new `cases/test_a_negative_control.sh` consumes `with_timeout` from `lib/common.sh` directly (via `run_claude`/`run_codex`) rather than re-inlining a `run_with_outer_timeout` helper, so finding #2's hardened `with_timeout` is the single source of truth. README §Negative-control regression check no longer carries the inline fallback chain.
