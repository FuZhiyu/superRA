...
> **Review notes (Bundle B first pass, 2026-04-17):**
>
> 1. **[MINOR]** `skills/execution-workflow/SKILL.md:97` — sub-step `2a.` breaks the numbered-list rendering (`2a.` is not a valid ordered-list marker; markdown renderers will restart or flatten the list). Content is correct and agents will read it regardless, but if `## Workflow Status` + numbered steps are ever rendered, item "2a" will not display as a nested/sub-item. Consider `2. ... ` → relabel as plain bullet indented under 2, or keep 2a as a stand-alone unnumbered paragraph. Same markdown-rendering concern applies to the new full-drift-suite paragraph at `integration-workflow/SKILL.md:70` inserted between numbered items 2 and 3 (breaks the ordered list). Advisory — not blocking.
>    → implemented: converted `2a.` to an indented sub-bullet under item 2 (`execution-workflow/SKILL.md:97`); folded the full-drift-suite paragraph into a blockquote nested under numbered item 2 in `integration-workflow/SKILL.md:70` to preserve list continuity.
> 2. **[MAJOR]** Content parity sweep in Task 5 validate was scoped only to `skills/`. Stale pointers to `session-bootstrap.md` remain in `README.md:173` (live skill-inventory table) and `tests/structural-invariants.sh` (lines 403 comment + 440–444 existence assertion). See Task 5 blockquote for details — flagging here because Task 3's commit scope landed alongside the deletions. No fix required in Task 3 itself; the fix lives under Task 5.
>    → implemented: fixes applied in Task 5 (see Task 5 review notes annotations).
...
> **Review notes (Bundle B first pass, 2026-04-17):**
>
> 1. **[CRITICAL]** `tests/structural-invariants.sh` asserts existence of the deleted file. Lines 440–444 contain:
>    ```
...
>    fi
>    ```
>    Running `bash tests/structural-invariants.sh` on the current commit produces `FAIL missing: skills/using-superRA/references/session-bootstrap.md` → overall `FAILED: 1 invariant(s) failed`. Task 5's validate sweep (`grep -rn ... skills/`) missed this because the grep was scoped to `skills/` only, not `tests/`. Fix: update the existence check to assert `skills/using-superRA/references/main-agent.md` instead, and update the stale comment on line 403 (still says "references/session-bootstrap.md is the main-agent-only bootstrap reference"). Since block 20 is the test block that `using-superRA` integrity hangs on, getting the test green again is a hard gate for merge.
>    → implemented: updated existence check to assert `main-agent.md` and updated stale comment on line 403 (`tests/structural-invariants.sh:440-444, :403`). Suite now returns 0 FAIL.
> 2. **[MAJOR]** `README.md:173` — live skill-inventory table describing `using-superRA` still reads: "Main-agent-only cross-session detection lives in `references/session-bootstrap.md`." This is the user-facing public skill doc. Update to point at `references/main-agent.md` (parallel to the `CATEGORIES.md:49` edit already made in this commit).
>    → implemented: updated `README.md:173` to "Main-agent-only cross-session detection and autonomy contract live in `references/main-agent.md`."
> 3. **[MINOR]** Content parity of main-agent.md vs the two deleted files: the opening framing paragraph of `main-agent-autonomy.md` ("This contract generalizes what used to live in `execution-workflow` §Autonomy and Stop Points. It applies to every workflow phase — planning, execution, integration, merge, semantic-merge — not just execution. Workflow skills carry stage-specific stop points...") is not carried forward into `main-agent.md`. The "applies to every phase" framing is echoed in `execution-workflow`'s own §Autonomy ("it applies to every workflow phase, not just execution"), so not strictly lost; however, the main-agent reference is the canonical home and the framing paragraph helps a new main agent understand the contract's scope. Consider inserting a one-line note at the top of §The Three Pause Classes (or in the file's opening note) that this contract applies across every workflow phase, not just execution. Advisory.
>    → implemented: added a two-sentence framing paragraph at the top of `## The Three Pause Classes` in `main-agent.md` stating the contract applies across every workflow phase (`skills/using-superRA/references/main-agent.md:54-55`).
