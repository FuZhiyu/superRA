...
> **Review notes:**
> 1. [STANDARD] `RESULTS.md` Task 1 §Validation line 37 claim "Stale Stage names (`implementation review`, `drift test creation`, `integration review` (unhyphenated), `merge proposer`, `documentation finalization`) in live prose — zero" is inaccurate. Live prose occurrences remain in: `README.md:60,65,68,71,74` (top-level architectural diagram literally contains `Stage: implementation review`, `Stage: drift test creation / drift test review`, `Stage: refactoring / integration review`, `Stage: doc writer / doc reviewer`, `Stage: merge proposer / merge review` — five Stage-name strings in the user-facing diagram); `skills/agent-orchestration/SKILL.md:127` ("`execution-workflow` (implementation review), `integration-workflow` (drift test review, integration review, doc review)"); `agents/reviewer.md:6,109` (frontmatter + "Regardless of stage (implementation review, drift test review, integration review, merge review, ad-hoc)"); `skills/execution-workflow/SKILL.md:69,88,89,91` (four graphviz flowchart labels `Dispatch reviewer (implementation review)`); `skills/report-in-markdown/SKILL.md:33` (table row `execution-workflow reviewer (implementation review)`); `skills/refactor-and-integrate/SKILL.md:17,80,82` (three prose occurrences enumerating `drift test creation, refactoring, and merge proposal stages` / `drift test review, integration review, and merge review stages` / `merge proposer (\`merge-quality.md\`) and merge reviewer`); `skills/econ-data-analysis/SKILL.md:327` ("### Refactor integrity (applies at the `refactoring` and `integration review` stages)"); `skills/econ-data-analysis/references/integration.md:3,5` (twice: "`refactoring` and `integration review` stages"); plus `tests/structural-invariants.sh:431,433,435` testing for the old `drift test creation` row name. Most of these files are out of Task 1's Files-affected list, so fixing them is deferrable to Task 16 Step 6 (which PLAN.md line 453 explicitly names these strings for the final sweep). The STANDARD issue is the RESULTS.md validation claim overstating what was verified. Fix by narrowing RESULTS.md line 37 to describe exactly what Task 1 verified in scope — e.g., "Stale Stage names as live `Stage:` field emissions in dispatch blocks across `skills/` + `agents/` — zero (every `Stage:` value matches a manifest row). Stale stage-name _prose_ references remain in files outside Task 1's Files-affected list (README.md architectural diagram; role-description enumerations in `agent-orchestration`, `reviewer.md`, `execution-workflow` graphviz labels, `report-in-markdown`, `refactor-and-integrate`, `econ-data-analysis`, `econ-data-analysis/references/integration.md`; test-name in `tests/structural-invariants.sh`) — deferred to Task 16 Step 6 per PLAN.md." The scope limit is legitimate; overstating the validation is what needs fixing. The doc is the record — the dev-log claim must match what was actually verified.
>    → implemented: narrowed RESULTS.md Task 1 §Validation line 37 — claim now scoped to live `Stage:` field emissions in dispatch blocks (the thing Task 1 actually verified), with prose references explicitly enumerated and deferred to Task 16 Step 6 (`RESULTS.md:37`).
