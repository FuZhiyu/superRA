...
- [ ] **Step 1:** `grep -rn "## Findings\|Sources: F\|F-ID\|per-aspect task block\|Stage-1 task block\|Stage-1 per-aspect\|per-aspect block\|temporary task block" skills/ --include="*.md"`. Every surviving mention should either be in the new contributor-notes (d) explaining the rejected alternative, or in something orthogonal. Flag and fix stragglers. Known hit to fix: `skills/writing/CLAUDE.md` ~line 48 (`## Multi-agent review pattern` decision item on the rename rule) still says "Stage 1 per-aspect blocks live in the workflow's `PLAN.md` as temporary task blocks" — under the new design, when the review rides a workflow PLAN.md, the Stage-1 `## Findings` section is created inside that PLAN.md and the Stage-2 rewrite rebuilds task blocks inline; rewrite the line accordingly.
