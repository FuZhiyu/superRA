...
>
> 1. **MINOR — paragraph 3 ambiguity leak across the bundle.** `PLAN.md` Step 1 frames paragraph 3 as testing "audience-model elicitation" on an "ambiguous venue" short sentence (`This paper investigates the AE/EM sovereign-spread puzzle.`). Bundling places that sentence in the same fragment as paragraph 1 (IMF WEO + AE/EM + coreAE + Zenodo) and paragraph 2 (country-clustered SEs + robustness transition), which together fix the venue unambiguously. The disclosure in both `PLAN.md` Task 2 Input and `RESULTS.md §Task 2` ("each test concern lives in its own intent-tagged paragraph so bundling does not blur which signal fired where") is true for detection vs over-firing but understates the ambiguity-leak risk: audience modeling is document-scoped, not paragraph-scoped, so paragraph 3's ambiguity is dissolved by its neighbors. The record then correctly notes the venue was *inferred from the bundle* (`RESULTS.md §Task 2 → Audience model → Venue`), which is what the bundling produces — not what the ambiguity-elicitation test was designed to probe. The conclusion "audience model built explicitly before editing" still holds (upstream modeling is what the plan's Expected Results actually names — "elicit *or assume*"), but the elicitation-on-ambiguity property the original three-fragment design would have isolated is not in fact demonstrated. Fix: add one sentence to `RESULTS.md §Task 2 → Audience model` acknowledging that paragraph 3's ambiguity is dissolved by bundle context and that the recorded outcome demonstrates *assumed* modeling from cross-paragraph signals rather than *elicited* modeling from an isolated ambiguous sentence. No skill-file edit needed; this is a record-honesty fix only.
>    → implemented: added new bullet "Ambiguity-elicitation caveat" to RESULTS.md §Task 2 → Audience model. The bullet names the bundle-dissolution effect, clarifies that the recorded outcome shows assumed modeling not elicited modeling, and ties back to the plan's "elicit *or assume*" Expected Result so the conclusion still parses.
>
> 2. **MINOR — paragraph 3 intent-tag / authorial-surface inconsistency.** `PLAN.md` Step 1 describes paragraph 3's intent as "audience-model elicitation," but `RESULTS.md §Task 2 → authorial surface` reports that the surfaced `authorial` item read *"Intent comment says state the contribution; the sentence states the topic instead."* Either the constructed fragment carried `% intent: state the contribution` (in which case paragraph 3 was carrying a second design purpose not surfaced in PLAN Step 1) or the record is paraphrasing a different intent tag than the PLAN names. Fix: in `RESULTS.md §Task 2 → authorial surface → Paragraph 3 topic-vs-contribution gap`, add one parenthetical clarifying the fragment's actual intent comment vs the PLAN's test-purpose label, so a future reader can reconstruct what the fragment said. No skill-file edit needed.
>    → implemented: rewrote the paragraph-3 bullet in RESULTS.md §Task 2 → authorial surface to name both purposes — PLAN Step 1 labels paragraph 3 the audience-model-elicitation test case; the fragment's actual `% intent: state the contribution.` comment names the paragraph's in-document job — and clarifies that the agent's authorial finding was against the in-fragment intent comment.
>
> 3. **ADVISORY — citation granularity in RESULTS.md.** The dispatch asked the reviewer to verify with file/line citations; my verification resolved every cited section pointer (`style.md §Audience`, `polish.md §Triage`, `SKILL.md §Write to the reader, not the conversation`) against the committed skill files and confirmed the claimed replacements match the section text verbatim (style.md L24–L27 for the four replacement patterns; L31–L33 for the three §Do NOT exceptions; SKILL.md L27–L30 for the two-question audience model). Section-name pointers are the project's standing citation convention, so this is not a finding — recording here only because the dispatch named it.
>
> Conclusion bearing on "no skill-file edits needed": findings 1 and 2 do not contradict the conclusion. Positive detection (4/4 families with replacements matching style.md §Audience verbatim) and no-over-firing (3/3 §Do NOT exceptions respected, plus the IMF-WEO bonus skip consistent with the style.md §Audience replacement example) are demonstrated cleanly; the upstream-audience-model discipline named in `SKILL.md §Write to the reader, not the conversation` is exercised (with the caveat in finding 1 that elicitation-on-ambiguity is not isolated). The skill-file wording committed in `dfe87bc` is supported as correct by the record; the REVISE here is to clean up two record-honesty items in `RESULTS.md`, not to re-open the skill files.
