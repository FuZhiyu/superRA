...
Target: [`docs/plans/2026-04-17-parallel-implementer-worktree-isolation-results.md`](docs/plans/2026-04-17-parallel-implementer-worktree-isolation-results.md) at HEAD `6e1ce9d`. Fact-check walked against merged state. Every spot-check confirmed accurate: `worktree-harness-fallback.md` = 79 lines; merge-guard regex at `hooks/merge-guard:41` matches doc text; `test_worktree_data_sync.py` = 30 passed; §Concurrent Writers at `agent-orchestration/SKILL.md:73`; §Dispatch Templates at line 100 with `Worktree:` at line 113; two subsections §Ownership split / §Worktree lifecycle confirmed; narrowed `worktree-data-sync` section outline matches actual headings; `semantic-merge/SKILL.md:23` Exception paragraph confirmed; merge-guard 4-input regression table reproduces live. Reader-facing structure (Problem / Resolution / Verification / Limitations / Reproducibility) stands on its own without task-order context. Known dogfood limitations preserved with reproduction detail. Three MINOR items remain:

> 1. [MINOR] Matured doc line 47 overstates the implementer.md restructure: claims "§Handoff → Update the Docs and Commit — split into (A) shared-worktree path with the existing discipline and (B) dedicated-worktree path on `parallel/<branch>/<slug>`. Atomic-commit block stays shared across both paths." Reality at `agents/implementer.md`: there is no formal (A)/(B) subsection split — line 159 is a single conditional paragraph handling the dedicated-worktree case embedded in the existing §Update the Docs and Commit section, and the "path A / path B" labels appear only in §Report Format at line 191. **Fix:** reword to describe the actual mechanism, e.g. "§Update the Docs and Commit gains a conditional paragraph (`agents/implementer.md:159`) for the dedicated-worktree path (commit on the provisioned `parallel/<branch>/<slug>` branch; do not merge / rebase / push / clean up); §Report Format (line 191) gains a new `Worktree return (path B only)` field reporting branch name and HEAD SHA."
>
> 2. [MINOR] Matured doc line 78 §Limitations: "Root cause is **likely** a JSON-escaping or command-string difference in how the harness serializes the `Bash` invocation to the hook's stdin." "Likely" is on the prohibited-speculation list per `report-in-markdown/references/final-form.md` §Speculation. The hypothesis is legitimate content for a known-issue entry but should be framed factually. **Fix:** "Root cause not isolated. One hypothesis is a JSON-escaping or command-string difference in how the harness serializes the `Bash` invocation to the hook's stdin; a follow-up would harden the hook's input parsing and add an integration test that invokes the hook through a Bash-tool-equivalent JSON wrapper."
>
> 3. [MINOR] Frontmatter at lines 1–10: `git_commit: 9a1b097` and `git_message: "plan: flip Integration status + Drift tests / Refactored boxes"` are stale relative to the maturation commits. Per `baseline-io.md` §Frontmatter, `git_commit` should reflect the integration commit at seal time (Commit 4 `6e1ce9d`, `git mv` of the matured doc into `docs/plans/`). Additionally, the template lists `timestamp` and `session_id` as standard fields; both are absent. **Fix:** update `git_commit` to `6e1ce9d43f6b40bd3873a04db96511bb57f00728` and `git_message` to the actual relocation-commit subject (`"results: relocate to docs/plans/2026-04-17-parallel-implementer-worktree-isolation-results.md"`); add `timestamp` and `session_id` per the template (or explicitly note the project convention omits them). Note: the §Reproducibility pointer at line 84 citing `9a1b097` as the code-state commit is defensible (that was the last code-shaping commit before maturation) — keep it there; this fix is frontmatter-only.
