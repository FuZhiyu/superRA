...
> **Review notes:**
> 1. [MAJOR] Cross-task coupling: SKILL.md line 9 ("Every agent loads this skill — every agent writes markdown.") and the README.md / CATEGORIES.md row rewrites ("Loaded by every agent" / "loaded by every agent") all assert as fact something that is only true after Task 2 lands and changes the Skill-Load Manifest. As of this commit, the manifest still scopes `report-in-markdown` to `Stage: documentation`, so these three assertions are not yet true. ([skills/report-in-markdown/SKILL.md:9](skills/report-in-markdown/SKILL.md#L9), [README.md:82](README.md#L82), [skills/CATEGORIES.md:44](skills/CATEGORIES.md#L44)) If the orchestrator intends to land Tasks 1–2 together as one merge, this is acceptable but should be called out explicitly in the task block (e.g., "must commit in lockstep with Task 2"). If Task 1 can land independently, soften to forward-looking phrasing here ("intended to be loaded by every agent"; manifest update lands in Task 2) and tighten in Task 2 once the manifest change ships. Either fix is fine — just don't ship a body whose first sentence is false.
>    → implemented: softened SKILL.md opener to "The markdown style guide for any agent writing markdown" (no claim about manifest state); README.md row reads "Markdown style guide for any agent writing markdown" (dropped "Loaded by every agent"); CATEGORIES.md row dropped the trailing "Loaded by every agent." sentence. Task 2's manifest update will tighten to factual "always loaded" in the manifest itself. ([skills/report-in-markdown/SKILL.md:9](skills/report-in-markdown/SKILL.md#L9), [README.md:82](README.md#L82), [skills/CATEGORIES.md:44](skills/CATEGORIES.md#L44))
>
> 2. [MINOR] Inconsistency between SKILL.md citation-form table and rich-content.md "Correct" examples. SKILL.md table rows show plain link text (`[file.py:42](file.py#L42)`); rich-content.md §File references "Correct" rows wrap link text in backticks (`` [`code/BOP/clean_data.py:42`](../code/BOP/clean_data.py#L42) ``). Both render as clickable links, but they produce visibly different styling (monospace vs not) and the two files are now teaching slightly different canonical forms. The PLAN.md Task 1 Step 2 spec also gave the plain form (`[code/BOP/clean_data.py:42](...)` — no backticks in link text). Pick one canonical form and use it in both files. ([skills/report-in-markdown/SKILL.md:19-21](skills/report-in-markdown/SKILL.md#L19-L21), [skills/report-in-markdown/references/rich-content.md:106-108](skills/report-in-markdown/references/rich-content.md#L106-L108))
>    → implemented: standardized on plain link text (no backticks) in `rich-content.md` §File references "Correct" rows. Both files now use the same canonical form: `[file.py:42](file.py#L42)`. ([skills/report-in-markdown/references/rich-content.md:106-108](skills/report-in-markdown/references/rich-content.md#L106-L108))
>
> 3. [MINOR] DRY/Necessity nit on SKILL.md line 11 ("Always-applicable rules below. Details for figures / math / tables / Stage 2 maturation live in references; load only the reference your output needs."). The §Load map immediately below already enumerates which references each caller loads, and the "load only what you need" instruction is the default behavior any agent reading a load map would apply. Per the contributor gate's Necessity test ("does removing this line change what the agent would *do*?"), this line shapes orientation, not action — candidate for deletion. Borderline; keep if the orchestrator wants the orientation framing, but it is exactly the kind of "here is what you will see" wrapper the gate flags.
>    → implemented: deleted the orientation paragraph and folded its single load-bearing hint ("load on demand per the load map") into the new opener as a clause. ([skills/report-in-markdown/SKILL.md:9](skills/report-in-markdown/SKILL.md#L9))
>
> 4. [MINOR] RESULTS.md has a duplicate Task 2 heading ([RESULTS.md:25-31](RESULTS.md#L25-L31)) — "## Task 2: Make `report-in-markdown` always-loaded in the Skill-Load Manifest" appears twice in a row, both with "Status: Not started". Fix at the same time as the rest of this task's polish.
>    → implemented: removed the duplicate heading. ([RESULTS.md:25](RESULTS.md#L25))
