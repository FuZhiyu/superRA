...
**Incoming intent:** Upstream PR #27 (`d861089 [codex] add Codex plugin hooks`) ships the Codex lifecycle hook bundle — `hooks/{ask-user-question-logger,codex-plan-stop,exit-plan-mode,hooks-codex.json,run-hook.cmd}` plus `tests/hooks/test-codex-{hooks,e2e-cli}.sh`, with companion Codex-instruction and parallel-worktree placement guidance under `skills/`. The PR squashes the original hook scaffolding plus several post-review fixes (`fix(codex-hooks): harden plugin hook launcher`, `Keep ephemeral agent worktrees under tmp`, `Fix Codex plan Stop hook packaging`) that softened the hook reminder language from imperative to advisory, added `${PLUGIN_ROOT}`/`${CLAUDE_PLUGIN_ROOT}` fallback in `hooks-codex.json`, added a PATH-sparse `bash` discovery branch in `run-hook.cmd`, added a `stop_hook_active` guard plus broader proposed-plan marker matching in `codex-plan-stop`, and grew `test-codex-hooks.sh` from 7 to 15 cases.
**Resolution thesis:** This branch had its own older snapshot of the same hooks work (commits `9eb21be`..`6e67655`, scoped under the Codex-hooks design but predating the upstream review polish) plus the unrelated `research-project-setup/` skill. The hook infrastructure files therefore had add/add and content conflicts where upstream is a strict superset; take upstream throughout for `hooks/`, `tests/hooks/`, `.codex-plugin/plugin.json`, and `docs/plans/2026-05-21-codex-hooks-*.md`. The branch's `research-project-setup` skill and template are entirely disjoint and survive unchanged.

### Sync Clusters

> **Sync cluster `codex-hooks-supersede` (2026-05-21):** commits `d861089`; paths `hooks/{ask-user-question-logger,codex-plan-stop,exit-plan-mode,hooks-codex.json,run-hook.cmd}`, `tests/hooks/test-codex-hooks.sh`, `.codex-plugin/plugin.json`, `docs/plans/2026-05-21-codex-hooks-*.md`; affects Tasks: none (research-project-setup tasks are disjoint).
> **Incoming intent:** Land the post-review polish of the Codex hook bundle — advisory rather than imperative reminder text, hardened launcher with `${CLAUDE_PLUGIN_ROOT}` fallback, PATH-sparse `bash` resolution, `stop_hook_active` continuation guard, anchored proposed-plan marker matching, expanded test coverage (7 -> 15 cases), and refreshed handoff plan/results docs.
> **Sync resolution:** Took upstream (`--theirs`) for every conflicting hook script, `tests/hooks/test-codex-hooks.sh`, `.codex-plugin/plugin.json`, and both `docs/plans/2026-05-21-codex-hooks-*.md`. README hook-table rows resolved to upstream wording while preserving this branch's `research-project-setup` rows (intro item #4 and the utility-skills row) that upstream does not have. The four cleanly auto-merged files (`hooks/exit-plan-mode`, `hooks/run-hook.cmd`, `skills/agent-orchestration/references/worktree-harness-fallback.md`, `skills/using-superRA/references/codex-instructions.md`) carry the upstream polish as well.
> **Integration context:** Branch commits `9eb21be`, `740c876`, `9bb7d8a`, `2509ee9`, `3b5de66`, `6e67655` (this branch's earlier Codex-hooks work and its bookkeeping commits) are now redundant — their content is fully represented by upstream `d861089`. Integrate should treat them as superseded; no follow-up code edit is required because the working tree already matches upstream for every overlapping path. If a future PR-friendly diff against `origin/main` is desired, this branch's diff for `hooks/`, `tests/hooks/`, `.codex-plugin/plugin.json`, `docs/plans/2026-05-21-codex-hooks-*.md` is now empty; only `skills/research-project-setup/**`, `README.md` rows for that skill, and PLAN.md/RESULTS.md remain as net-new contribution.
> **User decision:** None — upstream is a strict superset for every conflict, so the resolution did not change intent.

### Verification

- `bash tests/hooks/test-codex-hooks.sh` -> 15/15 PASS on the merged tree.
- Stale-reference sweep across `skills/research-project-setup/` for `ask-user-question-logger`, `exit-plan-mode`, `codex-plan-stop`, `hooks-codex`, `run-hook` -> no matches; the bundled `template/.codex/config.toml` declares agents and sandbox roots only, no hook references.
