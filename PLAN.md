...
> **For agentic workers:** REQUIRED DISCIPLINE: Use `superRA:handoff-doc` for all PLAN.md / RESULTS.md editing. Use `superRA:execution-workflow` to execute this plan. Skill changes — see `CLAUDE.md` §Skill Changes. Steps use checkbox (`- [ ]`) syntax for tracking and cross-session handoff.
...
> **User decision (2026-04-18, integration):** (a) Add a directive to `skills/using-superRA/SKILL.md` establishing that whenever there is a significant plan change — a new task, or a material update to an existing task — the agent re-enters `planning-workflow §Changing Plans`. (b) Update `CLAUDE.md` and `README.md` to reframe the superRA workflow as **iterative**, not linear: PLAN → IMPLEMENT → VALIDATE → INTEGRATE is not a one-shot pipeline, phases cycle, and `planning-workflow §Changing Plans` is the hinge that makes re-entry safe. This is a natural extension of the iterative re-entry mechanism already on this branch.
> **Question asked:** (implicit, during integration) Where should the "significant plan change → re-enter planning-workflow" rule live, and how should the top-level framing reflect the workflow's iterative nature?
> **Rationale:** The iterative re-entry mechanism this PR introduces is currently described only inside `planning-workflow §Changing Plans`. Adding it to `using-superRA` surfaces it at the level every agent reads; updating `CLAUDE.md` / `README.md` makes the iterative framing visible to contributors and new users who read project docs before diving into skills.
...
- [x] **Step 1: Author `## Changing Plans` in `planning-workflow/SKILL.md`.** Added as top-level section after "Living Plan and Results Docs" and before "No Placeholders". Preamble collapsed (post-review refinement 2026-04-17) to a single framing — "when the plan changes, follow this protocol" — because the prior "(a) in-execution / (b) post-integration" split encoded a phase distinction the body doesn't honor (the Material bullets are a flat list). Body carries the 6-step protocol, DAG cascade paragraph, and banned-shortcuts bullets verbatim from handoff-doc, with internal pointers retargeted to `handoff-doc/references/plan-anatomy.md` and §User Decisions Log references to `handoff-doc` §User Decisions Log. Living Plan paragraph's cross-ref retargeted to §Changing Plans below.
...
- [x] **Step 1: `execution-workflow/SKILL.md`** — two edits:
  - Rewrote Step 1 sub-step 2a (refined post-review 2026-04-17) to reframe the box/status mismatch as *resumption* of a paused `§Changing Plans` run, not an independent detection trigger: the mismatch exists only because Step 4 of that protocol unchecked the boxes as a cross-session persistence marker, so resuming at Step 6 is the correct handoff. Mid-execution researcher pings enter `§Changing Plans` at Step 1 instead.
