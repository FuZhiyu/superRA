...
>    → implemented: rewrote items 5 and 6 of the "Before drafting or reviewing, read" list at `docs/writing-references/README.md:40-41` — collapsed the two stale entries into one entry pointing at the redesign plan + `/CLAUDE.md §Design Principles` + LRS argument slides for `review.md` / `polish.md` / `draft.md`, and renumbered the `integration.md` entry. Broader grep `grep -rn 'collaboration\.md\|workflow\.md\|planning\.md' docs/` clean except for an unrelated `planning-workflow/references/planning.md` mention in `docs/process-issues-2026-04-16.md`.
>
...
>    → implemented: removed the two superseded sync sections (`SEMANTIC_MERGE.md:57-178`) — the 2026-05-02 record and its correction follow-up. The pre-existing `tighten-integration-rules` block at lines 1-55 stays (file now 53 lines); current-sync record is in `PLAN.md ## Sync Map`.
